...
#define eCV5546_PLANE_READ(offset) readl(sp_dev->crtc_regs + (offset))
#define eCV5546_PLANE_WRITE(offset, val) writel(val, sp_dev->crtc_regs + (offset))

/* ecv5546-hw-format translate table */
struct ecv5546_plane_format {
	u32 pixel_format;
	u32 hw_format;
};

static const u64 ecv5546_kms_modifiers[] = {
...
static const u32 ecv5546_kms_vpp_formats[5] = {
	DRM_FORMAT_YUYV,  /* eCV5546_VPP_IMGREAD_DATA_FMT_YUY2 */
	DRM_FORMAT_UYVY,  /* eCV5546_VPP_IMGREAD_DATA_FMT_UYVY */
	DRM_FORMAT_NV12,  /* eCV5546_VPP_IMGREAD_DATA_FMT_NV12 */
	DRM_FORMAT_NV16,  /* eCV5546_VPP_IMGREAD_DATA_FMT_NV16 */
	DRM_FORMAT_NV24,  /* eCV5546_VPP_IMGREAD_DATA_FMT_NV24 */
};

static const u32 ecv5546_kms_osd_formats[8] = {
	DRM_FORMAT_C8,        /* eCV5546_OSD_COLOR_MODE_8BPP ??? */
	DRM_FORMAT_YUYV,      /* eCV5546_OSD_COLOR_MODE_YUY2 */
	DRM_FORMAT_RGB565,    /* eCV5546_OSD_COLOR_MODE_RGB565 */
	DRM_FORMAT_RGBA4444,  /* eCV5546_OSD_COLOR_MODE_RGBA4444 */
	DRM_FORMAT_ARGB4444,  /* eCV5546_OSD_COLOR_MODE_ARGB4444 */
	DRM_FORMAT_ARGB1555,  /* eCV5546_OSD_COLOR_MODE_ARGB1555 */
	DRM_FORMAT_RGBA8888,  /* eCV5546_OSD_COLOR_MODE_RGBA8888 */
	DRM_FORMAT_ARGB8888,  /* eCV5546_OSD_COLOR_MODE_ARGB8888 */
	//DRM_FORMAT_XRGB8888,  /* eCV5546_OSD_COLOR_MODE_ARGB8888 */
	//DRM_FORMAT_RGBX8888,  /* eCV5546_OSD_COLOR_MODE_RGBA8888 */
	//DRM_FORMAT_RGBX4444,  /* eCV5546_OSD_COLOR_MODE_RGBA4444 */
	//DRM_FORMAT_XRGB4444,  /* eCV5546_OSD_COLOR_MODE_ARGB4444 */
	//DRM_FORMAT_XRGB1555,  /* eCV5546_OSD_COLOR_MODE_ARGB1555 */
};

static const struct ecv5546_plane_format ecv5546_vpp_formats[] = {
	{ DRM_FORMAT_YUYV, eCV5546_VPP_IMGREAD_DATA_FMT_YUY2 },
	{ DRM_FORMAT_UYVY, eCV5546_VPP_IMGREAD_DATA_FMT_UYVY },
	{ DRM_FORMAT_NV12, eCV5546_VPP_IMGREAD_DATA_FMT_NV12 },
	{ DRM_FORMAT_NV16, eCV5546_VPP_IMGREAD_DATA_FMT_NV16 },
...
static const struct ecv5546_plane_format ecv5546_osd_formats[] = {
	{ DRM_FORMAT_C8, eCV5546_OSD_COLOR_MODE_8BPP },
	/* 16bpp RGB: */
	{ DRM_FORMAT_RGB565, eCV5546_OSD_COLOR_MODE_RGB565 },
	{ DRM_FORMAT_ARGB1555, eCV5546_OSD_COLOR_MODE_ARGB1555 },
...
#define eCV5546_FORMAT_UNSUPPORT 0xF

static void ecv5546_drm_plane_set(struct drm_plane *plane, int fg_sel, int layer_mode)
{
	struct ecv5546_plane *sp_plane = to_ecv5546_plane(plane);
	struct drm_device *drm = sp_plane->base.dev;
	struct ecv5546_dev *sp_dev = to_ecv5546_dev(drm);
	u32 value, value1, tmp_value;
	u32 layer, i;

	/* Find Layer number */
	value = eCV5546_PLANE_READ(DMIX_LAYER_CONFIG_0);
	for (i = 0; i < eCV5546_DMIX_MAX_LAYER; i++) {
	...
	/* Set layer mode */
	value1 = eCV5546_PLANE_READ(DMIX_LAYER_CONFIG_1);
	if (layer != eCV5546_DMIX_BG) {
		value1 &= ~(GENMASK(1, 0) << ((layer - 1) << 1));
		value1 |= (layer_mode << ((layer - 1) << 1));
	}
	eCV5546_PLANE_WRITE(DMIX_LAYER_CONFIG_1, value1);
}

static int ecv5546_vpp_plane_vpost_opif_alpha_set(struct drm_plane *plane, int alpha, int mask_alpha)
{
	struct ecv5546_plane *sp_plane = to_ecv5546_plane(plane);
	struct drm_device *drm = sp_plane->base.dev;
	struct ecv5546_dev *sp_dev = to_ecv5546_dev(drm);
	...
	value = eCV5546_PLANE_READ(VPOST_OPIF_CONFIG);
	value |= eCV5546_VPP_VPOST_WIN_ALPHA_EN;
	eCV5546_PLANE_WRITE(VPOST_OPIF_CONFIG, value);

	/*set alpha value*/
	value = eCV5546_PLANE_READ(VPOST_OPIF_ALPHA);
	value &= ~(eCV5546_VPP_VPOST_WIN_ALPHA_MASK | eCV5546_VPP_VPOST_VPP_ALPHA_MASK);
	value |= (eCV5546_VPP_VPOST_WIN_ALPHA_SET(mask_alpha) |
		eCV5546_VPP_VPOST_VPP_ALPHA_SET(alpha));
	eCV5546_PLANE_WRITE(VPOST_OPIF_ALPHA, value);

	return 0;
}

static int ecv5546_vpp_plane_imgread_set(struct drm_plane *plane, u32 data_addr1,
								 ...
								 int input_w, int input_h, int yuv_fmt)
{
	...
	value = eCV5546_PLANE_READ(IMGREAD_GLOBAL_CONTROL);
	value |= eCV5546_VPP_IMGREAD_FETCH_EN;
	eCV5546_PLANE_WRITE(IMGREAD_GLOBAL_CONTROL, value);

	value = eCV5546_PLANE_READ(IMGREAD_CONFIG);
	value &= ~(eCV5546_VPP_IMGREAD_DATA_FMT |
		eCV5546_VPP_IMGREAD_YC_SWAP | eCV5546_VPP_IMGREAD_UV_SWAP);
	if (yuv_fmt == eCV5546_VPP_IMGREAD_DATA_FMT_YUY2)
		value |= eCV5546_VPP_IMGREAD_DATA_FMT_SEL(eCV5546_VPP_IMGREAD_DATA_FMT_YUY2);
	else if (yuv_fmt == eCV5546_VPP_IMGREAD_DATA_FMT_NV12)
		value |= eCV5546_VPP_IMGREAD_DATA_FMT_SEL(eCV5546_VPP_IMGREAD_DATA_FMT_NV12);
	else if (yuv_fmt == eCV5546_VPP_IMGREAD_DATA_FMT_NV16)
		value |= eCV5546_VPP_IMGREAD_DATA_FMT_SEL(eCV5546_VPP_IMGREAD_DATA_FMT_NV16);
	else if (yuv_fmt == eCV5546_VPP_IMGREAD_DATA_FMT_NV24)
		value |= eCV5546_VPP_IMGREAD_DATA_FMT_SEL(eCV5546_VPP_IMGREAD_DATA_FMT_NV24);
	else if (yuv_fmt == eCV5546_VPP_IMGREAD_DATA_FMT_UYVY)
		value |= eCV5546_VPP_IMGREAD_DATA_FMT_SEL(eCV5546_VPP_IMGREAD_DATA_FMT_YUY2) |
			eCV5546_VPP_IMGREAD_YC_SWAP;
	else
		value |= eCV5546_VPP_IMGREAD_DATA_FMT_SEL(eCV5546_VPP_IMGREAD_DATA_FMT_YUY2);

	value |= eCV5546_VPP_IMGREAD_FM_MODE;

	//if (disp_dev->out_res.mipitx_mode == eCV5546_MIPITX_DSI)
	//	;//TBD
	//else
	//	value |= eCV5546_VPP_IMGREAD_UV_SWAP; //for CSI YUY2 test only
	eCV5546_PLANE_WRITE(IMGREAD_CONFIG, value);

	value = 0;
	//#if (eCV5546_VPP_SCALE_METHOD == 1) //method 1 , set imgread xstart&ystart
	value |= (y << 16) | (x << 0);
	//#endif
	eCV5546_PLANE_WRITE(IMGREAD_CROP_START, value);

	value = 0;
	value |= (img_src_h << 16) | (img_src_w << 0);
	eCV5546_PLANE_WRITE(IMGREAD_FRAME_SIZE, value);

	value = 0;
	if ((yuv_fmt == eCV5546_VPP_IMGREAD_DATA_FMT_YUY2) ||
		 (yuv_fmt == eCV5546_VPP_IMGREAD_DATA_FMT_UYVY))
	...
	eCV5546_PLANE_WRITE(IMGREAD_LINE_STRIDE_SIZE, value);

	eCV5546_PLANE_WRITE(IMGREAD_DATA_ADDRESS_1, (u32)data_addr1);
	eCV5546_PLANE_WRITE(IMGREAD_DATA_ADDRESS_2, (u32)data_addr1 + (input_w * input_h));

	return 0;
}

static int ecv5546_vpp_plane_vscl_set(struct drm_plane *plane, int x, int y, int img_src_w, int img_src_h,
							  ...
							  int output_w, int output_h)
{
	...
	value = eCV5546_PLANE_READ(VSCL_CONFIG2);
	value |= (eCV5546_VPP_VSCL_BUFR_EN |
		eCV5546_VPP_VSCL_VINT_EN |
		eCV5546_VPP_VSCL_HINT_EN |
		eCV5546_VPP_VSCL_DCTRL_EN |
		eCV5546_VPP_VSCL_ACTRL_EN);
	eCV5546_PLANE_WRITE(VSCL_CONFIG2, value);
	...
	eCV5546_PLANE_WRITE(VSCL_ACTRL_I_XLEN, crop_xlen);
	eCV5546_PLANE_WRITE(VSCL_ACTRL_I_YLEN, crop_ylen);

	//#if (eCV5546_VPP_SCALE_METHOD == 1) //method 1 , set imgread xstart&ystart, fix 0 here
	eCV5546_PLANE_WRITE(VSCL_ACTRL_S_XSTART, 0);
	eCV5546_PLANE_WRITE(VSCL_ACTRL_S_YSTART, 0);
	//#elif (eCV5546_VPP_SCALE_METHOD == 2) //method 2 , set vscl xstart&ystart
	//eCV5546_PLANE_WRITE(VSCL_ACTRL_S_XSTART, x);
	//eCV5546_PLANE_WRITE(VSCL_ACTRL_S_YSTART, y);
	//#endif

	//#if 1 //crop left & top only
	eCV5546_PLANE_WRITE(VSCL_ACTRL_S_XLEN, crop_xlen);
	eCV5546_PLANE_WRITE(VSCL_ACTRL_S_YLEN, crop_ylen);
	//#else //crop left & top & right & bot
	//eCV5546_PLANE_WRITE(VSCL_ACTRL_S_XLEN, crop_xlen - x);
	//eCV5546_PLANE_WRITE(VSCL_ACTRL_S_YLEN, crop_ylen - y);
	//#endif

	if (img_dest_x < 0)
		eCV5546_PLANE_WRITE(VSCL_DCTRL_D_XSTART, 0-img_dest_x);
	else
		eCV5546_PLANE_WRITE(VSCL_DCTRL_D_XSTART, img_dest_x);
	if (img_dest_y < 0)
		eCV5546_PLANE_WRITE(VSCL_DCTRL_D_YSTART, 0-img_dest_y);
	else
		eCV5546_PLANE_WRITE(VSCL_DCTRL_D_YSTART, img_dest_y);

	eCV5546_PLANE_WRITE(VSCL_DCTRL_D_XLEN, img_dest_w);
	eCV5546_PLANE_WRITE(VSCL_DCTRL_D_YLEN, img_dest_h);

	eCV5546_PLANE_WRITE(VSCL_DCTRL_O_XLEN, output_w);
	eCV5546_PLANE_WRITE(VSCL_DCTRL_O_YLEN, output_h);

	/*
	 * VSCL SETTING for HORIZONTAL
	 */
	value = eCV5546_PLANE_READ(VSCL_HINT_CTRL);
	value |= eCV5546_VPP_VSCL_HINT_FLT_EN;
	eCV5546_PLANE_WRITE(VSCL_HINT_CTRL, value);

	source_w = eCV5546_PLANE_READ(VSCL_ACTRL_S_XLEN);
	/* cal h_factor */
	if (source_w == img_dest_w)
		...
		//#if (eCV5546_VPP_FACTOR_METHOD == 0)
		factor64 -= 1;
		//#endif
	}
	value = ((u32)factor64 >> 0) & 0x0000ffff;
	eCV5546_PLANE_WRITE(VSCL_HINT_HFACTOR_LOW, value);
	value = ((u32)factor64 >> 16) & 0x000001ff;
	eCV5546_PLANE_WRITE(VSCL_HINT_HFACTOR_HIGH, value);
	...
	if (source_w == img_dest_w)
		...
		//#if (eCV5546_VPP_FACTOR_METHOD == 1)
		//factor_init64 = img_dest_w - factor_init64;
		//#endif
	}
	value = ((u32)factor_init64 >> 0) & 0x0000ffff;
	eCV5546_PLANE_WRITE(VSCL_HINT_INITF_LOW, value);
	value = ((u32)factor_init64 >> 16) & 0x0000003f;
	//#if (eCV5546_VPP_FACTOR_METHOD == 1)
	//value |= eCV5546_VPP_VSCL_HINT_INITF_PN;
	//#endif
	eCV5546_PLANE_WRITE(VSCL_HINT_INITF_HIGH, value);

	/*
	 * VSCL SETTING for VERTICAL
	 */
	value = eCV5546_PLANE_READ(VSCL_VINT_CTRL);
	value |= eCV5546_VPP_VSCL_VINT_FLT_EN;
	eCV5546_PLANE_WRITE(VSCL_VINT_CTRL, value);

	source_h = eCV5546_PLANE_READ(VSCL_ACTRL_S_YLEN);
	/* cal v_factor */
	...
	value = ((u32)factor64 >> 0) & 0x0000ffff;
	eCV5546_PLANE_WRITE(VSCL_VINT_VFACTOR_LOW, value);
	value = ((u32)factor64 >> 16) & 0x000001ff;
	eCV5546_PLANE_WRITE(VSCL_VINT_VFACTOR_HIGH, value);
	...
	value = ((u32)factor_init64 >> 0) & 0x0000ffff;
	eCV5546_PLANE_WRITE(VSCL_VINT_INITF_LOW, value);
	value = ((u32)factor_init64 >> 16) & 0x0000003f;
	//#if (eCV5546_VPP_FACTOR_METHOD == 1)
	//value |= eCV5546_VPP_VSCL_VINT_INITF_PN;
	//#endif
	eCV5546_PLANE_WRITE(VSCL_VINT_INITF_HIGH, value);

	return 0;
}

#define eCV5546_VPP_VPOST_WIN_ALPHA_VALUE	0xff
#define eCV5546_VPP_VPOST_VPP_ALPHA_VALUE	0xff

int ecv5546_vpp_plane_vpost_opif_set(struct drm_plane *plane, int act_x, int act_y,
									 int act_w, int act_h, int output_w, int output_h)
{
	 ...
	 * VPOST SETTING
	 */
	value = eCV5546_PLANE_READ(VPOST_CONFIG);
	value |= eCV5546_VPP_VPOST_OPIF_EN;
	eCV5546_PLANE_WRITE(VPOST_CONFIG, value);

	value = eCV5546_PLANE_READ(VPOST_OPIF_CONFIG);
	value |= eCV5546_VPP_VPOST_WIN_ALPHA_EN | eCV5546_VPP_VPOST_WIN_YUV_EN;
	eCV5546_PLANE_WRITE(VPOST_OPIF_CONFIG, value);

	/*set alpha value*/
	value = eCV5546_PLANE_READ(VPOST_OPIF_ALPHA);
	value &= ~(eCV5546_VPP_VPOST_WIN_ALPHA_MASK | eCV5546_VPP_VPOST_VPP_ALPHA_MASK);
	value |= (eCV5546_VPP_VPOST_WIN_ALPHA_SET(0) |
		eCV5546_VPP_VPOST_VPP_ALPHA_SET(eCV5546_VPP_VPOST_VPP_ALPHA_VALUE));
	eCV5546_PLANE_WRITE(VPOST_OPIF_ALPHA, value);

	/*set mask region*/
	value = eCV5546_PLANE_READ(VPOST_OPIF_MSKTOP);
	value &= ~eCV5546_VPP_VPOST_OPIF_TOP_MASK;
	if (act_y > 0)
		value |= eCV5546_VPP_VPOST_OPIF_TOP_SET(act_y);
	eCV5546_PLANE_WRITE(VPOST_OPIF_MSKTOP, value);

	value = eCV5546_PLANE_READ(VPOST_OPIF_MSKBOT);
	value &= ~eCV5546_VPP_VPOST_OPIF_BOT_MASK;
	if (act_h + act_y < output_h)
		value |= eCV5546_VPP_VPOST_OPIF_BOT_SET(output_h - act_h - act_y);
	eCV5546_PLANE_WRITE(VPOST_OPIF_MSKBOT, value);

	value = eCV5546_PLANE_READ(VPOST_OPIF_MSKLEFT);
	value &= ~eCV5546_VPP_VPOST_OPIF_LEFT_MASK;
	if (act_x > 0)
		value |= eCV5546_VPP_VPOST_OPIF_LEFT_SET(act_x);
	eCV5546_PLANE_WRITE(VPOST_OPIF_MSKLEFT, value);

	value = eCV5546_PLANE_READ(VPOST_OPIF_MSKRIGHT);
	value &= ~eCV5546_VPP_VPOST_OPIF_RIGHT_MASK;
	if (act_w + act_x < output_w)
		value |= eCV5546_VPP_VPOST_OPIF_RIGHT_SET(output_w - act_w - act_x);
	eCV5546_PLANE_WRITE(VPOST_OPIF_MSKRIGHT, value);

	return 0;
}

int ecv5546_vpp_vpost_adj_enable(struct drm_plane *plane, int enable)
{
	struct ecv5546_plane *sp_plane = to_ecv5546_plane(plane);
	struct ecv5546_dev *sp_dev = to_ecv5546_dev(sp_plane->base.dev);
	u32 value;


	value = eCV5546_PLANE_READ(VPOST_CONFIG);
	if (enable)
		value |= eCV5546_VPP_VPOST_ADJ_EN;
	else
		value &= ~eCV5546_VPP_VPOST_ADJ_EN;
...
int ecv5546_vpp_vpost_adj_turning_point_set(struct drm_plane *plane,
						const u8 *cp_src, const u8 *cp_sdt, u32 cp_size)
{
	...
	eCV5546_PLANE_WRITE(VPOST_ADJ_SRC, value);

	value = 0;
	value = cp_sdt[0] + (cp_sdt[1] << 8);
	eCV5546_PLANE_WRITE(VPOST_ADJ_DES, value);

	return 0;
}

int ecv5546_vpp_vpost_adj_slope_set(struct drm_plane *plane, const u16 *slope, u32 slope_size)
{
	struct ecv5546_plane *sp_plane = to_ecv5546_plane(plane);
	struct ecv5546_dev *sp_dev = to_ecv5546_dev(sp_plane->base.dev);
	u32 value;
	...
	value = slope[0] & 0x1FF;
	eCV5546_PLANE_WRITE(VPOST_ADJ_SLOPE0, value);

	value = slope[1] & 0x1FF;
	eCV5546_PLANE_WRITE(VPOST_ADJ_SLOPE1, value);

	value = slope[2] & 0x1FF;
	eCV5546_PLANE_WRITE(VPOST_ADJ_SLOPE2, value);

	return 0;
}

int ecv5546_osd_gpost_contrast_adj_enable(struct drm_plane *plane, int enable)
{
	struct ecv5546_plane *sp_plane = to_ecv5546_plane(plane);
	struct ecv5546_dev *sp_dev = to_ecv5546_dev(sp_plane->base.dev);
	u32 value;
	...
	value = eCV5546_PLANE_READ(GPOST_CONTRAST_CONFIG + (sp_plane->osd_layer_sel << 7));
	if (enable)
		value |= 0x1;
	else
		value &= ~0x1;

	eCV5546_PLANE_WRITE(GPOST_CONTRAST_CONFIG + (sp_plane->osd_layer_sel << 7), value);

	return 0;
}

int ecv5546_osd_gpost_contrast_adj_turning_point_set(struct drm_plane *plane,
						const u8 *cp_src, const u8 *cp_sdt, u32 cp_size)
{
	struct ecv5546_plane *sp_plane = to_ecv5546_plane(plane);
	struct ecv5546_dev *sp_dev = to_ecv5546_dev(sp_plane->base.dev);
	...
	value = 0;
	value = cp_src[0] + (cp_src[1] << 8);
	eCV5546_PLANE_WRITE(GPOST_ADJ_SRC + (sp_plane->osd_layer_sel << 7), value);

	value = 0;
	value = cp_sdt[0] + (cp_sdt[1] << 8);
	eCV5546_PLANE_WRITE(GPOST_ADJ_DES + (sp_plane->osd_layer_sel << 7), value);

	return 0;
}

int ecv5546_osd_gpost_contrast_adj_slope_set(struct drm_plane *plane,
						const u16 *slope, u32 slope_size)
{
	...
	value = slope[0] & 0x1FF;
	eCV5546_PLANE_WRITE(GPOST_ADJ_SLOPE0 + (sp_plane->osd_layer_sel << 7), value);

	value = slope[1] & 0x1FF;
	eCV5546_PLANE_WRITE(GPOST_ADJ_SLOPE1 + (sp_plane->osd_layer_sel << 7), value);

	value = slope[2] & 0x1FF;
	eCV5546_PLANE_WRITE(GPOST_ADJ_SLOPE2 + (sp_plane->osd_layer_sel << 7), value);

	return 0;
}

int ecv5546_osd_gpost_brightness_adj_set(struct drm_plane *plane, int brightness)
{
	struct ecv5546_plane *sp_plane = to_ecv5546_plane(plane);
	struct ecv5546_dev *sp_dev = to_ecv5546_dev(sp_plane->base.dev);
	u32 value;
	...
	eCV5546_PLANE_WRITE(GPOST_BRI_VALUE + (sp_plane->osd_layer_sel << 7), value);

	return 0;
}

int ecv5546_vpp_vpost_adj_luma_boundary_set(struct drm_plane *plane, u8 upper, u8 lower)
{
	struct ecv5546_plane *sp_plane = to_ecv5546_plane(plane);
	struct ecv5546_dev *sp_dev = to_ecv5546_dev(sp_plane->base.dev);
	u32 value;

	//value = eCV5546_PLANE_READ(VPOST_ADJ_BOUND);
	value = 0;
	value = (upper << 8) | lower;
	eCV5546_PLANE_WRITE(VPOST_ADJ_BOUND, value);
	 ...
	 */
	value = eCV5546_PLANE_READ(VPOST_ADJ_CONFIG);
	value |= (0x3 << 2);
	eCV5546_PLANE_WRITE(VPOST_ADJ_CONFIG, value);

	return 0;
}

static u32 ecv5546_plane_brightness_setting(struct drm_plane *plane, int brightness)
{
	struct ecv5546_plane *sp_plane = to_ecv5546_plane(plane);
	...
	if (sp_plane->is_media_plane) {
		...
		slope[0] = slope[2] = 0;
		slope[1] = 0x100;
		ecv5546_vpp_vpost_adj_turning_point_set(plane, srcY, destY, 2);
		ecv5546_vpp_vpost_adj_slope_set(plane, slope, 3);
		ecv5546_vpp_vpost_adj_enable(plane, 1);
	} else {
		/* [-128, 127] */
		ecv5546_osd_gpost_brightness_adj_set(plane, brightness);
	}
	return 0;
}

static u32 ecv5546_plane_contrast_setting(struct drm_plane *plane, int contrast)
{
	struct ecv5546_plane *sp_plane = to_ecv5546_plane(plane);
	u8 srcY[2];
	u8 destY[2];
	...
	if (sp_plane->is_media_plane) {
		...
		ecv5546_osd_gpost_contrast_adj_turning_point_set(plane, srcY, destY, 2);
		ecv5546_osd_gpost_contrast_adj_slope_set(plane, slope, 3);
		ecv5546_osd_gpost_contrast_adj_enable(plane, 1);
	}

	return 0;
}

static void ecv5546_drm_plane_alpha_config(struct drm_plane *plane, int layer, int enable,
								    int fix_alpha, int alpha_value)
{
	...
	value1 = eCV5546_PLANE_READ(DMIX_PLANE_ALPHA_CONFIG_0);
	value2 = eCV5546_PLANE_READ(DMIX_PLANE_ALPHA_CONFIG_1);

	switch (layer) {
	case eCV5546_DMIX_L1:
		value1 &= ~(GENMASK(15, 8));
		value1 |= FIELD_PREP(GENMASK(15, 15), enable) |
			FIELD_PREP(GENMASK(14, 14), fix_alpha) |
			FIELD_PREP(GENMASK(13, 8), alpha_value);
		break;
	case eCV5546_DMIX_L2:
		break;
	case eCV5546_DMIX_L3:
	...
	case eCV5546_DMIX_L4:
		...
		break;
	case eCV5546_DMIX_L5:
		value2 &= ~(GENMASK(15, 8));
		value2 |= FIELD_PREP(GENMASK(15, 15), enable) |
			FIELD_PREP(GENMASK(14, 14), fix_alpha) |
			FIELD_PREP(GENMASK(13, 8), alpha_value);
		break;
	case eCV5546_DMIX_L6:
		value2 &= ~(GENMASK(7, 0));
	...
	eCV5546_PLANE_WRITE(DMIX_PLANE_ALPHA_CONFIG_0, value1);
	eCV5546_PLANE_WRITE(DMIX_PLANE_ALPHA_CONFIG_1, value2);
}

static void ecv5546_osd_plane_init(struct drm_plane *plane)
{
	struct ecv5546_plane *sp_plane = to_ecv5546_plane(plane);
	struct drm_device *drm = sp_plane->base.dev;
	struct ecv5546_dev *sp_dev = to_ecv5546_dev(drm);
	...
	/*
	 * update ecv5546 osd layer register
	 */
	value = 0;
	value = OSD_CTRL_COLOR_MODE_RGB
		...
		| OSD_CTRL_A32B32_EN
		| OSD_CTRL_FIFO_DEPTH;
	eCV5546_PLANE_WRITE((OSD_CTRL + (sp_plane->osd_layer_sel << 7)), value);

	eCV5546_PLANE_WRITE((OSD_HVLD_OFFSET + (sp_plane->osd_layer_sel << 7)), 0);
	eCV5546_PLANE_WRITE((OSD_VVLD_OFFSET + (sp_plane->osd_layer_sel << 7)), 0);

	/* TODO: How get display mode? */
	//eCV5546_PLANE_WRITE((OSD_HVLD_WIDTH + (sp_plane->osd_layer_sel << 7)),
	//	plane->crtc->mode.hdisplay);
	//eCV5546_PLANE_WRITE((OSD_VVLD_HEIGHT + (sp_plane->osd_layer_sel << 7)),
	//	plane->crtc->mode.vdisplay);

	eCV5546_PLANE_WRITE((OSD_BIST_CTRL + (sp_plane->osd_layer_sel << 7)), 0);
	eCV5546_PLANE_WRITE((OSD_3D_H_OFFSET + (sp_plane->osd_layer_sel << 7)), 0);
	eCV5546_PLANE_WRITE((OSD_SRC_DECIMATION_SEL + (sp_plane->osd_layer_sel << 7)), 0);
	eCV5546_PLANE_WRITE((OSD_EN + (sp_plane->osd_layer_sel << 7)), 1);

	//GPOST bypass
	eCV5546_PLANE_WRITE((GPOST_CONFIG + (sp_plane->osd_layer_sel << 7)), 0);
	eCV5546_PLANE_WRITE((GPOST_MASTER_EN + (sp_plane->osd_layer_sel << 7)), 0);
	eCV5546_PLANE_WRITE((GPOST_BG1 + (sp_plane->osd_layer_sel << 7)), 0x8010);
	eCV5546_PLANE_WRITE((GPOST_BG2 + (sp_plane->osd_layer_sel << 7)), 0x0080);

	//GPOST PQ disable
	eCV5546_PLANE_WRITE((GPOST_CONTRAST_CONFIG + (sp_plane->osd_layer_sel << 7)), 0);
}

static void ecv5546_osd_plane_set_by_region(struct drm_plane *plane, struct ecv5546_osd_region *info)
{
	struct ecv5546_plane *sp_plane = to_ecv5546_plane(plane);
	struct drm_device *drm = sp_plane->base.dev;
	struct ecv5546_dev *sp_dev = to_ecv5546_dev(drm);
	...
	//tmp_alpha = eCV5546_OSD_HDR_BL | eCV5546_OSD_HDR_ALPHA;
	//value |= (tmp_color_mode << 24) | eCV5546_OSD_HDR_BS | tmp_alpha;
	//value = osd_header[0];

	value |= (tmp_color_mode << 24) | eCV5546_OSD_HDR_BS;

	/* setting region alpha */
	if (info->alpha_info.region_alpha_en)
		value |= eCV5546_OSD_HDR_BL | info->alpha_info.region_alpha;

	if (info->alpha_info.color_key_en)
		value |= eCV5546_OSD_HDR_KEY;

	if (info->color_mode == eCV5546_OSD_COLOR_MODE_8BPP)
		value |= eCV5546_OSD_HDR_CULT;
	...
	if (info->color_mode == eCV5546_OSD_COLOR_MODE_YUY2)
		value |= eCV5546_OSD_HDR_CSM_SET(eCV5546_OSD_CSM_BYPASS);
	else
		value |= eCV5546_OSD_HDR_CSM_SET(eCV5546_OSD_CSM_RGB_BT601);
	...
	eCV5546_PLANE_WRITE((OSD_BASE_ADDR + (sp_plane->osd_layer_sel << 7)),
...
#define eCV5546_PLANE_TYPE_OSD 0
#define eCV5546_PLANE_TYPE_VPP 1
static u32 ecv5546_get_plane_format(u32 pixel_format, int plane_type)
{
	int i;

	if (plane_type == eCV5546_PLANE_TYPE_VPP) {
		for (i = 0; i < ARRAY_SIZE(ecv5546_vpp_formats); i++)
			if (ecv5546_vpp_formats[i].pixel_format == pixel_format)
				return ecv5546_vpp_formats[i].hw_format;
	} else {
		for (i = 0; i < ARRAY_SIZE(ecv5546_osd_formats); i++)
			if (ecv5546_osd_formats[i].pixel_format == pixel_format)
				return ecv5546_osd_formats[i].hw_format;
	}

	/* not found */
	DRM_DEBUG_DRIVER("Not found pixel format!!fourcc_format= %d\n",
			 pixel_format);
	return eCV5546_FORMAT_UNSUPPORT;
}

static int ecv5546_plane_atomic_set_property(struct drm_plane *plane,
					    struct drm_plane_state *state,
					    struct drm_property *property,
					    uint64_t val)
{
	struct ecv5546_plane *sp_plane = to_ecv5546_plane(plane);
	struct ecv5546_plane_state *sp_state = to_ecv5546_plane_state(state);
	...
	if (!strcmp(property->name, "win alpha")) {
		...
		if (!(sp_plane->capabilities & eCV5546_DRM_PLANE_CAP_WIN_BLEND)) {
		...
		if (!(sp_plane->capabilities & eCV5546_DRM_PLANE_CAP_COLOR_KEYING)) {
			DRM_DEBUG_ATOMIC("the property isn't supported by the driver!\n");
			return -EINVAL;
		}
		...
		struct ecv5546_plane_region_color_keying_info *info;

		if (!(sp_plane->capabilities & eCV5546_DRM_PLANE_CAP_REGION_COLOR_KEYING)) {
			DRM_DEBUG_ATOMIC("the property isn't supported by the driver!\n");
			return -EINVAL;
		}

		if (blob->length != sizeof(struct ecv5546_plane_region_color_keying_info)) {
		...
		if (!(sp_plane->capabilities & eCV5546_DRM_PLANE_CAP_SCALE)) {
			DRM_DEBUG_ATOMIC("the property isn't supported by the driver!\n");
			return -EINVAL;
		}
...
static int ecv5546_plane_atomic_get_property(struct drm_plane *plane,
					    ...
					    uint64_t *val)
{
	...
	if ((property == sp_plane->win_alpha_property) &&
		(sp_plane->capabilities & eCV5546_DRM_PLANE_CAP_WIN_BLEND)) {
		*val = sp_state->win_alpha;

	} else if ((property == sp_plane->region_color_keying_property) &&
		       (sp_plane->capabilities & eCV5546_DRM_PLANE_CAP_REGION_COLOR_KEYING)) {
		*val = (sp_plane->region_color_keying_blob) ?
			 sp_plane->region_color_keying_blob->base.id : 0;
		       ...
		       (sp_plane->capabilities & eCV5546_DRM_PLANE_CAP_COLOR_KEYING)) {
		*val = sp_state->color_keying;

	} else if (!strcmp(property->name, "SCL_ADJ") &&
		      (sp_plane->capabilities & eCV5546_DRM_PLANE_CAP_SCALE)) {
...
/**
 * ecv5546_plane_atomic_duplicate_state - ecv5546 state duplicate hook
 * @plane: drm plane
 */
static struct drm_plane_state *
ecv5546_plane_atomic_duplicate_state(struct drm_plane *plane)
{
	struct ecv5546_plane_state *sp_state;

	if (WARN_ON(!plane->state))
		return NULL;

	DRM_DEBUG_ATOMIC("plane-%d atomic_duplicate_state.\n", plane->index);
	sp_state = kmemdup(to_ecv5546_plane_state(plane->state),
...
/**
 * ecv5546_plane_atomic_destroy_state - ecv5546 state destroy hook
 * @plane: drm plane
 * @state: plane state object to release
 */
static void ecv5546_plane_atomic_destroy_state(struct drm_plane *plane,
					   struct drm_plane_state *state)
{
	struct ecv5546_plane_state *sp_state = to_ecv5546_plane_state(state);
...
static void ecv5546_plane_reset(struct drm_plane *plane)
{
	struct ecv5546_plane_state *sp_state;
	struct ecv5546_plane *sp_plane = to_ecv5546_plane(plane);
	...
	if (plane->state)
		ecv5546_plane_atomic_destroy_state(plane, plane->state);
	...
	if (sp_plane->capabilities & eCV5546_DRM_PLANE_CAP_ROTATION)
		sp_state->base.rotation = DRM_MODE_ROTATE_0;
	if (sp_plane->capabilities & eCV5546_DRM_PLANE_CAP_BG_BLEND)
		sp_state->bg_alpha = 0;
	if (sp_plane->capabilities & eCV5546_DRM_PLANE_CAP_BG_FORMAT)
		sp_state->bg_format = 1;
	if (sp_plane->capabilities & eCV5546_DRM_PLANE_CAP_BG_COLOR)
		sp_state->bg_color = eCV5546_DMIX_PTG_BLACK;
	if (sp_plane->capabilities & eCV5546_DRM_PLANE_CAP_BRIGHTNESS)
		sp_state->brightness = 0;
	if (sp_plane->capabilities & eCV5546_DRM_PLANE_CAP_CONTRAST)
		sp_state->contrast = 0;

	if (sp_plane->capabilities & eCV5546_DRM_PLANE_CAP_ALPHA_BLEND) {
...
static const struct drm_plane_funcs ecv5546_plane_funcs = {
	.update_plane	= drm_atomic_helper_update_plane,
	.disable_plane	= drm_atomic_helper_disable_plane,
	.destroy = drm_plane_cleanup,
	.reset = ecv5546_plane_reset,
	.atomic_duplicate_state = ecv5546_plane_atomic_duplicate_state,
	.atomic_destroy_state = ecv5546_plane_atomic_destroy_state,
	.atomic_set_property = ecv5546_plane_atomic_set_property,
	.atomic_get_property = ecv5546_plane_atomic_get_property,
};

static void ecv5546_scaling_coordinate_adjust(int32_t src_w, int32_t src_h,
...
static void ecv5546_kms_plane_vpp_atomic_update(struct drm_plane *plane,
					       struct drm_atomic_state *state)
{
	...
	struct ecv5546_plane_state *sp_state = NULL;
	struct ecv5546_plane_state *old_sp_state = NULL;
	...
	if (!new_state || !new_state->fb || !new_state->crtc) {
		/* do nothing */
		DRM_DEBUG_ATOMIC("plane-%d eCV5546_DMIX_TRANSPARENT.\n", plane->index);
		ecv5546_drm_plane_set(plane, sp_plane->dmix_fg_sel, eCV5546_DMIX_TRANSPARENT);
		return;
	}
	DRM_DEBUG_ATOMIC("plane-%d zpos:%d\n", plane->index, sp_plane->zpos);
	sp_state = to_ecv5546_plane_state(new_state);
	if (old_state)
		old_sp_state = to_ecv5546_plane_state(old_state);
	...
	if (!old_state || new_state->fb != old_state->fb
		...
		ecv5546_vpp_plane_imgread_set(plane, (u32)paddr,
				 new_state->src_x >> 16, new_state->src_y >> 16,
				 new_state->src_w >> 16, new_state->src_h >> 16,
				 new_state->fb->width, new_state->fb->height,
				 ecv5546_get_plane_format(new_state->fb->format->format, eCV5546_PLANE_TYPE_VPP));
	}
	...
	if (!old_state || new_state->src_x != old_state->src_x || new_state->src_y != old_state->src_y
		...
		if ((sp_plane->capabilities & eCV5546_DRM_PLANE_CAP_ALPHA_BLEND)
			...
			ecv5546_scaling_coordinate_adjust(new_state->src_w >> 16, new_state->src_h >> 16,
		...
		ecv5546_vpp_plane_vscl_set(plane, new_state->src_x >> 16, new_state->src_y >> 16,
				    new_state->src_w >> 16, new_state->src_h >> 16,
				    new_state->crtc_x, new_state->crtc_y,
				    new_state->crtc_w, new_state->crtc_h,
		...
		ecv5546_vpp_plane_vpost_opif_set(plane, new_state->crtc_x, new_state->crtc_y,
					  new_state->crtc_w, new_state->crtc_h,
					  new_state->crtc_w + new_state->crtc_x,
					  new_state->crtc_h + new_state->crtc_y);
	}
	...
	if ((sp_plane->capabilities & eCV5546_DRM_PLANE_CAP_ALPHA_BLEND)
		...
		/* NOTES: vpp layer(eCV5546_DMIX_L3) used for media plane fixed. */
		ecv5546_drm_plane_alpha_config(plane, sp_plane->dmix_layer, 1, 0, new_state->alpha >> 10);
	...
	if ((sp_plane->capabilities & eCV5546_DRM_PLANE_CAP_WIN_BLEND)
		...
		ecv5546_vpp_plane_vpost_opif_alpha_set(plane, sp_state->win_alpha, 0);
		sp_plane->updated_win_alpha = sp_state->win_alpha;
	}
	...
	if ((sp_plane->capabilities & eCV5546_DRM_PLANE_CAP_BRIGHTNESS)
		...
		ecv5546_plane_brightness_setting(plane, sp_state->brightness);
	}

	if ((sp_plane->capabilities & eCV5546_DRM_PLANE_CAP_CONTRAST)
		&& (!old_sp_state || (!sp_state->brightness &&
		   (sp_state->contrast != old_sp_state->contrast)))) {
		ecv5546_plane_contrast_setting(plane, sp_state->contrast);
	}

	ecv5546_drm_plane_set(plane, sp_plane->dmix_fg_sel, eCV5546_DMIX_BLENDING);
}

static void ecv5546_kms_plane_osd_atomic_update(struct drm_plane *plane,
					       struct drm_atomic_state *state)
{
	struct ecv5546_plane *sp_plane = to_ecv5546_plane(plane);
	//struct drm_device *drm = sp_plane->base.dev;
	...
	struct ecv5546_plane_state *sp_state = NULL;
	struct ecv5546_plane_state *old_sp_state = NULL;
	struct ecv5546_osd_region *info = NULL;
	...
	if (updated) {
		info->color_mode = ecv5546_get_plane_format(new_state->fb->format->format,
												   eCV5546_PLANE_TYPE_OSD);
		ecv5546_osd_plane_set_by_region(plane, info);
...
static int ecv5546_kms_plane_atomic_check(struct drm_plane *plane,
					     struct drm_atomic_state *state)
{
	struct ecv5546_plane *sp_plane = to_ecv5546_plane(plane);
	struct drm_plane_state *new_state = drm_atomic_get_new_plane_state(state, plane);
	struct ecv5546_plane_state *sp_state = to_ecv5546_plane_state(new_state);
...
static const struct drm_plane_helper_funcs ecv5546_kms_vpp_helper_funcs = {
	.atomic_update = ecv5546_kms_plane_vpp_atomic_update,
	.atomic_check  = ecv5546_kms_plane_atomic_check,
};

static const struct drm_plane_helper_funcs ecv5546_kms_osd_helper_funcs = {
	.atomic_update = ecv5546_kms_plane_osd_atomic_update,
	.atomic_check  = ecv5546_kms_plane_atomic_check,
};

static void ecv5546_plane_create_propertys(struct ecv5546_plane *sp_plane)
{
	...
	if (sp_plane->capabilities & eCV5546_DRM_PLANE_CAP_BG_COLOR) {
		...
		drm_object_attach_property(&sp_plane->base.base,
			 sp_plane->background_color_property, eCV5546_DMIX_PTG_BLACK);
		if (sp_plane->state)
			sp_plane->state->bg_color = eCV5546_DMIX_PTG_BLACK;
	}
	if (sp_plane->capabilities & eCV5546_DRM_PLANE_CAP_BRIGHTNESS) {
		/* region: 0~100 */
		if (sp_plane->is_media_plane)
	...
	if (sp_plane->capabilities & eCV5546_DRM_PLANE_CAP_ZPOS)
		/* eCV5546_MAX_PLANE - 2, cursor not support adjustable zpos */
		drm_plane_create_zpos_property(&sp_plane->base, sp_plane->zpos, 0, eCV5546_MAX_PLANE - 2);
	else
	...
	if (sp_plane->capabilities & eCV5546_DRM_PLANE_CAP_PIX_BLEND)
...
static void ecv5546_plane_destroy_propertys(struct ecv5546_plane *sp_plane)
{
	...
	enum drm_plane_type type, enum ecv5546_plane_type sptype, int init_zpos)
{
	...
	struct ecv5546_plane *sp_plane = NULL;
	...
	if (sptype == eCV5546_PLANE_TYPE_VPP0) {
		sp_plane->pixel_formats = (u32 *)&ecv5546_kms_vpp_formats;
		sp_plane->num_pixel_formats = ARRAY_SIZE(ecv5546_kms_vpp_formats);
		sp_plane->funcs = &ecv5546_kms_vpp_helper_funcs;
		sp_plane->is_media_plane = true;
		sp_plane->capabilities = eCV5546_DRM_PLANE_CAP_SCALE |
			eCV5546_DRM_PLANE_CAP_PIX_BLEND | eCV5546_DRM_PLANE_CAP_ALPHA_BLEND |
			eCV5546_DRM_PLANE_CAP_WIN_BLEND | eCV5546_DRM_PLANE_CAP_BRIGHTNESS |
			eCV5546_DRM_PLANE_CAP_CONTRAST;
		...
		sp_plane->dmix_fg_sel = eCV5546_DMIX_VPP0_SEL;
		sp_plane->osd_layer_sel = -1;
	} else {
		sp_plane->pixel_formats = (u32 *)&ecv5546_kms_osd_formats;
		sp_plane->num_pixel_formats = ARRAY_SIZE(ecv5546_kms_osd_formats);
		sp_plane->funcs = &ecv5546_kms_osd_helper_funcs;
		sp_plane->is_media_plane = false;

		if (type != DRM_PLANE_TYPE_CURSOR)
			sp_plane->capabilities = eCV5546_DRM_PLANE_CAP_PIX_BLEND |
				eCV5546_DRM_PLANE_CAP_ALPHA_BLEND | eCV5546_DRM_PLANE_CAP_WIN_BLEND |
				eCV5546_DRM_PLANE_CAP_COLOR_KEYING | eCV5546_DRM_PLANE_CAP_BRIGHTNESS |
				eCV5546_DRM_PLANE_CAP_CONTRAST;
		/* HW Restricted. */
		...
		switch (sptype) {
		case eCV5546_PLANE_TYPE_OSD0:
			sp_plane->osd_layer_sel = eCV5546_LAYER_OSD0;
			sp_plane->dmix_fg_sel   = eCV5546_DMIX_OSD0_SEL;
			break;
		case eCV5546_PLANE_TYPE_OSD1:
			sp_plane->osd_layer_sel = eCV5546_LAYER_OSD1;
			sp_plane->dmix_fg_sel   = eCV5546_DMIX_OSD1_SEL;
			break;
		case eCV5546_PLANE_TYPE_OSD2:
			sp_plane->osd_layer_sel = eCV5546_LAYER_OSD2;
			sp_plane->dmix_fg_sel   = eCV5546_DMIX_OSD2_SEL;
			break;
		case eCV5546_PLANE_TYPE_OSD3:
			sp_plane->osd_layer_sel = eCV5546_LAYER_OSD3;
			sp_plane->dmix_fg_sel   = eCV5546_DMIX_OSD3_SEL;
			break;
		default:
	...
	if (type != DRM_PLANE_TYPE_CURSOR)
		sp_plane->capabilities |= eCV5546_DRM_PLANE_CAP_ZPOS;
	...
	ret = drm_universal_plane_init(drm, plane, GENMASK(drm->mode_config.num_crtc, 0),
				...
				ecv5546_kms_modifiers, type, NULL);
	if (ret)
		return ERR_PTR(ret);
	...
	ecv5546_plane_create_propertys(sp_plane);

	switch (init_zpos) {
	case 4:
		sp_plane->dmix_layer = eCV5546_DMIX_L6;
		sp_plane->dtg_adjust = eCV5546_TGEN_DTG_ADJ_DMIX_L6;
		break;
	case 3:
		sp_plane->dmix_layer = eCV5546_DMIX_L5;
		sp_plane->dtg_adjust = eCV5546_TGEN_DTG_ADJ_DMIX_L5;
		break;
	case 2:
		sp_plane->dmix_layer = eCV5546_DMIX_L4;
		sp_plane->dtg_adjust = eCV5546_TGEN_DTG_ADJ_DMIX_L4;
		break;
	case 1:
		sp_plane->dmix_layer = eCV5546_DMIX_L3;
		sp_plane->dtg_adjust = eCV5546_TGEN_DTG_ADJ_DMIX_L3;
		break;
	case 0:
		sp_plane->dmix_layer = eCV5546_DMIX_L1;
		sp_plane->dtg_adjust = eCV5546_TGEN_DTG_ADJ_DMIX_L1;
		break;
	default:
...
int ecv5546_plane_release(struct drm_device *drm, struct drm_plane *plane)
{
	...
	/* DO ANY??? */
	ecv5546_plane_destroy_propertys(sp_plane);
...
int ecv5546_plane_dev_resume(struct device *dev, struct drm_plane *plane)
{
	...
	if (!sp_plane->is_media_plane) {
		ecv5546_osd_plane_init(plane);
		ecv5546_kms_plane_osd_atomic_update(plane, NULL);
	} else {
		ecv5546_kms_plane_vpp_atomic_update(plane, NULL);
	}
