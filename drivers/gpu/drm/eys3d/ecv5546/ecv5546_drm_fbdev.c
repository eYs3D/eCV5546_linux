...
static int ecv5546_drm_fbdev_fb_mmap(struct fb_info *info,
 ...
 * callback.
 */
int ecv5546_drm_fb_helper_set_par(struct fb_info *info)
{
	struct drm_fb_helper *fb_helper = info->par;
 ...
 * @info: fbdev registered by the helper
 */
int ecv5546_drm_fbdev_pan_display(struct fb_var_screeninfo *var,
				 struct fb_info *info)
{
	struct drm_fb_helper *fb_helper = info->par;
...
static const struct fb_ops ecv5546_drm_fbdev_fb_ops = {
	.owner		= THIS_MODULE,
	DRM_FB_HELPER_DEFAULT_OPS,
	.fb_mmap        = ecv5546_drm_fbdev_fb_mmap,
	.fb_fillrect    = drm_fb_helper_cfb_fillrect,
	.fb_copyarea    = drm_fb_helper_cfb_copyarea,
	.fb_imageblit   = drm_fb_helper_cfb_imageblit,
	.fb_set_par     = ecv5546_drm_fb_helper_set_par,
	.fb_pan_display = ecv5546_drm_fbdev_pan_display,
};

static int ecv5546_drm_fbdev_create(struct drm_fb_helper *fb_helper,
				   struct drm_fb_helper_surface_size *sizes)
{
	...
	fbi->fbops = &ecv5546_drm_fbdev_fb_ops;
...
static const struct drm_fb_helper_funcs ecv5546_drm_fb_helper_funcs = {
	.fb_probe =	ecv5546_drm_fbdev_create,
};

static void ecv5546_drm_fbdev_cleanup(struct drm_fb_helper *fb_helper)
{
...
static void ecv5546_drm_fbdev_release(struct drm_fb_helper *fb_helper)
{
	ecv5546_drm_fbdev_cleanup(fb_helper);
	drm_client_release(&fb_helper->client);
	kfree(fb_helper);
}

static void ecv5546_drm_fbdev_client_unregister(struct drm_client_dev *client)
{
	...
	if (fb_helper->fbdev)
		...
		ecv5546_drm_fbdev_release(fb_helper);
}

static int ecv5546_drm_fbdev_client_restore(struct drm_client_dev *client)
{
...
static int ecv5546_drm_fbdev_client_hotplug(struct drm_client_dev *client)
{
	struct drm_fb_helper *fb_helper = drm_fb_helper_from_client(client);
	struct drm_device *dev = client->dev;
	int ret;
	...
	drm_fb_helper_prepare(dev, fb_helper, &ecv5546_drm_fb_helper_funcs);
...
static const struct drm_client_funcs ecv5546_drm_fbdev_client_funcs = {
	.owner		= THIS_MODULE,
	.unregister	= ecv5546_drm_fbdev_client_unregister,
	.restore	= ecv5546_drm_fbdev_client_restore,
	.hotplug	= ecv5546_drm_fbdev_client_hotplug,
};
			  ...
			  unsigned int preferred_bpp)
{
	...
	ret = drm_client_init(dev, &helper->client, "fbdev", &ecv5546_drm_fbdev_client_funcs);
	if (ret) {
		kfree(helper);
	...
	ret = ecv5546_drm_fbdev_client_hotplug(&helper->client);
	if (ret)
