...
/*
 * eYs3D eCV5546 DRM dsi panel simple driver.
 *
...
/* always keep 0 */
#define eCV5546_DRM_TODO    0

struct ecv5546_dsi_panel_desc {
...
struct ecv5546_panel_simple_dsi {
	struct drm_panel base;
	struct mipi_dsi_device *dsi;
	const struct ecv5546_dsi_panel_desc *desc;
...
static const struct ecv5546_dsi_panel_desc lx_hxm0686tft_001_desc = {
...
static const struct ecv5546_dsi_panel_desc xinli_tcxd024iblon_n2_desc = {
...
static const struct drm_display_mode wks_wks70wv055_wct_mode[] = {
	{
		/* Modeline comes from the eCV5546 firmware, with HFP=0
...
static const struct ecv5546_dsi_panel_desc wks_wks70wv055_wct_desc = {
...
static inline struct ecv5546_panel_simple_dsi *to_simple_panel(struct drm_panel *panel)
{
	return container_of(panel, struct ecv5546_panel_simple_dsi, base);
}

static unsigned int ecv5546_panel_simple_get_display_modes(struct ecv5546_panel_simple_dsi *sp_panel,
...
static int ecv5546_panel_init_dcs_cmd(struct ecv5546_panel_simple_dsi *sp_panel)
{
...
static int ecv5546_panel_simple_disable(struct drm_panel *panel)
{
	struct ecv5546_panel_simple_dsi *sp_panel = to_simple_panel(panel);
...
static int ecv5546_panel_simple_unprepare(struct drm_panel *panel)
{
	struct ecv5546_panel_simple_dsi *sp_panel = to_simple_panel(panel);
...
static int ecv5546_panel_simple_prepare(struct drm_panel *panel)
{
	struct ecv5546_panel_simple_dsi *sp_panel = to_simple_panel(panel);
	int ret;
	...
	ret = ecv5546_panel_init_dcs_cmd(sp_panel);
	if (ret < 0) {
...
static int ecv5546_panel_simple_enable(struct drm_panel *panel)
{
	struct ecv5546_panel_simple_dsi *sp_panel = to_simple_panel(panel);
...
static int ecv5546_panel_simple_get_modes(struct drm_panel *panel,
					 struct drm_connector *connector)
{
	struct ecv5546_panel_simple_dsi *sp_panel = to_simple_panel(panel);
	int num = 0;
	...
	num += ecv5546_panel_simple_get_display_modes(sp_panel, connector);
...
static const struct drm_panel_funcs ecv5546_panel_funcs = {
	.disable   = ecv5546_panel_simple_disable,
	.unprepare = ecv5546_panel_simple_unprepare,
	.prepare   = ecv5546_panel_simple_prepare,
	.enable    = ecv5546_panel_simple_enable,
	.get_modes = ecv5546_panel_simple_get_modes,
};
 ...
 */
#if eCV5546_DRM_TODO
static int ecv5546_panel_simple_backlight_update_status(struct backlight_device *bd)
{
	return 0;
}

static const struct backlight_ops ecv5546_panel_simple_backlight_ops = {
	.update_status = ecv5546_panel_simple_backlight_update_status,
};
#endif

static int ecv5546_panel_simple_resume(struct device *dev)
{
	struct ecv5546_panel_simple_dsi *sp_panel = dev_get_drvdata(dev);

	if (sp_panel->flags & FLAG_PANEL_PREPARED)
		ecv5546_panel_simple_prepare(&sp_panel->base);

	if (sp_panel->flags & FLAG_PANEL_ENABLED)
		ecv5546_panel_simple_enable(&sp_panel->base);
...
static int ecv5546_panel_simple_suspend(struct device *dev)
{
	struct ecv5546_panel_simple_dsi *sp_panel = dev_get_drvdata(dev);

	if (sp_panel->flags & FLAG_PANEL_ENABLED) {
		ecv5546_panel_simple_disable(&sp_panel->base);
		/* reset for resume. */
		sp_panel->flags |= FLAG_PANEL_ENABLED;
	}
	if (sp_panel->flags & FLAG_PANEL_PREPARED) {
		ecv5546_panel_simple_unprepare(&sp_panel->base);
		/* reset for resume. */
		sp_panel->flags |= FLAG_PANEL_PREPARED;
	}
...
static const struct dev_pm_ops ecv5546_panel_simple_pm_ops = {
	SET_SYSTEM_SLEEP_PM_OPS(ecv5546_panel_simple_suspend, ecv5546_panel_simple_resume)
};

static int ecv5546_panel_simple_add(struct ecv5546_panel_simple_dsi *sp_panel)
{
	...
	drm_panel_init(&sp_panel->base, dev, &ecv5546_panel_funcs,
...
static int ecv5546_panel_simple_dsi_probe(struct mipi_dsi_device *dsi)
{
	struct ecv5546_panel_simple_dsi *sp_panel;
	int ret;
	const struct ecv5546_dsi_panel_desc *desc;
	...
	ret = ecv5546_panel_simple_add(sp_panel);
	if (ret < 0)
		return ret;
...
static void ecv5546_panel_simple_dsi_shutdown(struct mipi_dsi_device *dsi)
{
	struct ecv5546_panel_simple_dsi *sp_panel = mipi_dsi_get_drvdata(dsi);

	drm_panel_disable(&sp_panel->base);
	drm_panel_unprepare(&sp_panel->base);
}

static void ecv5546_panel_simple_dsi_remove(struct mipi_dsi_device *dsi)
{
	struct ecv5546_panel_simple_dsi *sp_panel = mipi_dsi_get_drvdata(dsi);
	int ret;

	ecv5546_panel_simple_dsi_shutdown(dsi);
...
static const struct of_device_id panel_simple_dsi_of_match[] = {
	{
		.compatible = "eys3d,lx-hxm0686tft-001", .data = &lx_hxm0686tft_001_desc
	}, {
		.compatible = "eys3d,xinli-tcxd024iblon-2", .data = &xinli_tcxd024iblon_n2_desc
	}, {
		.compatible = "eys3d,wks-wks70wv055-wct", .data = &wks_wks70wv055_wct_desc
	}, {
		/* sentinel */
	}
};
...
static struct mipi_dsi_driver panel_simple_dsi_driver = {
	.driver = {
		...
		.pm = &ecv5546_panel_simple_pm_ops,
	},
	.probe = ecv5546_panel_simple_dsi_probe,
	.remove = ecv5546_panel_simple_dsi_remove,
	.shutdown = ecv5546_panel_simple_dsi_shutdown,
};

module_mipi_dsi_driver(panel_simple_dsi_driver);

MODULE_AUTHOR("dx.jiang <eys3d@eys3d.com>");
MODULE_DESCRIPTION("eYs3D eCV5546 DRM Driver for DSI Simple Panels");
