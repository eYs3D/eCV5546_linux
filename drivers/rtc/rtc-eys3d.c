...
/*
 * The RTC driver for eYs3D	SP7021
 *
 * Copyright (C) 2019 eys3d Technology Inc., All rights reseerved.
 */
...
{
	struct eys3d_rtc *sp_rtc = dev_get_drvdata(dev);
...
static irqreturn_t sp_rtc_irq_handler(int irq, void *dev_id)
{
	...
	struct eys3d_rtc *sp_rtc = dev_get_drvdata(&plat_dev->dev);
...
MODULE_AUTHOR("Vincent Shih <vincent.eys3d@gmail.com>");
MODULE_DESCRIPTION("eYs3D RTC driver");
