...
#define DEVICE_NAME			"eys3d,ecv5546-axi"
...
{
	DBG_INFO("eYs3D AXI module open\n");
	return 0;
}

static int sp_axi_release(struct inode *inode, struct file *pfile)
{
	DBG_INFO("eYs3D AXI module release\n");
	return 0;
}
...
{
	//struct eys3d_axi *axi = platform_get_drvdata(pdev);
	FUNC_DEBUG();
...
MODULE_DESCRIPTION("eYs3D AXI Driver");
MODULE_LICENSE("GPL");
