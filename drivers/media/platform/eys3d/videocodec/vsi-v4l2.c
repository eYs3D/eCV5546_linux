...
{
	unsigned long ctxid = pmsg->inst_id;
	int inbufidx, outbufidx, bytesused[VB2_MAX_PLANES] = {0};
	struct vsi_v4l2_ctx *ctx;
...
static int v4l2_probe(struct platform_device *pdev)
{
	...
	if (ret) {
		v4l2_klog(LOGLVL_ERROR,  "Could not get reserved memory!\n");
		return ret;
	}

	ret = dma_set_coherent_mask(vpu->dev, dma_get_required_mask(vpu->dev));
	if (ret) {
		v4l2_klog(LOGLVL_ERROR, "consistent DMA enable failed\n");
	}

	// dev_info (vpu->dev, "5. size = %lx, GFP_KERNEL = %x\n", size, GFP_KERNEL);
	// lvaddr = dma_alloc_coherent(vpu->dev, size, &dma_handle, GFP_KERNEL);
	// dev_info(vpu->dev, "Allocated coherent memory, vaddr: 0x%0llX, paddr: 0x%0llX\n", (u64)lvaddr, dma_handle);
...
static const struct of_device_id v4l2_of_match[] = {
	{ .compatible = "eys3d,ecv5546-hantro-vsiv4l2", },
	{/* sentinel */}
};
