...
static const char * const ecv5546_tgen_usr[] = {"Internal", "User Mode"};
static const char * const ecv5546_tgen_fmt[] = {
	"480P", "576P", "720P", "1080P", "rsv", "rsv",
	"64x64(360x100)", "64x64(144x100)"};
static const char * const ecv5546_tgen_fps[] = {"60Hz", "50Hz", "24Hz", "rsv"};

static const char * const ecv5546_pllh_pstdiv[] = {
	"DIV2.5", "DIV3", "DIV3.5", "DIV4", "DIV5", "DIV5.5", "DIV6", "DIV7",
	"DIV7.5", "DIV8", "DIV9", "DIV10", "DIV10.5", "DIV11", "DIV12", "DIV12.5"
};
static const char * const ecv5546_pllh_mipitx_sel[] = {
...
void ecv5546_tgen_decrypt_info(void)
{
	...
	pr_info("  USER_MODE[%s] %s %s\n", 
		user_mode?"ON ":"OFF",
		user_mode?"---":ecv5546_tgen_fmt[FIELD_GET(GENMASK(10,8), value)],
		user_mode?"---":ecv5546_tgen_fps[FIELD_GET(GENMASK(5,4), value)]);
...
EXPORT_SYMBOL(ecv5546_tgen_decrypt_info);
...
EXPORT_SYMBOL(ecv5546_tgen_resolution_chk);
...
void ecv5546_tgen_set_user_int1(u32 count)
{
	...
	if (count >= eCV5546_TGEN_USER_INT1_CONFIG)
		count = eCV5546_TGEN_USER_INT1_CONFIG;
...
void ecv5546_tgen_set_user_int2(u32 count)
{
	...
	if (count >= eCV5546_TGEN_USER_INT2_CONFIG)
		count = eCV5546_TGEN_USER_INT2_CONFIG;
...
EXPORT_SYMBOL(ecv5546_tgen_get_current_line_count);
...
static const u32 sp_tgen_para_dsi[11][10] = {
	...
	{ 480, 1280, 1, 0, 0,  678,  480, 1312, 1297, 16}, /* 480x1280 60Hz */ //update for sync drm setting
	...
	{ 800,  480, 1, 0, 0,  917,  800,  510,  504, 23}, /* 800x480 60Hz */ //update for sync drm setting
...
void ecv5546_tgen_timing_set_dsi(void)
{
	...
	value = readl(disp_dev->base + TGEN_DTG_CONFIG);
	value &= ~(eCV5546_TGEN_FORMAT | eCV5546_TGEN_FPS | eCV5546_TGEN_USER_MODE);
	...
	value |= eCV5546_TGEN_FORMAT_SET(sp_tgen_para_dsi[time_cnt][4]) |
			eCV5546_TGEN_FPS_SET(sp_tgen_para_dsi[time_cnt][3]) |
...
void ecv5546_tgen_timing_set_csi(void)
{
	...
	value |= eCV5546_TGEN_FORMAT_SET(sp_tgen_para_csi[time_cnt][4]) |
			eCV5546_TGEN_FPS_SET(sp_tgen_para_csi[time_cnt][3]) |
...
void ecv5546_tgen_timing_get(void)
{
	...
	pr_info("  tgen cur tim %s (%s %s)\n",
		ecv5546_tgen_usr[user_mode],
		user_mode?"USER_DEF":ecv5546_tgen_fmt[dtg_fmt],
		ecv5546_tgen_fps[dtg_fps]);
...
void ecv5546_tgen_input_adjust(int tgen_input_adj, u32 adj_value)
{
	...
	switch (tgen_input_adj) {
		case eCV5546_TGEN_DTG_ADJ_VPP0:
			value1 &= ~(eCV5546_TGEN_DTG_ADJ_MASKA);
			value1 |= eCV5546_TGEN_DTG_ADJ1_VPP0(adj_value);
			writel(value1, disp_dev->base + TGEN_DTG_ADJUST1);
			break;
		case eCV5546_TGEN_DTG_ADJ_VPP1:
			/* not supported */
			break;
		case eCV5546_TGEN_DTG_ADJ_VPP2:
			/* not supported */
			break;
		case eCV5546_TGEN_DTG_ADJ_OSD3:
			value2 &= ~(eCV5546_TGEN_DTG_ADJ_MASKA);
			value2 |= eCV5546_TGEN_DTG_ADJ2_OSD3(adj_value);
			writel(value2, disp_dev->base + TGEN_DTG_ADJUST2);
			break;
		case eCV5546_TGEN_DTG_ADJ_OSD2:
			value2 &= ~(eCV5546_TGEN_DTG_ADJ_MASKB);
			value2 |= eCV5546_TGEN_DTG_ADJ2_OSD2(adj_value);
			writel(value2, disp_dev->base + TGEN_DTG_ADJUST2);
			break;
		case eCV5546_TGEN_DTG_ADJ_OSD1:
			value3 &= ~(eCV5546_TGEN_DTG_ADJ_MASKA);
			value3 |= eCV5546_TGEN_DTG_ADJ3_OSD1(adj_value);
			writel(value3, disp_dev->base + TGEN_DTG_ADJUST3);
			break;
		case eCV5546_TGEN_DTG_ADJ_OSD0:
			value3 &= ~(eCV5546_TGEN_DTG_ADJ_MASKB);
			value3 |= eCV5546_TGEN_DTG_ADJ3_OSD0(adj_value);
			writel(value3, disp_dev->base + TGEN_DTG_ADJUST3);
			break;
		case eCV5546_TGEN_DTG_ADJ_PTG:
			value4 &= ~(eCV5546_TGEN_DTG_ADJ_MASKA);
			value4 |= eCV5546_TGEN_DTG_ADJ4_PTG(adj_value);
			writel(value4, disp_dev->base + TGEN_DTG_ADJUST4);
			break;
		case eCV5546_TGEN_DTG_ADJ_ALL:
			break;
		default:
			break;
	}
