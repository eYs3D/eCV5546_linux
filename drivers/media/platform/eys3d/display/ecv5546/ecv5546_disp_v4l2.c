...
/*
 * eYs3D eCV5546 SoC Display driver for V4L2
 *
...
extern int ecv5546_vpp_restore(struct sp_disp_device *disp_dev);
...
static int sp_start_streaming(struct vb2_queue *vq, unsigned int count)
{
	...
	if ((dev_id >= SP_DISP_DEVICE_0) && (dev_id <= SP_DISP_DEVICE_4))
		ecv5546_dmix_layer_cfg_set(dev_id);
	...
	if ((dev_id >= SP_DISP_DEVICE_0) && (dev_id <= SP_DISP_DEVICE_3)) {
		...
		ecv5546_osd_layer_set(&info, dev_id);
...
static void sp_stop_streaming(struct vb2_queue *vq)
{
	...
	if (dev_id <= SP_DISP_DEVICE_3)
		ecv5546_osd_header_clear(dev_id);

	ecv5546_vpp_restore(disp_dev);
...
int ecv5546_v4l2_initialize(struct device *dev,
...
EXPORT_SYMBOL(ecv5546_v4l2_initialize);

int ecv5546_v4l2_init_multi_layer(int i, struct platform_device *pdev,
		struct sp_disp_device *disp_dev)
{
...
EXPORT_SYMBOL(ecv5546_v4l2_init_multi_layer);

int ecv5546_v4l2_reg_multi_layer(int i, struct platform_device *pdev,
		struct sp_disp_device *disp_dev)
{
	struct vb2_queue *q;
	int ret;
...
EXPORT_SYMBOL(ecv5546_v4l2_reg_multi_layer);
