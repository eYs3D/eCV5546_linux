...
/*
 * eYs3D eCV5546 SoC Display driver
 *
...
#if defined(CONFIG_VIDEO_eCV5546_DISP_PI_PANEL)
#define REG_ID			0x80
...
extern int ecv5546_disp_state;
struct sp_disp_device *gdisp_dev;

#if defined(CONFIG_VIDEO_eCV5546_DISP_PI_PANEL)
extern struct rpi_touchscreen *g_ts;

extern int rpi_touchscreen_i2c_read(struct rpi_touchscreen *ts, u8 reg);
extern void rpi_touchscreen_i2c_write(struct rpi_touchscreen *ts,
				      u8 reg, u8 val);
#endif
#if defined(CONFIG_VIDEO_eCV5546_DISP_LT8912B)
...
extern int ecv5546_vpp_resolution_init(struct sp_disp_device *disp_dev);
extern int ecv5546_osd_resolution_init(struct sp_disp_device *disp_dev);
#ifdef SP_DISP_V4L2_SUPPORT
extern int ecv5546_v4l2_initialize(struct device *dev,
		struct sp_disp_device *disp_dev);
extern int ecv5546_v4l2_init_multi_layer(int i, struct platform_device *pdev,
		struct sp_disp_device *disp_dev);
extern int ecv5546_v4l2_reg_multi_layer(int i, struct platform_device *pdev,
		struct sp_disp_device *disp_dev);
#endif
...
static irqreturn_t ecv5546_display_irq_fs(int irq, void *param)
{
		...
		if (layer->streaming) {
			...
			if (!list_empty(&layer->dma_queue)) {
				...
				if ((i >= SP_DISP_DEVICE_0) && (i <= SP_DISP_DEVICE_3)) {
					 ...
					 */
					ecv5546_vpp_imgread_set((u32)addr,
					...
					ecv5546_vpp_vscl_set(disp_dev->vpp_res[0].x_ofs, disp_dev->vpp_res[0].y_ofs,
...
static irqreturn_t ecv5546_display_irq_fe(int irq, void *param)
{
	if (ecv5546_disp_state == 0) {
		ecv5546_osd_region_update();
	}

	return IRQ_HANDLED;
}

static irqreturn_t ecv5546_display_irq_int1(int irq, void *param)
{
	return IRQ_HANDLED;
}

static irqreturn_t ecv5546_display_irq_int2(int irq, void *param)
{
	return IRQ_HANDLED;
}
...
static int ecv5546_resolution_get(struct sp_disp_device *disp_dev)
{
	 ...
	 */
	ret = of_property_read_string_index(disp_dev->pdev->of_node, "eys3d,dev_name", 0, &connect_dev_name);
	...
	if (ret) {
		...
		disp_dev->vpp_res[0].color_mode = eCV5546_VPP_IMGREAD_DATA_FMT_YUY2;
	} else {
	 ...
	 */
	ret = of_property_read_u32_array(disp_dev->pdev->of_node,
		"eys3d,disp_output", out_res, 3);
	if (ret) {
		disp_dev->out_res.width = 0;//720;
		disp_dev->out_res.height = 0;//480;
		disp_dev->out_res.mipitx_mode = eCV5546_MIPITX_DSI;
	} else {
	...
	ret = of_property_read_u32(disp_dev->pdev->of_node,
		"eys3d,disp_mipitx_lane", &mipitx_lane);
	if (ret)
	 ...
	 */
	ret = of_property_read_u32(disp_dev->pdev->of_node,
		"eys3d,disp_mipitx_clk_edge", &mipitx_clk_edge);
	if (ret)
	 ...
	 */
	ret = of_property_read_u32(disp_dev->pdev->of_node,
		"eys3d,disp_mipitx_sync_timing", &mipitx_sync_timing);
	if (ret)
	 ...
	 */
	ret = of_property_read_u32(disp_dev->pdev->of_node,
		"eys3d,disp_mipitx_format", &mipitx_format);
	if (ret)
...
void __iomem *ecv5546_display_ioremap_regs(int index)
{
	struct sp_disp_device *disp_dev = gdisp_dev;

	if (!disp_dev) {
		pr_warn("Failed to map registers: ecv5546_display not register!\n");
		return NULL;
	}
...
static int ecv5546_display_probe(struct platform_device *pdev)
{
	...
	ret = devm_request_irq(&pdev->dev, irq, ecv5546_display_irq_fs,
	...
	ret = devm_request_irq(&pdev->dev, irq, ecv5546_display_irq_fe,
	...
	ret = devm_request_irq(&pdev->dev, irq, ecv5546_display_irq_int1,
	...
	ret = devm_request_irq(&pdev->dev, irq, ecv5546_display_irq_int2,
...
//#endif
	 ...
	 */
	for (i = 0; i < 16; i++) {
		disp_dev->disp_clk[i] = devm_clk_get(&pdev->dev, ecv5546_disp_clkc[i]);
		...
		disp_dev->disp_rstc[i] = devm_reset_control_get_exclusive(&pdev->dev, ecv5546_disp_rtsc[i]);
	 ...
	 */
	ecv5546_resolution_get(disp_dev);
...
//#ifndef CONFIG_DRM_eCV5546
	...
	//ecv5546_dmix_plane_alpha_config(eCV5546_DMIX_L6, 1, 0, 0x1f);
	//ecv5546_dmix_plane_alpha_config(eCV5546_DMIX_L5, 1, 0, 0x1f);
	//ecv5546_dmix_plane_alpha_config(eCV5546_DMIX_L4, 1, 0, 0x1f);
	//ecv5546_dmix_plane_alpha_config(eCV5546_DMIX_L3, 1, 0, 0x1f);
	//ecv5546_dmix_plane_alpha_config(eCV5546_DMIX_L1, 1, 0, 0x1f);

	//ecv5546_dmix_all_layer_info();
	ecv5546_dmix_layer_cfg_store();

	/*
	 * v4l2 init for osd/vpp layers
	 */
	#ifdef SP_DISP_V4L2_SUPPORT
	ret = ecv5546_v4l2_initialize(&pdev->dev, disp_dev);
	if (ret)
		return ret;

	for (i = 0; i < SP_DISP_MAX_DEVICES; i++) {
		if (ecv5546_v4l2_init_multi_layer(i, pdev, disp_dev)) {
			ret = -ENODEV;
			return ret;
		}
		if (ecv5546_v4l2_reg_multi_layer(i, pdev, disp_dev)) {
			ret = -ENODEV;
			return ret;
		}
	}
...
//#endif
	 ...
	 */
	ecv5546_osd_resolution_init(disp_dev);

	/*
	 * init resolution setting for vpp layers
	 */
	ecv5546_vpp_resolution_init(disp_dev);
...
//#ifndef CONFIG_DRM_eCV5546
	...
	for (i = 0; i < SP_DISP_MAX_DEVICES; i++) {
		if (i == 4) {
			...
			if (disp_dev->vpp_res[0].color_mode == eCV5546_VPP_IMGREAD_DATA_FMT_UYVY)
				disp_dev->dev[i]->fmt.fmt.pix.pixelformat = V4L2_PIX_FMT_UYVY;
			else if (disp_dev->vpp_res[0].color_mode == eCV5546_VPP_IMGREAD_DATA_FMT_YUY2)
				disp_dev->dev[i]->fmt.fmt.pix.pixelformat = V4L2_PIX_FMT_YUYV;
			else if (disp_dev->vpp_res[0].color_mode == eCV5546_VPP_IMGREAD_DATA_FMT_NV16)
				disp_dev->dev[i]->fmt.fmt.pix.pixelformat = V4L2_PIX_FMT_NV16;
			else if (disp_dev->vpp_res[0].color_mode == eCV5546_VPP_IMGREAD_DATA_FMT_NV24)
				disp_dev->dev[i]->fmt.fmt.pix.pixelformat = V4L2_PIX_FMT_NV24;
			else if (disp_dev->vpp_res[0].color_mode == eCV5546_VPP_IMGREAD_DATA_FMT_NV12)
			...
			if (disp_dev->osd_res[i].color_mode == eCV5546_OSD_COLOR_MODE_8BPP)
				disp_dev->dev[i]->fmt.fmt.pix.pixelformat = V4L2_PIX_FMT_GREY;
			else if (disp_dev->osd_res[i].color_mode == eCV5546_OSD_COLOR_MODE_YUY2)
				disp_dev->dev[i]->fmt.fmt.pix.pixelformat = V4L2_PIX_FMT_YUYV;
			else if (disp_dev->osd_res[i].color_mode == eCV5546_OSD_COLOR_MODE_RGB565)
				disp_dev->dev[i]->fmt.fmt.pix.pixelformat = V4L2_PIX_FMT_RGB565;
			else if (disp_dev->osd_res[i].color_mode == eCV5546_OSD_COLOR_MODE_ARGB1555)
				disp_dev->dev[i]->fmt.fmt.pix.pixelformat = V4L2_PIX_FMT_ARGB555;
			else if (disp_dev->osd_res[i].color_mode == eCV5546_OSD_COLOR_MODE_RGBA4444)
				disp_dev->dev[i]->fmt.fmt.pix.pixelformat = V4L2_PIX_FMT_RGB444;
			else if (disp_dev->osd_res[i].color_mode == eCV5546_OSD_COLOR_MODE_ARGB4444)
				disp_dev->dev[i]->fmt.fmt.pix.pixelformat = V4L2_PIX_FMT_ARGB444;
			else if (disp_dev->osd_res[i].color_mode == eCV5546_OSD_COLOR_MODE_RGBA8888)
				disp_dev->dev[i]->fmt.fmt.pix.pixelformat = V4L2_PIX_FMT_ABGR32;
			else if (disp_dev->osd_res[i].color_mode == eCV5546_OSD_COLOR_MODE_ARGB8888)
	 ...
	 */
	if (disp_dev->out_res.mipitx_mode == eCV5546_MIPITX_DSI)
		ecv5546_mipitx_phy_init_dsi();
	else
		ecv5546_mipitx_phy_init_csi();
//#endif
...
static int ecv5546_display_remove(struct platform_device *pdev)
{
	 ...
	 * release debug fs
	 */
	ecv5546_debug_cleanup(disp_dev);
	/*
	 * release debug proc
	 */
	if (entry)
...
static int ecv5546_display_suspend(struct platform_device *pdev, pm_message_t state)
{
	struct sp_disp_device *disp_dev = platform_get_drvdata(pdev);
	int i;
	...
	if (disp_dev->mipitx_dev_id == 0x00001002) {
		...
		rpi_touchscreen_i2c_write(g_ts, REG_PWM, 0);
		rpi_touchscreen_i2c_write(g_ts, REG_POWERON, 0);
		udelay(1);
	}
#endif
	...
	ecv5546_tcon_store();
#ifndef CONFIG_DRM_eCV5546
	ecv5546_mipitx_store();
#endif
	ecv5546_osd_store();
	ecv5546_osd_header_save();
	ecv5546_vpp0_store();
...
static int ecv5546_display_resume(struct platform_device *pdev)
{
	struct sp_disp_device *disp_dev = platform_get_drvdata(pdev);
	int i;
	 ...
	 */
	ecv5546_dmix_layer_cfg_restore();
	ecv5546_tgen_restore();
	ecv5546_tcon_restore();
//#ifndef CONFIG_DRM_eCV5546
	ecv5546_mipitx_restore();
//#endif
	ecv5546_osd_restore();
	for (i = 0; i < SP_DISP_MAX_OSD_LAYER; i++)
		ecv5546_osd_header_restore(i);

	ecv5546_vpp0_restore();
...
//#endif
	...
	if (disp_dev->mipitx_dev_id == 0x00001002) {
		...
		rpi_touchscreen_i2c_write(g_ts, REG_POWERON, 1);
		/* Wait for nPWRDWN to go low to indicate poweron is done. */
		for (i = 0; i < 100; i++) {
			if (rpi_touchscreen_i2c_read(g_ts, REG_PORTB) & 1)
				break;
		}
...
static const struct of_device_id ecv5546_display_ids[] = {
	{ .compatible = "eys3d,ecv5546-display"},
	{}
};
MODULE_DEVICE_TABLE(of, ecv5546_display_ids);

static struct platform_driver ecv5546_display_driver = {
	.probe		= ecv5546_display_probe,
	.remove		= ecv5546_display_remove,
	.suspend	= ecv5546_display_suspend,
	.resume		= ecv5546_display_resume,
	.driver		= {
		.name	= "ecv5546_display",
		.of_match_table	= ecv5546_display_ids,
	},
};
module_platform_driver(ecv5546_display_driver);

MODULE_DESCRIPTION("eYs3D eCV5546 Display Driver");
MODULE_AUTHOR("Hammer Hsieh <eys3d@eys3d.com>");
