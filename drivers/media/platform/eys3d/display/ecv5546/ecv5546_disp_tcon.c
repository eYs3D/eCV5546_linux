...
void ecv5546_tcon_init(void)
{
	...
	if (disp_dev->out_res.mipitx_mode == eCV5546_MIPITX_DSI) {
		...
		if (disp_dev->mipitx_format == 0x0)
			value |= eCV5546_TCON_OUT_PACKAGE_SET(eCV5546_TCON_OUT_PACKAGE_DSI_RGB565);
		else if (disp_dev->mipitx_format == 0x1)
			value |= eCV5546_TCON_OUT_PACKAGE_SET(eCV5546_TCON_OUT_PACKAGE_DSI_RGB666_18);
		else if (disp_dev->mipitx_format == 0x2)
			value |= eCV5546_TCON_OUT_PACKAGE_SET(eCV5546_TCON_OUT_PACKAGE_DSI_RGB666_24);
		else if (disp_dev->mipitx_format == 0x3)
			value |= eCV5546_TCON_OUT_PACKAGE_SET(eCV5546_TCON_OUT_PACKAGE_DSI_RGB888);
		else
			value |= eCV5546_TCON_OUT_PACKAGE_SET(eCV5546_TCON_OUT_PACKAGE_DSI_RGB888);

		value |= eCV5546_TCON_DOT_RGB888_MASK |
			eCV5546_TCON_DOT_ORDER_SET(eCV5546_TCON_DOT_ORDER_RGB) |
			eCV5546_TCON_HVIF_EN | eCV5546_TCON_YU_SWAP;
	} else {
		value = 0;
		if (disp_dev->mipitx_format == 0x0)
			value |= eCV5546_TCON_OUT_PACKAGE_SET(eCV5546_TCON_OUT_PACKAGE_CSI_RGB565);
		else if (disp_dev->mipitx_format == 0x1)
			value |= eCV5546_TCON_OUT_PACKAGE_SET(eCV5546_TCON_OUT_PACKAGE_CSI_YUY2);
		else if (disp_dev->mipitx_format == 0x2)
			value |= eCV5546_TCON_OUT_PACKAGE_SET(eCV5546_TCON_OUT_PACKAGE_CSI_YUY2);
		else if (disp_dev->mipitx_format == 0x3)
			value |= eCV5546_TCON_OUT_PACKAGE_SET(eCV5546_TCON_OUT_PACKAGE_CSI_RGB888);
		else
			value |= eCV5546_TCON_OUT_PACKAGE_SET(eCV5546_TCON_OUT_PACKAGE_CSI_RGB888);

		if ((disp_dev->mipitx_format == 0x1) || (disp_dev->mipitx_format == 0x2))
			value |= eCV5546_TCON_DITHER_YUV_EN;
	...
	value = 0;
	value |= (eCV5546_TCON_EN | eCV5546_TCON_YUV_UV_SWAP |
		eCV5546_TCON_STHLR_DLY_SET(eCV5546_TCON_STHLR_DLY_1T));
	writel(value, disp_dev->base + TCON_TCON1);

	value = 0;
	//value |= eCV5546_TCON_HDS_FILTER;
	writel(value, disp_dev->base + TCON_TCON2); //don't care

	value = 0;
	//value |= eCV5546_TCON_OEH_POL;
	writel(value, disp_dev->base + TCON_TCON3); //don't care

	value = 0;
	value |= eCV5546_TCON_PIX_EN_SEL_SET(eCV5546_TCON_PIX_EN_DIV_1_CLK_TCON);
	writel(value, disp_dev->base + TCON_TCON4); //fixed , don't change it

	value = 0;
	value |= eCV5546_TCON_CHK_SUM_EN;
...
EXPORT_SYMBOL(ecv5546_tcon_decrypt_info);
...
EXPORT_SYMBOL(ecv5546_tcon_resolution_chk);
...
EXPORT_SYMBOL(ecv5546_tcon_bist_info);
...
void ecv5546_tcon_bist_set(int bist_mode, int tcon_bist_pat)
{
	...
	value &= ~(eCV5546_TCON_TPG_PATTERN | eCV5546_TCON_TPG_MODE);
	value |= eCV5546_TCON_TPG_PATTERN_SET(tcon_bist_pat) |
		eCV5546_TCON_TPG_MODE_SET(bist_mode);
	writel(value , disp_dev->base + TCON_TPG_CTRL);

}
EXPORT_SYMBOL(ecv5546_tcon_bist_set);
...
void ecv5546_tcon_gen_pix_set(int enable)
{
	...
	if (enable)
		value |= eCV5546_TCON_AFIFO_DIS;
	else
		value &= ~(eCV5546_TCON_AFIFO_DIS);
	writel(value , disp_dev->base + TCON_TCON5);
}
EXPORT_SYMBOL(ecv5546_tcon_gen_pix_set);
...
static const u32 sp_tcon_para_dsi[11][12] = {
	...
	{ 480, 1280,    0,  479,  670,  674,  670,  674,    0,    0, 1311,    0}, /* 480x1280 */ //update for sync drm setting
	...
	{ 800,  480,    0,  799,  909,  913,  909,  913,    0,    0,  509,    0}, /* 800x480 */ //update for sync drm setting
...
static const u32 sp_tcon_tpg_para_dsi[11][10] = {
	...
	{ 480, 1280,    4,    4,  677, 1311,  479, 1279,  15, 0x01}, /* 480x1280 */
	...
	{ 800,  480,    4,    4,  916,  509,  799,  479,  22, 0x01}, /* 800x480 */
...
void ecv5546_tcon_gamma_table_set(u32 updsel_rgb, const u16 *table, u32 tablesize)
{
	...
	do {
		value = readl(disp_dev->base + TCON_GAMMA0); //G200.00
		if (!(value & eCV5546_TCON_GM_UPDEN)) {
			break;
		}
		udelay(50);
	} while(1);

	value &= ~(eCV5546_TCON_GM_UPDDEL_RGB_MASK);
	value |= eCV5546_TCON_GM_EN | eCV5546_TCON_GM_UPD_SCHEME | eCV5546_TCON_GM_BYPASS | eCV5546_TCON_GM_UPDWE |
			eCV5546_TCON_GM_UPDDEL_RGB_SET(updsel_rgb);
	...
	/*value = readl(disp_dev->base + TCON_GAMMA0); //G200.00
	value |= eCV5546_TCON_GM_EN;
	value &= ~(eCV5546_TCON_GM_BYPASS | eCV5546_TCON_GM_UPDWE);
	writel(value, disp_dev->base + TCON_GAMMA0);
	*/
}
EXPORT_SYMBOL(ecv5546_tcon_gamma_table_set);
...
void ecv5546_tcon_gamma_table_get(u32 updsel_rgb, u16 *table, u32 tablesize)
{
	...
	value &= ~(eCV5546_TCON_GM_UPDDEL_RGB_MASK);
#if C3V_DISP_TCON_GAMMA_WORKAROUND_EN
	value |= eCV5546_TCON_GM_EN | eCV5546_TCON_GM_BYPASS |
			eCV5546_TCON_GM_UPDDEL_RGB_SET(updsel_rgb);
#else
	value |= eCV5546_TCON_GM_EN | eCV5546_TCON_GM_UPD_SCHEME | eCV5546_TCON_GM_BYPASS |
			eCV5546_TCON_GM_UPDDEL_RGB_SET(updsel_rgb);
#endif
	value &= ~(eCV5546_TCON_GM_UPDWE);
	writel(value, disp_dev->base + TCON_GAMMA0);
...
EXPORT_SYMBOL(ecv5546_tcon_gamma_table_get);
...
void ecv5546_tcon_gamma_table_enable(int enable)
{
	...
	if (enable) {
		...
		value &= ~(eCV5546_TCON_GM_EN);
		value |= eCV5546_TCON_GM_BYPASS;
	}

	writel(value, disp_dev->base + TCON_GAMMA0);
}
EXPORT_SYMBOL(ecv5546_tcon_gamma_table_enable);
...
int ecv5546_tcon_rgb_adjust_cp_set(u32 channel_sel, const u8 *cp_src, const u8 *cp_sdt, u32 cp_size)
{
	...
	if (channel_sel & eCV5546_TCON_RGB_ADJ_CHANNEL_R_EN)
		cp_base = TCON_R_ADJ_CP1;
	else if (channel_sel & eCV5546_TCON_RGB_ADJ_CHANNEL_G_EN)
		cp_base = TCON_G_ADJ_CP1;
	else if (channel_sel & eCV5546_TCON_RGB_ADJ_CHANNEL_B_EN)
		cp_base = TCON_B_ADJ_CP1;
	else
		return -1;

	for (i = 0; i < cp_size; i++) {
		//value |= eCV5546_TCON_RGB_ADJ_CP_SRC_SET(cp_src[i]) | eCV5546_TCON_RGB_ADJ_CP_SDT_SET(cp_sdt);
...
EXPORT_SYMBOL(ecv5546_tcon_rgb_adjust_cp_set);
...
EXPORT_SYMBOL(ecv5546_tcon_rgb_adjust_slope_set);
...
int ecv5546_tcon_rgb_adjust_cp_get(u32 channel_sel, u8 *cp_src, u8 *cp_sdt, u32 cp_size)
{
	...
	for (i = 0; i < cp_size; i++) {
		...
		cp_src[i] = value & eCV5546_TCON_RGB_ADJ_CP_SRC_MASK;
		cp_sdt[i] = (value & eCV5546_TCON_RGB_ADJ_CP_SDT_MASK) >>8;
	}

	return 0;
}
EXPORT_SYMBOL(ecv5546_tcon_rgb_adjust_cp_get);
...
EXPORT_SYMBOL(ecv5546_tcon_rgb_adjust_slope_get);
...
void ecv5546_tcon_rgb_adjust_enable(u32 channel_sel)
{
	...
	value &= ~(eCV5546_TCON_RGB_ADJ_R_EN | eCV5546_TCON_RGB_ADJ_G_EN | eCV5546_TCON_RGB_ADJ_B_EN);
	if (channel_sel & eCV5546_TCON_RGB_ADJ_CHANNEL_R_EN)
		value |= eCV5546_TCON_RGB_ADJ_R_EN;
	if (channel_sel & eCV5546_TCON_RGB_ADJ_CHANNEL_G_EN)
		value |= eCV5546_TCON_RGB_ADJ_G_EN;
	if (channel_sel & eCV5546_TCON_RGB_ADJ_CHANNEL_B_EN)
		value |= eCV5546_TCON_RGB_ADJ_B_EN;

	writel(value, disp_dev->base + TCON_RGB_ADJ_CTRL);
}
EXPORT_SYMBOL(ecv5546_tcon_rgb_adjust_enable);
...
void ecv5546_tcon_enhanced_dither_6bit_set(u32 mode, u32 table_v_shift_en, u32 table_h_shift_en)
{
	...
	if (mode == eCV5546_TCON_DITHER_6BIT_MODE_ROBUST)
		value |= eCV5546_TCON_DITHER_6BIT_MODE;
	else
		value &= ~(eCV5546_TCON_DITHER_6BIT_MODE);

	if (table_v_shift_en)
		value |= eCV5546_TCON_DITHER_6BIT_TABLE_V_SHIFT_EN;
	else
		value &= ~(eCV5546_TCON_DITHER_6BIT_TABLE_V_SHIFT_EN);

	if (table_h_shift_en)
		value |= eCV5546_TCON_DITHER_6BIT_TABLE_H_SHIFT_EN;
	else
		value &= ~(eCV5546_TCON_DITHER_6BIT_TABLE_H_SHIFT_EN);
	...
	value &= ~(eCV5546_TCON_DOT_RGB888_MASK | eCV5546_TCON_OUT_PACKAGE_MASK);
	value |= eCV5546_TCON_OUT_PACKAGE_SET(eCV5546_TCON_OUT_PACKAGE_DSI_RGB666_24);
	writel(value, disp_dev->base + TCON_TCON0);
}
EXPORT_SYMBOL(ecv5546_tcon_enhanced_dither_6bit_set);
...
void ecv5546_tcon_enhanced_dither_8bit_set(void)
{
	...
	value = readl(disp_dev->base + TCON_TCON0);
	value &= ~(eCV5546_TCON_OUT_PACKAGE_MASK);
	value |= eCV5546_TCON_DOT_RGB888_MASK | eCV5546_TCON_OUT_PACKAGE_SET(eCV5546_TCON_OUT_PACKAGE_DSI_RGB888);
	writel(value, disp_dev->base + TCON_TCON0);
}
EXPORT_SYMBOL(ecv5546_tcon_enhanced_dither_8bit_set);
...
void ecv5546_tcon_enhanced_dither_set(u32 rgbc_sel, u32 method , u32 temporal_mode_en, u32 dot_mode)
{
	...
	if (rgbc_sel == eCV5546_TCON_DITHER_RGBC_SEL_RGB)
		value |= eCV5546_TCON_DITHER_RGBC_SEL;
	else
		value &= ~(eCV5546_TCON_DITHER_RGBC_SEL);

	if (method == eCV5546_TCON_DITHER_INIT_MODE_METHOD1)
		value |= eCV5546_TCON_DITHER_INIT_MODE;
	else
		value &= ~(eCV5546_TCON_DITHER_INIT_MODE);

	if (temporal_mode_en)
		value |= eCV5546_TCON_DITHER_TEMP_EN;
	else
		value &= ~(eCV5546_TCON_DITHER_TEMP_EN);

	value &= ~(eCV5546_TCON_DITHER_PANEL_DOT_MODE_MASK);
	value |= eCV5546_TCON_DITHER_PANEL_DOT_MODE_SET(dot_mode);

	writel(value, disp_dev->base + TCON_DITHER_TVOUT);
}
EXPORT_SYMBOL(ecv5546_tcon_enhanced_dither_set);
...
void ecv5546_tcon_enhanced_dither_enable(u32 enable)
{
	...
	if (enable)
		value |= eCV5546_TCON_DITHER_NEW_EN;
	else
		value &= ~(eCV5546_TCON_DITHER_NEW_EN);

	writel(value, disp_dev->base + TCON_DITHER_TVOUT);
}
EXPORT_SYMBOL(ecv5546_tcon_enhanced_dither_enable);
...
int ecv5546_tcon_bitswap_set(int bit_mode, int channel_mode)
{
	...
	if (eCV5546_TCON_BIT_SW_CHNL_RGB == channel_mode) {
		#if BIT_SWAP_IN_EN
	...
	else if (eCV5546_TCON_BIT_SW_CHNL_RBG == channel_mode) {
		#if BIT_SWAP_IN_EN
		memcpy(dotmap,    dotr, 8*sizeof(tmp[0]));
	...
	else if (eCV5546_TCON_BIT_SW_CHNL_GBR == channel_mode) {
		#if BIT_SWAP_IN_EN
		memcpy(dotmap,    dotg, 8*sizeof(tmp[0]));
		memcpy(dotmap+8,  dotb, 8*sizeof(tmp[0]));
		memcpy(dotmap+16, dotr, 8*sizeof(tmp[0]));
	...
	else if (eCV5546_TCON_BIT_SW_CHNL_GRB == channel_mode) {
		#if BIT_SWAP_IN_EN
		memcpy(dotmap,    dotg, 8*sizeof(tmp[0]));
	...
	else if (eCV5546_TCON_BIT_SW_CHNL_BRG == channel_mode) {
		#if BIT_SWAP_IN_EN
		memcpy(dotmap,    dotb, 8*sizeof(tmp[0]));
		memcpy(dotmap+8,  dotr, 8*sizeof(tmp[0]));
		memcpy(dotmap+16, dotg, 8*sizeof(tmp[0]));
	...
	else if (eCV5546_TCON_BIT_SW_CHNL_BGR == channel_mode) {
		#if BIT_SWAP_IN_EN
		memcpy(dotmap,    dotb, 8*sizeof(tmp[0]));
		memcpy(dotmap+8,  dotg, 8*sizeof(tmp[0]));
		memcpy(dotmap+16, dotr, 8*sizeof(tmp[0]));
	...
	if (eCV5546_TCON_BIT_SW_BIT_LSB == bit_mode) {
		for(i=0; i < 24; i++) {
			tmp[i] = dotmap[23-i];
		}
...
EXPORT_SYMBOL(ecv5546_tcon_bitswap_set);
...
void ecv5546_tcon_bitswap_enable(int enable)
{
	...
	if (enable)
		value |= eCV5546_TCON_BIT_SW_ON;
	else
		value &= ~(eCV5546_TCON_BIT_SW_ON);

	writel(value, disp_dev->base + TCON_BIT_SWAP_CFG);
}
EXPORT_SYMBOL(ecv5546_tcon_bitswap_enable);
