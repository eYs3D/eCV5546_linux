...
#include "ecv5546_osd_palette.h"
#include "ecv5546_osd_header.h" //this is for test
#include <media/eys3d/disp/ecv5546/ecv5546_disp_osd.h>

int ecv5546_disp_state = 1;
EXPORT_SYMBOL(ecv5546_disp_state);

static struct ecv5546_osd_region *sp_osd_region_vir;
...
EXPORT_SYMBOL(ecv5546_osd_decrypt_info);
...
EXPORT_SYMBOL(ecv5546_osd_resolution_chk);
...
EXPORT_SYMBOL(ecv5546_osd_bist_info);
...
void ecv5546_osd_bist_set(int osd_layer_sel, int bist_en, int osd_bist_type)
{
	...
	pr_info("ecv5546_osd_bist_set: OSD%d\n", osd_layer_sel);
	value = readl(disp_dev->base + (osd_layer_sel << 7) + OSD_BIST_CTRL);
	value &= ~(eCV5546_OSD_BIST_MASK);
	value |= eCV5546_OSD_BIST_EN(bist_en) | eCV5546_OSD_BIST_TYPE(osd_bist_type);
	writel(value, disp_dev->base + (osd_layer_sel << 7) + OSD_BIST_CTRL);

}
EXPORT_SYMBOL(ecv5546_osd_bist_set);
...
void ecv5546_osd_header_init(void)
{
	...
	for (i = 0; i < SP_DISP_MAX_OSD_LAYER; i++) {
		disp_dev->osd_hdr[i] = dma_alloc_coherent(disp_dev->pdev,
				sizeof(struct ecv5546_osd_header) + 1024,
...
EXPORT_SYMBOL(ecv5546_osd_header_init);
...
EXPORT_SYMBOL(ecv5546_osd_header_clear);
...
EXPORT_SYMBOL(ecv5546_osd_header_show);
...
EXPORT_SYMBOL(ecv5546_osd_header_save);
...
EXPORT_SYMBOL(ecv5546_osd_header_restore);
...
EXPORT_SYMBOL(ecv5546_osd_header_update);
...
void ecv5546_osd_layer_set(struct ecv5546fb_info *info, int osd_layer_sel)
{
	...
	//if (disp_dev->osd_res[osd_layer_sel].color_mode == eCV5546_OSD_COLOR_MODE_8BPP)
	...
	if ((disp_dev->dev[osd_layer_sel]) &&
		(info->color_mode == eCV5546_OSD_COLOR_MODE_8BPP)) {
...
EXPORT_SYMBOL(ecv5546_osd_layer_set);
...
void ecv5546_osd_layer_set_by_region(struct ecv5546_osd_region *info, int osd_layer_sel)
{
	...
	if(info->alpha_info.region_alpha_en){
		...
		value |= eCV5546_OSD_HDR_BL |tmp_alpha;
	}
...
EXPORT_SYMBOL(ecv5546_osd_layer_set_by_region);

int ecv5546_osd_resolution_init(struct sp_disp_device *disp_dev)
{
...
EXPORT_SYMBOL(ecv5546_osd_resolution_init);
...
int ecv5546_osd_get_fbinfo(struct ecv5546fb_info *pinfo)
{
	...
	//pr_info("ecv5546_osd_get_fbinfo\n");
	/* get ecv5546fb resolution from dts */
	#if 1
...
EXPORT_SYMBOL(ecv5546_osd_get_fbinfo);
...
int ecv5546_osd_region_create(struct ecv5546fb_info *pinfo)
{
	...
	/*
	 * alloc ecv5546 osd header/region memory
	 */
	...
	sp_osd_region_vir = dma_alloc_coherent(disp_dev->pdev,
			sizeof(struct ecv5546_osd_region),
			&sp_osd_region_phy,
			GFP_KERNEL | __GFP_ZERO);
	...
	/*
	 * update ecv5546 osd region info
	 */
	//todo
	...
	sp_osd_region_vir->hdr = (struct ecv5546_osd_header *)sp_osd_header_vir;
	sp_osd_region_vir->hdr_pal = sp_osd_header_vir + sizeof(struct ecv5546_osd_header);
	...
	/*
	 * update ecv5546 osd header info
	 */
	osd_header = (u32 *)sp_osd_header_vir;

	if (pinfo->color_mode == eCV5546_OSD_COLOR_MODE_8BPP)
	...
	if (pinfo->color_mode == eCV5546_OSD_COLOR_MODE_YUY2)
...
EXPORT_SYMBOL(ecv5546_osd_region_create);
...
EXPORT_SYMBOL(ecv5546_osd_region_release);

void ecv5546_osd_region_update(void)
{
	struct ecv5546_osd_region *region = sp_osd_region_vir;
	struct sp_disp_device *disp_dev = gdisp_dev;
	struct ecv5546_osd_header *osd_hdr;
	u32 region_ofs;
	...
	osd_hdr = (struct ecv5546_osd_header *)region->hdr;
	...
	if (disp_dev->osd_fe_protect) {
		disp_dev->osd_fe_protect &= ~(1 << eCV5546_LAYER_OSD0);
...
EXPORT_SYMBOL(ecv5546_osd_region_wait_sync);
...
EXPORT_SYMBOL(ecv5546_osd_region_buf_fetch_done);

void ecv5546_osd_region_irq_disable(void)
{
	ecv5546_disp_state = 1;

}
EXPORT_SYMBOL(ecv5546_osd_region_irq_disable);

void ecv5546_osd_region_irq_enable(void)
{
	ecv5546_disp_state = 0;

}
EXPORT_SYMBOL(ecv5546_osd_region_irq_enable);
