...
EXPORT_SYMBOL(ecv5546_vpp_decrypt_info);
...
EXPORT_SYMBOL(ecv5546_vpp_imgread_resolution_chk);
...
EXPORT_SYMBOL(ecv5546_vpp_vscl_resolution_chk);
...
void ecv5546_vpp_layer_onoff(int onoff)
{
	...
	//pr_info("ecv5546_vpp_layer: VPP0 %s\n", onoff?"ON":"OFF");
...
EXPORT_SYMBOL(ecv5546_vpp_layer_onoff);
...
EXPORT_SYMBOL(ecv5546_vpp_bist_info);
...
void ecv5546_vpp_bist_set(int img_vscl_sel, int bist_en, int vpp_bist_type)
{
	...
	if (img_vscl_sel == 0) {
		pr_info("ecv5546_vpp_bist_set: VPP0 for imgread\n");
		value = readl(disp_dev->base + IMGREAD_CONFIG);
		value &= ~(eCV5546_VPP_IMGREAD_BIST_MASK);
		...
		pr_info("ecv5546_vpp_bist_set: VPP0 for vscl\n");
		value = readl(disp_dev->base + VSCL_CONFIG2);
		value &= ~(eCV5546_VPP_VSCL_BIST_MASK);
		writel((value | bist_en << 7 | vpp_bist_type << 8), disp_dev->base + VSCL_CONFIG2);
	} else {
		pr_info("ecv5546_vpp_bist_set: undef\n");
	}

}
EXPORT_SYMBOL(ecv5546_vpp_bist_set);
 ...
 */
#define eCV5546_VPP_SCALE_METHOD		1
/*
 * 0: use (factor - 1) , closet
 * 1: use (factor) , round up
 */
#define eCV5546_VPP_FACTOR_METHOD	0
...
EXPORT_SYMBOL(ecv5546_vpp_imgread_set);
#else
...
int ecv5546_vpp_vscl_set(int x, int y, int img_src_w, int img_src_h, int img_dest_x, int img_dest_y, int img_dest_w, int img_dest_h, int output_w, int output_h)
{
	...
	#if  0//CONFIG_DRM_eCV5546
	writel(output_w, disp_dev->base + VSCL_ACTRL_I_XLEN);
	writel(output_h, disp_dev->base + VSCL_ACTRL_I_YLEN);
	#else
	writel(crop_xlen, disp_dev->base + VSCL_ACTRL_I_XLEN);
	writel(crop_ylen, disp_dev->base + VSCL_ACTRL_I_YLEN);
	#endif
	...
	writel(crop_xlen, disp_dev->base + VSCL_ACTRL_S_XLEN);
	writel(crop_ylen, disp_dev->base + VSCL_ACTRL_S_YLEN);
	...
	source_w = readl(disp_dev->base + VSCL_ACTRL_S_XLEN);
	...
	source_h = readl(disp_dev->base + VSCL_ACTRL_S_YLEN);
...
EXPORT_SYMBOL(ecv5546_vpp_vscl_set);
#else
int ecv5546_vpp_vscl_set(int x, int y, int xlen, int ylen, int input_w, int input_h, int img_dest_x, int img_dest_y, int output_w, int output_h)
{
	struct sp_disp_device *disp_dev = gdisp_dev;
	u64 factor64, factor_init64;
	u32 value;
...
int ecv5546_vpp_vpost_set(int x, int y, int input_w, int input_h, int output_w, int output_h)
{
	...
	//value |= eCV5546_VPP_VPOST_WIN_YUV_EN;
	...
	value |= (eCV5546_VPP_VPOST_WIN_ALPHA_SET(eCV5546_VPP_VPOST_WIN_ALPHA_VALUE) |
	...
	value |= eCV5546_VPP_VPOST_OPIF_TOP_SET(output_h >> 2);
	...
	value |= eCV5546_VPP_VPOST_OPIF_BOT_SET(output_h >> 2);
	...
	value |= eCV5546_VPP_VPOST_OPIF_LEFT_SET(output_w >> 2);
	...
	value |= eCV5546_VPP_VPOST_OPIF_RIGHT_SET(output_w >> 2);
	writel(value, disp_dev->base + VPOST_OPIF_MSKRIGHT);

	return 0;
}
EXPORT_SYMBOL(ecv5546_vpp_vpost_set);
...
EXPORT_SYMBOL(ecv5546_vpp_vpost_opif_set);
...
EXPORT_SYMBOL(ecv5546_vpp_vpost_opif_alpha_set);

int ecv5546_vpp_resolution_init(struct sp_disp_device *disp_dev)
{
	/*
	 * enable vpp layer
	 */
	 ...
	 */
	#ifdef SP_DISP_VPP_SCALE_NEW
	ecv5546_vpp_imgread_set((u32)virt_to_phys(&vpp0_data_array),
...
EXPORT_SYMBOL(ecv5546_vpp_resolution_init);

int ecv5546_vpp_restore(struct sp_disp_device *disp_dev)
{
	/*
	 * set vpp layer for imgread block
	 */
...
EXPORT_SYMBOL(ecv5546_vpp_restore);
