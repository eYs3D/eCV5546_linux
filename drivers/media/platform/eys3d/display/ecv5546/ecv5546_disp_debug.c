...
/*
 * eYs3D eCV5546 SoC Display function debug
 *
...
#include "ecv5546_display.h"
#include "ecv5546_disp_regs.h"

static void ecv5546_dmix_luma_adjust_selftest(void);
static void ecv5546_dmix_chroma_adjust_selftest(void);
static void ecv5546_tcon_rgb_adjust_selftest(u32 channel_sel);
 ...
 *
 */
static void eys3d_debug_cmd(char *tmpbuf)
{
	...
	if (!strncasecmp(tmpbuf, "bist", 4)) {
		...
		if (!strncasecmp(tmpbuf, "info", 4)) {
			ecv5546_vpp_bist_info();
			ecv5546_osd_bist_info();
			ecv5546_dmix_bist_info();
			ecv5546_tcon_bist_info();
		} else if (!strncasecmp(tmpbuf, "img", 3)) {
			tmpbuf = spmon_skipspace(tmpbuf + 3);
			pr_info("bist vpp0 imgread cmd\n");
			ecv5546_vpp_layer_onoff(1);
			ecv5546_dmix_layer_init(eCV5546_DMIX_L6, eCV5546_DMIX_OSD0, eCV5546_DMIX_TRANSPARENT);
			ecv5546_dmix_layer_init(eCV5546_DMIX_L5, eCV5546_DMIX_OSD1, eCV5546_DMIX_TRANSPARENT);
			ecv5546_dmix_layer_init(eCV5546_DMIX_L4, eCV5546_DMIX_OSD2, eCV5546_DMIX_TRANSPARENT);
			ecv5546_dmix_layer_init(eCV5546_DMIX_L3, eCV5546_DMIX_OSD3, eCV5546_DMIX_TRANSPARENT);
			ecv5546_dmix_layer_init(eCV5546_DMIX_L1, eCV5546_DMIX_VPP0, eCV5546_DMIX_BLENDING);
			if (!strncasecmp(tmpbuf, "off", 3)) {
				pr_info("bist vpp0 imgread off\n");
				ecv5546_vpp_bist_set(0, 0, 0);
			} else if (!strncasecmp(tmpbuf, "bar", 3)) {
				pr_info("bist vpp0 imgread bar\n");
				ecv5546_vpp_bist_set(0, 1, 0);
			} else if (!strncasecmp(tmpbuf, "bor", 3)) {
				pr_info("bist vpp0 imgread bor\n");
				ecv5546_vpp_bist_set(0, 1, 1);
			} else
			...
			if (!strncasecmp(tmpbuf, "off", 3)) {
				...
				ecv5546_vpp_bist_set(1, 0, 0);
			} else if (!strncasecmp(tmpbuf, "bar", 3)) {
				pr_info("bist vpp0 vscl bar\n");
				ecv5546_vpp_bist_set(1, 1, 0);
			} else if (!strncasecmp(tmpbuf, "bor", 3)) {
				pr_info("bist vpp0 vscl bor\n");
				ecv5546_vpp_bist_set(1, 1, 1);
			} else
			...
			ecv5546_osd_layer_onoff(eCV5546_LAYER_OSD0, 1); //OSD0 on
			ecv5546_dmix_layer_init(eCV5546_DMIX_L6, eCV5546_DMIX_OSD0, eCV5546_DMIX_BLENDING);
			ecv5546_dmix_layer_init(eCV5546_DMIX_L5, eCV5546_DMIX_OSD1, eCV5546_DMIX_TRANSPARENT);
			ecv5546_dmix_layer_init(eCV5546_DMIX_L4, eCV5546_DMIX_OSD2, eCV5546_DMIX_TRANSPARENT);
			ecv5546_dmix_layer_init(eCV5546_DMIX_L3, eCV5546_DMIX_OSD3, eCV5546_DMIX_TRANSPARENT);
			ecv5546_dmix_layer_init(eCV5546_DMIX_L1, eCV5546_DMIX_VPP0, eCV5546_DMIX_TRANSPARENT);
			if (!strncasecmp(tmpbuf, "off", 3)) {
				pr_info("bist osd0 off\n");
				ecv5546_osd_bist_set(0, 0, 0);
			} else if (!strncasecmp(tmpbuf, "bar", 3)) {
				pr_info("bist osd0 bar\n");
				ecv5546_osd_bist_set(0, 1, 0);
			} else if (!strncasecmp(tmpbuf, "bor", 3)) {
				pr_info("bist osd0 bor\n");
				ecv5546_osd_bist_set(0, 1, 1);
			} else
			...
			ecv5546_osd_layer_onoff(eCV5546_LAYER_OSD1, 1); //OSD1 on
			ecv5546_dmix_layer_init(eCV5546_DMIX_L6, eCV5546_DMIX_OSD0, eCV5546_DMIX_TRANSPARENT);
			ecv5546_dmix_layer_init(eCV5546_DMIX_L5, eCV5546_DMIX_OSD1, eCV5546_DMIX_BLENDING);
			ecv5546_dmix_layer_init(eCV5546_DMIX_L4, eCV5546_DMIX_OSD2, eCV5546_DMIX_TRANSPARENT);
			ecv5546_dmix_layer_init(eCV5546_DMIX_L3, eCV5546_DMIX_OSD3, eCV5546_DMIX_TRANSPARENT);
			ecv5546_dmix_layer_init(eCV5546_DMIX_L1, eCV5546_DMIX_VPP0, eCV5546_DMIX_TRANSPARENT);
			if (!strncasecmp(tmpbuf, "off", 3)) {
				pr_info("bist osd1 off\n");
				ecv5546_osd_bist_set(1, 0, 0);
			} else if (!strncasecmp(tmpbuf, "bar", 3)) {
				pr_info("bist osd1 bar\n");
				ecv5546_osd_bist_set(1, 1, 0);
			} else if (!strncasecmp(tmpbuf, "bor", 3)) {
				pr_info("bist osd1 bor\n");
				ecv5546_osd_bist_set(1, 1, 1);
			} else
			...
			ecv5546_osd_layer_onoff(eCV5546_LAYER_OSD2, 1); //OSD2 on
			ecv5546_dmix_layer_init(eCV5546_DMIX_L6, eCV5546_DMIX_OSD0, eCV5546_DMIX_TRANSPARENT);
			ecv5546_dmix_layer_init(eCV5546_DMIX_L5, eCV5546_DMIX_OSD1, eCV5546_DMIX_TRANSPARENT);
			ecv5546_dmix_layer_init(eCV5546_DMIX_L4, eCV5546_DMIX_OSD2, eCV5546_DMIX_BLENDING);
			ecv5546_dmix_layer_init(eCV5546_DMIX_L3, eCV5546_DMIX_OSD3, eCV5546_DMIX_TRANSPARENT);
			ecv5546_dmix_layer_init(eCV5546_DMIX_L1, eCV5546_DMIX_VPP0, eCV5546_DMIX_TRANSPARENT);
			if (!strncasecmp(tmpbuf, "off", 3)) {
				pr_info("bist osd2 off\n");
				ecv5546_osd_bist_set(2, 0, 0);
			} else if (!strncasecmp(tmpbuf, "bar", 3)) {
				pr_info("bist osd2 bar\n");
				ecv5546_osd_bist_set(2, 1, 0);
			} else if (!strncasecmp(tmpbuf, "bor", 3)) {
				pr_info("bist osd2 bor\n");
				ecv5546_osd_bist_set(2, 1, 1);
			} else
			...
			ecv5546_osd_layer_onoff(eCV5546_LAYER_OSD3, 1); //OSD3 on
			ecv5546_dmix_layer_init(eCV5546_DMIX_L6, eCV5546_DMIX_OSD0, eCV5546_DMIX_TRANSPARENT);
			ecv5546_dmix_layer_init(eCV5546_DMIX_L5, eCV5546_DMIX_OSD1, eCV5546_DMIX_TRANSPARENT);
			ecv5546_dmix_layer_init(eCV5546_DMIX_L4, eCV5546_DMIX_OSD2, eCV5546_DMIX_TRANSPARENT);
			ecv5546_dmix_layer_init(eCV5546_DMIX_L3, eCV5546_DMIX_OSD3, eCV5546_DMIX_BLENDING);
			ecv5546_dmix_layer_init(eCV5546_DMIX_L1, eCV5546_DMIX_VPP0, eCV5546_DMIX_TRANSPARENT);
			if (!strncasecmp(tmpbuf, "off", 3)) {
				pr_info("bist osd3 off\n");
				ecv5546_osd_bist_set(3, 0, 0);
			} else if (!strncasecmp(tmpbuf, "bar", 3)) {
				pr_info("bist osd3 bar\n");
				ecv5546_osd_bist_set(3, 1, 0);
			} else if (!strncasecmp(tmpbuf, "bor", 3)) {
				pr_info("bist osd3 bor\n");
				ecv5546_osd_bist_set(3, 1, 1);
			} else
			...
			if (!strncasecmp(tmpbuf, "dtgadj", 6)) {
				...
				if (!strncasecmp(tmpbuf, "ptg", 3)) {
					...
					input = eCV5546_TGEN_DTG_ADJ_PTG;
					tmpbuf = spmon_skipspace(tmpbuf + 3);
				} else if (!strncasecmp(tmpbuf, "osd0", 4)) {
					pr_info("bist tgen dtgadj osd0\n");
					input = eCV5546_TGEN_DTG_ADJ_OSD0;
					tmpbuf = spmon_skipspace(tmpbuf + 4);
				} else if (!strncasecmp(tmpbuf, "osd1", 4)) {
					pr_info("bist tgen dtgadj osd1\n");
					input = eCV5546_TGEN_DTG_ADJ_OSD1;
					tmpbuf = spmon_skipspace(tmpbuf + 4);
				} else if (!strncasecmp(tmpbuf, "osd2", 4)) {
					pr_info("bist tgen dtgadj osd2\n");
					input = eCV5546_TGEN_DTG_ADJ_OSD2;
					tmpbuf = spmon_skipspace(tmpbuf + 4);
				} else if (!strncasecmp(tmpbuf, "osd3", 4)) {
					pr_info("bist tgen dtgadj osd3\n");
					input = eCV5546_TGEN_DTG_ADJ_OSD3;
					tmpbuf = spmon_skipspace(tmpbuf + 4);
				} else {
				...
				if (adj_value != -1) {
					...
					ecv5546_tgen_input_adjust(input, adj_value);
				}
				else {	/* autotest */
					for (i = 0; i <= 0x3f; i++) {
						pr_info("Set[%d] adj_value %d\n",input, i);
						ecv5546_tgen_input_adjust(input, i);
						msleep(300);
					}
				}
			}  else
			...
			if (!strncasecmp(tmpbuf, "region", 6)) {
				...
				ecv5546_dmix_ptg_set(eCV5546_DMIX_BIST_REGION, 0x29f06e);
			} else if (!strncasecmp(tmpbuf, "snowhalf", 8)) {
				pr_info("bist dmix snowhalf\n");
				ecv5546_dmix_ptg_set(eCV5546_DMIX_BIST_SNOW_HALF, 0x29f06e);
			} else if (!strncasecmp(tmpbuf, "snowmax", 7)) {
				pr_info("bist dmix snowmax\n");
				ecv5546_dmix_ptg_set(eCV5546_DMIX_BIST_SNOW_MAX, 0x29f06e);
			} else if (!strncasecmp(tmpbuf, "snow", 4)) {
				pr_info("bist dmix snow\n");
				ecv5546_dmix_ptg_set(eCV5546_DMIX_BIST_SNOW, 0x29f06e);
			} else if (!strncasecmp(tmpbuf, "barv", 4)) {
				pr_info("bist dmix bar rot90\n");
				ecv5546_dmix_ptg_set(eCV5546_DMIX_BIST_COLORBAR_ROT90, 0x29f06e);
			} else if (!strncasecmp(tmpbuf, "bar", 3)) {
				pr_info("bist dmix bar rot0\n");
				ecv5546_dmix_ptg_set(eCV5546_DMIX_BIST_COLORBAR_ROT0, 0x29f06e);
			} else if (!strncasecmp(tmpbuf, "bor0", 4)) {
				pr_info("bist dmix bor pix0\n");
				ecv5546_dmix_ptg_set(eCV5546_DMIX_BIST_BORDER_NONE, 0x29f06e);
			} else if (!strncasecmp(tmpbuf, "bor1", 4)) {
				pr_info("bist dmix bor pix1\n");
				ecv5546_dmix_ptg_set(eCV5546_DMIX_BIST_BORDER_ONE, 0x29f06e);
			} else if (!strncasecmp(tmpbuf, "bor", 3)) {
				tmpbuf = spmon_skipspace(tmpbuf + 3);
				pr_info("bist dmix bor pix7\n");
				if (!strncasecmp(tmpbuf, "black", 5)) {
					pr_info("bist dmix bor black\n");
					ecv5546_dmix_ptg_set(eCV5546_DMIX_BIST_BORDER, 0x108080); //BG black
				} else if (!strncasecmp(tmpbuf, "red", 3)) {
					pr_info("bist dmix bor red\n");
					ecv5546_dmix_ptg_set(eCV5546_DMIX_BIST_BORDER, 0x4040f0); //BG red
				} else if (!strncasecmp(tmpbuf, "green", 5)) {
					pr_info("bist dmix bor green\n");
					ecv5546_dmix_ptg_set(eCV5546_DMIX_BIST_BORDER, 0x101010); //BG green
				} else if (!strncasecmp(tmpbuf, "blue", 4)) {
					pr_info("bist dmix bor blue\n");
					ecv5546_dmix_ptg_set(eCV5546_DMIX_BIST_BORDER, 0x29f06e); //BG blue
				} else {
			...
			if (!strncasecmp(tmpbuf, "off", 3)) {
				...
				if (!strncasecmp(tmpbuf, "inter", 5)) {
					...
					ecv5546_tcon_bist_set(1, 1);
				} else if (!strncasecmp(tmpbuf, "exter", 5)) {
					pr_info("bist tcon H_2_RAMP external\n");
					ecv5546_tcon_bist_set(2, 1);
				} else
				...
				if (!strncasecmp(tmpbuf, "inter", 5)) {
					...
					ecv5546_tcon_bist_set(1, 2);
				} else if (!strncasecmp(tmpbuf, "exter", 5)) {
					pr_info("bist tcon H_3_ODDEVEN external\n");
					ecv5546_tcon_bist_set(2, 2);
				} else
				...
				if (!strncasecmp(tmpbuf, "inter", 5)) {
					...
					ecv5546_tcon_bist_set(1, 3);
				} else if (!strncasecmp(tmpbuf, "exter", 5)) {
					pr_info("bist tcon V_1_COLORBAR external\n");
					ecv5546_tcon_bist_set(2, 3);
				} else
				...
				if (!strncasecmp(tmpbuf, "inter", 5)) {
					...
					ecv5546_tcon_bist_set(1, 4);
				} else if (!strncasecmp(tmpbuf, "exter", 5)) {
					pr_info("bist tcon V_2_RAMP external\n");
					ecv5546_tcon_bist_set(2, 4);
				} else
				...
				if (!strncasecmp(tmpbuf, "inter", 5)) {
					...
					ecv5546_tcon_bist_set(1, 5);
				} else if (!strncasecmp(tmpbuf, "exter", 5)) {
					pr_info("bist tcon V_3_ODDEVEN external\n");
					ecv5546_tcon_bist_set(2, 5);
				} else
				...
				if (!strncasecmp(tmpbuf, "inter", 5)) {
					...
					ecv5546_tcon_bist_set(1, 6);
				} else if (!strncasecmp(tmpbuf, "exter", 5)) {
					pr_info("bist tcon HV_1_CHECK external\n");
					ecv5546_tcon_bist_set(2, 6);
				} else
				...
				if (!strncasecmp(tmpbuf, "inter", 5)) {
					...
					ecv5546_tcon_bist_set(1, 7);
				} else if (!strncasecmp(tmpbuf, "exter", 5)) {
					pr_info("bist tcon HV_2_FRAME external\n");
					ecv5546_tcon_bist_set(2, 7);
				} else
				...
				if (!strncasecmp(tmpbuf, "inter", 5)) {
					...
					ecv5546_tcon_bist_set(1, 8);
				} else if (!strncasecmp(tmpbuf, "exter", 5)) {
					pr_info("bist tcon HV_3_MOIRE_A external\n");
					ecv5546_tcon_bist_set(2, 8);
				} else
				...
				if (!strncasecmp(tmpbuf, "inter", 5)) {
					...
					ecv5546_tcon_bist_set(1, 9);
				} else if (!strncasecmp(tmpbuf, "exter", 5)) {
					pr_info("bist tcon HV_4_MOIRE_B external\n");
					ecv5546_tcon_bist_set(2, 9);
				} else
				...
				if (!strncasecmp(tmpbuf, "inter", 5)) {
					...
					ecv5546_tcon_bist_set(1, 10);
				} else if (!strncasecmp(tmpbuf, "exter", 5)) {
					pr_info("bist tcon HV_5_CONTRAST external\n");
					ecv5546_tcon_bist_set(2, 10);
				} else
				...
				if (!strncasecmp(tmpbuf, "upd1", 4)) {
					...
					for (i = 0; i < 3; i++) {
						ecv5546_tcon_gamma_table_set(i+1, tmptable, tablesize);
						ecv5546_tcon_gamma_table_get(i+1, tmptable2, tablesize);
					...
					ecv5546_tcon_gamma_table_set(eCV5546_TCON_GM_UPDDEL_RGB_R, tmptable, tablesize);
					...
					ecv5546_tcon_gamma_table_set(eCV5546_TCON_GM_UPDDEL_RGB_G, tmptable, tablesize);
					...
					ecv5546_tcon_gamma_table_set(eCV5546_TCON_GM_UPDDEL_RGB_B, tmptable, tablesize);
				} else if (!strncasecmp(tmpbuf, "rr", 2)){
					pr_info("bist tcon gamma table read r channel\n");
					ecv5546_tcon_gamma_table_get(eCV5546_TCON_GM_UPDDEL_RGB_R, tmptable2, tablesize);
					pr_info("Output gamma R table:\n");
					print_hex_dump(KERN_INFO, " ", DUMP_PREFIX_OFFSET, 16, 1,
						tmptable2, tablesize*sizeof(u16), true);
				} else if (!strncasecmp(tmpbuf, "rg", 2)){
					pr_info("bist tcon gamma table read g channel\n");
					ecv5546_tcon_gamma_table_get(eCV5546_TCON_GM_UPDDEL_RGB_G, tmptable2, tablesize);
					pr_info("Output gamma G table:\n");
					print_hex_dump(KERN_INFO, " ", DUMP_PREFIX_OFFSET, 16, 1,
						tmptable2, tablesize*sizeof(u16), true);
				} else if (!strncasecmp(tmpbuf, "rb", 2)){
					pr_info("bist tcon gamma table read b channel\n");
					ecv5546_tcon_gamma_table_get(eCV5546_TCON_GM_UPDDEL_RGB_B, tmptable2, tablesize);
					...
					ecv5546_tcon_gamma_table_enable(1);
				} else if (!strncasecmp(tmpbuf, "off", 3)) {
					pr_info("bist tcon gamma table disable.\n");
					ecv5546_tcon_gamma_table_enable(0);
				} else
				...
				if (!strncasecmp(tmpbuf, "selftest", 8)) {
					...
					if (strstr(tmpbuf, "R"))
						channel_sel |= eCV5546_TCON_RGB_ADJ_CHANNEL_R_EN;
					if (strstr(tmpbuf, "G"))
						channel_sel |= eCV5546_TCON_RGB_ADJ_CHANNEL_G_EN;
					if (strstr(tmpbuf, "B"))
						channel_sel |= eCV5546_TCON_RGB_ADJ_CHANNEL_B_EN;
					...
					if (!channel_sel) {
						...
						ecv5546_tcon_rgb_adjust_selftest(channel_sel);
					}
				} else if (!strncasecmp(tmpbuf, "on", 2)) {
					tmpbuf = spmon_skipspace(tmpbuf + 2);
					if (strstr(tmpbuf, "R"))
					...
					if (!channel_sel) {
						...
						ecv5546_tcon_rgb_adjust_enable(channel_sel);
					}

				} else if (!strncasecmp(tmpbuf, "off", 3)) {
					ecv5546_tcon_rgb_adjust_enable(0);
				} else
				...
				if (!strncasecmp(tmpbuf, "set6bit", 7)) {
					...
					if (!strncasecmp(tmpbuf, "0", 1))
						mode = eCV5546_TCON_DITHER_6BIT_MODE_MATCHED;
					else
						mode = eCV5546_TCON_DITHER_6BIT_MODE_ROBUST;
					...
					ecv5546_tcon_enhanced_dither_6bit_set(mode, table_v_shift_en, table_h_shift_en);
				} else if (!strncasecmp(tmpbuf, "set8bit", 7)) {
					ecv5546_tcon_enhanced_dither_8bit_set();
				}else if (!strncasecmp(tmpbuf, "set", 3)) {
					u32 rgbc_sel = 0, method = 0, temporal_mode_en = 0, dot_mode = 0;
					tmpbuf = spmon_skipspace(tmpbuf + 3);
					if (!strncasecmp(tmpbuf, "0", 1))
						rgbc_sel = eCV5546_TCON_DITHER_RGBC_SEL_R;
					else
						rgbc_sel = eCV5546_TCON_DITHER_RGBC_SEL_RGB;

					tmpbuf = spmon_skipspace(tmpbuf + 1);
					if (!strncasecmp(tmpbuf, "0", 1))
						method = eCV5546_TCON_DITHER_INIT_MODE_METHOD2;
					else
						method = eCV5546_TCON_DITHER_INIT_MODE_METHOD1;
					...
					tmpbuf = spmon_skipspace(tmpbuf + 1);
					if (!strncasecmp(tmpbuf, "0", 1))
						dot_mode = eCV5546_TCON_DITHER_PANEL_DOT_MODE_1DOT;
					else if (!strncasecmp(tmpbuf, "1", 1))
						dot_mode = eCV5546_TCON_DITHER_PANEL_DOT_MODE_H2DOT;
					else if (!strncasecmp(tmpbuf, "2", 1))
						dot_mode = eCV5546_TCON_DITHER_PANEL_DOT_MODE_V2DOT;
					else if (!strncasecmp(tmpbuf, "3", 1))
						dot_mode = eCV5546_TCON_DITHER_PANEL_DOT_MODE_2DOT;
					else {
					...
					pr_info("DITHER set With RGBC[%s] METHOD[%d] TEMP[%d] DOT[%s].\n",
						...
						dot_mode == eCV5546_TCON_DITHER_PANEL_DOT_MODE_2DOT ? "2DOT":
						(dot_mode == eCV5546_TCON_DITHER_PANEL_DOT_MODE_V2DOT ?"V2DOT":
						(dot_mode == eCV5546_TCON_DITHER_PANEL_DOT_MODE_H2DOT?"H2DOT":"1DOT")));
					ecv5546_tcon_enhanced_dither_set(rgbc_sel, method, temporal_mode_en, dot_mode);
				} else if (!strncasecmp(tmpbuf, "on", 2)) {
					ecv5546_tcon_enhanced_dither_enable(1);
				} else if (!strncasecmp(tmpbuf, "off", 3)) {
					ecv5546_tcon_enhanced_dither_enable(0);
				} else
				...
				if (!strncasecmp(tmpbuf, "set", 3)) {
					...
					if (!strncasecmp(tmpbuf, "RGB", 3))
						channel_mode = eCV5546_TCON_BIT_SW_CHNL_RGB;
					else if (!strncasecmp(tmpbuf, "RBG", 3))
						channel_mode = eCV5546_TCON_BIT_SW_CHNL_RBG;
					else if (!strncasecmp(tmpbuf, "GBR", 3))
						channel_mode = eCV5546_TCON_BIT_SW_CHNL_GBR;
					else if (!strncasecmp(tmpbuf, "GRB", 3))
						channel_mode = eCV5546_TCON_BIT_SW_CHNL_GRB;
					else if (!strncasecmp(tmpbuf, "BGR", 3))
						channel_mode = eCV5546_TCON_BIT_SW_CHNL_BGR;
					else if (!strncasecmp(tmpbuf, "BRG", 3))
						channel_mode = eCV5546_TCON_BIT_SW_CHNL_BRG;
					else {
					...
					if (!strncasecmp(tmpbuf, "MSB", 3))
						bit_mode = eCV5546_TCON_BIT_SW_BIT_MSB;
					else if (!strncasecmp(tmpbuf, "LSB", 3))
						bit_mode = eCV5546_TCON_BIT_SW_BIT_LSB;
					else {
					...
					ecv5546_tcon_bitswap_set(bit_mode, channel_mode);
				} else if (!strncasecmp(tmpbuf, "on", 2)) {
					ecv5546_tcon_bitswap_enable(1);
				} else if (!strncasecmp(tmpbuf, "off", 3)) {
					ecv5546_tcon_bitswap_enable(0);
				} else
		...
		if (!strncasecmp(tmpbuf, "info", 4)) {
			...
			if (!strncasecmp(tmpbuf, "blend", 5)) {
				...
				ecv5546_dmix_layer_init(eCV5546_DMIX_L1, eCV5546_DMIX_VPP0, eCV5546_DMIX_OPACITY);
			} else
			...
			if (!strncasecmp(tmpbuf, "blend", 5)) {
				...
				ecv5546_dmix_layer_init(eCV5546_DMIX_L6, eCV5546_DMIX_OSD0, eCV5546_DMIX_OPACITY);
			} else
			...
			if (!strncasecmp(tmpbuf, "blend", 5)) {
				...
				ecv5546_dmix_layer_init(eCV5546_DMIX_L5, eCV5546_DMIX_OSD1, eCV5546_DMIX_OPACITY);
			} else
			...
			if (!strncasecmp(tmpbuf, "blend", 5)) {
				...
				ecv5546_dmix_layer_init(eCV5546_DMIX_L4, eCV5546_DMIX_OSD2, eCV5546_DMIX_OPACITY);
			} else
			...
			if (!strncasecmp(tmpbuf, "blend", 5)) {
				...
				ecv5546_dmix_layer_init(eCV5546_DMIX_L3, eCV5546_DMIX_OSD3, eCV5546_DMIX_OPACITY);
			} else
		...
		if (!strncasecmp(tmpbuf, "vpp", 3)) {
			...
			ecv5546_vpp_imgread_resolution_chk();
			ecv5546_vpp_vscl_resolution_chk();
			ecv5546_osd_resolution_chk();
			ecv5546_tgen_resolution_chk();
			ecv5546_tcon_resolution_chk();
			ecv5546_mipitx_resolution_chk();
			ecv5546_mipitx_pllclk_get();
			ecv5546_mipitx_txpll_get();
		} else if (!strncasecmp(tmpbuf, "timing", 6)) {
			pr_info("dump timing\n");
			ecv5546_tgen_timing_get();
			ecv5546_tcon_timing_get();
			ecv5546_mipitx_timing_get();
			ecv5546_mipitx_pllclk_get();
			ecv5546_mipitx_txpll_get();
		}
...
int eys3d_proc_open_show(struct seq_file *m, void *v)
{
	//pr_info("%s\n", __func__);
	//pr_info("eCV5546 DISPLAY DEBUG TEST\n");

	return 0;
}

int eys3d_proc_open(struct inode *inode, struct file *file)
{
	return single_open(file, eys3d_proc_open_show, NULL);
}

static ssize_t eys3d_proc_write(struct file *filp, const char __user *buffer,
...
const struct proc_ops sp_disp_proc_ops = {
	.proc_open = eys3d_proc_open,
	.proc_write = eys3d_proc_write,
	.proc_read = seq_read,
...
MODULE_PARM_DESC(debug, "eCV5546 display debug test");

/*
 * ecv5546 debug fs reference
 *
 ...
 *
 */
static int ecv5546_debug_dump_regs(struct sp_disp_device *disp_dev,
				  struct seq_file *m)
{
	pr_info("%s test\n", __func__);
...
static int ecv5546_dump_regs_show(struct seq_file *m, void *p)
{
	struct sp_disp_device *disp_dev = m->private;

	return ecv5546_debug_dump_regs(disp_dev, m);
}
DEFINE_SHOW_ATTRIBUTE(ecv5546_dump_regs);

static int ecv5546_debug_dump_vpp_regs(struct sp_disp_device *disp_dev,
				  struct seq_file *m)
{
	pr_info("%s test\n", __func__);
...
static int ecv5546_dump_vpp_regs_show(struct seq_file *m, void *p)
{
	struct sp_disp_device *disp_dev = m->private;

	return ecv5546_debug_dump_vpp_regs(disp_dev, m);
}
DEFINE_SHOW_ATTRIBUTE(ecv5546_dump_vpp_regs);

void ecv5546_debug_init(struct sp_disp_device *disp_dev)
{
	struct ecv5546_debug *debug = &disp_dev->debug;
	...
	debugfs_create_file("ecv5546_dump_regs", 0444, debug->debugfs_dir, disp_dev,
			    &ecv5546_dump_regs_fops);

	regs_dir = debugfs_create_dir("regs", debug->debugfs_dir);

	debugfs_create_file("vpp", 0444, regs_dir, disp_dev,
			    &ecv5546_dump_vpp_regs_fops);
...
static void ecv5546_dmix_luma_adjust_selftest(void)
{
	u8 tmpcptable[eCV5546_TCON_RGB_ADJ_CP_SIZE];
	u8 tmpcpsrctable[eCV5546_TCON_RGB_ADJ_CP_SIZE];
	u8 tmpcpsdttable[eCV5546_TCON_RGB_ADJ_CP_SIZE];
	u16 tmpslopetable[eCV5546_TCON_RGB_ADJ_SLOPE_SIZE];
	u16 tmpslopetable2[eCV5546_TCON_RGB_ADJ_SLOPE_SIZE];
	int i,j;
	...
	for (i = 0; i < 0xff; i+=3) {
		...
		for (j = 0; j < eCV5546_TCON_RGB_ADJ_CP_SIZE; j++) {
			tmpcptable[j] = (i + j)%0xff;
		}
		#endif
		for (j = 0; j < eCV5546_TCON_RGB_ADJ_SLOPE_SIZE; j++) {
		...
		ecv5546_dmix_color_adj_luma_cp_set( tmpcptable, tmpcptable, eCV5546_TCON_RGB_ADJ_CP_SIZE);
		#else
		pr_info(" DMIX Luma Adjustment SELFTEST slope[%04x %04x %04x %04x].\n",
			tmpslopetable[0], tmpslopetable[1], tmpslopetable[2], tmpslopetable[3]);
		#endif
		ecv5546_dmix_color_adj_luma_slope_set( tmpslopetable, eCV5546_TCON_RGB_ADJ_SLOPE_SIZE);
		ecv5546_dmix_color_adj_onoff(1);
		msleep(300);
		#if DEBUG_DMIX_ADJUST_LUMA_CP_EN
		ecv5546_dmix_color_adj_luma_cp_get( tmpcpsrctable, tmpcpsdttable, eCV5546_TCON_RGB_ADJ_CP_SIZE);
		if (memcmp(tmpcptable, tmpcpsrctable, eCV5546_TCON_RGB_ADJ_CP_SIZE)) {
			pr_info(" DMIX Luma Adjust control point source update fail.\n");
			pr_info("Input:\n");
			print_hex_dump(KERN_INFO, "", DUMP_PREFIX_OFFSET, 16, 1,
				tmpcptable, eCV5546_TCON_RGB_ADJ_CP_SIZE, false);
			pr_info("Output:\n");
			print_hex_dump(KERN_INFO, "", DUMP_PREFIX_OFFSET, 16, 1,
				tmpcpsrctable, eCV5546_TCON_RGB_ADJ_CP_SIZE, false);
			break;
		}
		if (memcmp(tmpcptable, tmpcpsdttable, eCV5546_TCON_RGB_ADJ_CP_SIZE)) {
			...
			pr_info("Output:\n");
			print_hex_dump(KERN_INFO, "", DUMP_PREFIX_OFFSET, 16, 1,
				tmpcpsdttable, eCV5546_TCON_RGB_ADJ_CP_SIZE, false);
			break;
		}
		#endif
		ecv5546_dmix_color_adj_luma_slope_get(tmpslopetable2, eCV5546_TCON_RGB_ADJ_SLOPE_SIZE);
		if (memcmp(tmpslopetable, tmpslopetable2, eCV5546_TCON_RGB_ADJ_SLOPE_SIZE)) {
			pr_info(" DMIX Luma Adjust slope update fail.\n");
			pr_info("Input:\n");
			print_hex_dump(KERN_INFO, "", DUMP_PREFIX_OFFSET, 16, 1,
				tmpslopetable, eCV5546_TCON_RGB_ADJ_SLOPE_SIZE, false);
			pr_info("Output:\n");
			print_hex_dump(KERN_INFO, "", DUMP_PREFIX_OFFSET, 16, 1,
				tmpslopetable2, eCV5546_TCON_RGB_ADJ_SLOPE_SIZE, false);
			break;
		}
	}
...
static void ecv5546_dmix_chroma_adjust_selftest(void)
{
	...
	for (i = 0; i < 3; i++) {
		...
		for (j = 0; j < 0x3ff; j+=8) {
			...
			ecv5546_dmix_color_adj_crma_set( satcos, satcos);
			msleep(300);
			ecv5546_dmix_color_adj_crma_get( &tmpsatcos, &tmpsatsin);
...
static void ecv5546_tcon_rgb_adjust_selftest(u32 channel_sel)
{
	...
	pr_info(" RGB Adjustment SELFTEST with %s%s%s Channel.\n",
		(channel_sel & eCV5546_TCON_RGB_ADJ_CHANNEL_R_EN) ? "R" : "",
		(channel_sel & eCV5546_TCON_RGB_ADJ_CHANNEL_G_EN) ? "G" : "",
		(channel_sel & eCV5546_TCON_RGB_ADJ_CHANNEL_B_EN) ? "B" : "");
	...
	for (i = 0; i < 0xff; i+=3) {
		...
		pr_info(" RGB Adjustment SELFTEST cpsrc[%02x %02x %02x], cpsdt[%02x %02x %02x], slope[%04x %04x %04x %04x].\n",
			...
			tmpcptable[0], tmpcptable[1], tmpcptable[2],
			tmpslopetable[0], tmpslopetable[1], tmpslopetable[2], tmpslopetable[3]);
		ecv5546_tcon_rgb_adjust_cp_set(eCV5546_TCON_RGB_ADJ_CHANNEL_R_EN, tmpcptable, tmpcptable, eCV5546_TCON_RGB_ADJ_CP_SIZE);
		ecv5546_tcon_rgb_adjust_cp_set(eCV5546_TCON_RGB_ADJ_CHANNEL_G_EN, tmpcptable, tmpcptable, eCV5546_TCON_RGB_ADJ_CP_SIZE);
		ecv5546_tcon_rgb_adjust_cp_set(eCV5546_TCON_RGB_ADJ_CHANNEL_B_EN, tmpcptable, tmpcptable, eCV5546_TCON_RGB_ADJ_CP_SIZE);
		ecv5546_tcon_rgb_adjust_slope_set(eCV5546_TCON_RGB_ADJ_CHANNEL_R_EN, tmpslopetable, eCV5546_TCON_RGB_ADJ_SLOPE_SIZE);
		ecv5546_tcon_rgb_adjust_slope_set(eCV5546_TCON_RGB_ADJ_CHANNEL_G_EN, tmpslopetable, eCV5546_TCON_RGB_ADJ_SLOPE_SIZE);
		ecv5546_tcon_rgb_adjust_slope_set(eCV5546_TCON_RGB_ADJ_CHANNEL_B_EN, tmpslopetable, eCV5546_TCON_RGB_ADJ_SLOPE_SIZE);
		//ecv5546_tcon_rgb_adjust_enable(eCV5546_TCON_RGB_ADJ_CHANNEL_R_EN|eCV5546_TCON_RGB_ADJ_CHANNEL_G_EN|eCV5546_TCON_RGB_ADJ_CHANNEL_B_EN);
		ecv5546_tcon_rgb_adjust_enable(channel_sel);
		//udelay(1000);
		msleep(300);
		ecv5546_tcon_rgb_adjust_cp_get(eCV5546_TCON_RGB_ADJ_CHANNEL_R_EN, tmpcpsrctable, tmpcpsdttable, eCV5546_TCON_RGB_ADJ_CP_SIZE);
		if (memcmp(tmpcptable, tmpcpsrctable, eCV5546_TCON_RGB_ADJ_CP_SIZE)) {
		...
		ecv5546_tcon_rgb_adjust_cp_get(eCV5546_TCON_RGB_ADJ_CHANNEL_G_EN, tmpcpsrctable, tmpcpsdttable, eCV5546_TCON_RGB_ADJ_CP_SIZE);
		if (memcmp(tmpcptable, tmpcpsrctable, eCV5546_TCON_RGB_ADJ_CP_SIZE)) {
		...
		ecv5546_tcon_rgb_adjust_cp_get(eCV5546_TCON_RGB_ADJ_CHANNEL_B_EN, tmpcpsrctable, tmpcpsdttable, eCV5546_TCON_RGB_ADJ_CP_SIZE);
		if (memcmp(tmpcptable, tmpcpsrctable, eCV5546_TCON_RGB_ADJ_CP_SIZE)) {
		...
		ecv5546_tcon_rgb_adjust_slope_get(eCV5546_TCON_RGB_ADJ_CHANNEL_R_EN, tmpslopetable2, eCV5546_TCON_RGB_ADJ_SLOPE_SIZE);
		if (memcmp(tmpslopetable, tmpslopetable2, eCV5546_TCON_RGB_ADJ_SLOPE_SIZE)) {
		...
		ecv5546_tcon_rgb_adjust_slope_get(eCV5546_TCON_RGB_ADJ_CHANNEL_G_EN, tmpslopetable2, eCV5546_TCON_RGB_ADJ_SLOPE_SIZE);
		if (memcmp(tmpslopetable, tmpslopetable2, eCV5546_TCON_RGB_ADJ_SLOPE_SIZE)) {
			pr_info(" RGB Adjust slope G update fail.\n");
			pr_info("Input:\n");
			print_hex_dump(KERN_INFO, "", DUMP_PREFIX_OFFSET, 16, 1,
				tmpslopetable, eCV5546_TCON_RGB_ADJ_CP_SIZE, false);
			pr_info("Output:\n");
			print_hex_dump(KERN_INFO, "", DUMP_PREFIX_OFFSET, 16, 1,
				tmpslopetable2, eCV5546_TCON_RGB_ADJ_CP_SIZE, false);
			break;
		}
		ecv5546_tcon_rgb_adjust_slope_get(eCV5546_TCON_RGB_ADJ_CHANNEL_B_EN, tmpslopetable2, eCV5546_TCON_RGB_ADJ_SLOPE_SIZE);
		if (memcmp(tmpslopetable, tmpslopetable2, eCV5546_TCON_RGB_ADJ_SLOPE_SIZE)) {
