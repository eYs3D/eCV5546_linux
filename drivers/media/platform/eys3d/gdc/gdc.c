...
static int gdc_parse_dt(struct platform_device *pdev, struct gdc_device *gdev)
{
	...
	ret = device_property_read_u32(&pdev->dev, "eys3d,max-config-size",
				       &gdev->max_config_size);
	if (ret) {
		dev_err(&pdev->dev, "No eys3d,max_config_size from DT\n");
		return ret;
	}
	...
	ret = device_property_read_u32(&pdev->dev, "eys3d,devnode-number",
				       &gdev->v4l2.devnode_number);
	if (ret) {
		dev_info(
			&pdev->dev,
			"No eys3d,devnode_number from DT, use defaut value(%d)\n",
			GDC_DEV_NODE_NUM);
...
MODULE_DESCRIPTION("eYs3D Geometric Distortion Correction");
