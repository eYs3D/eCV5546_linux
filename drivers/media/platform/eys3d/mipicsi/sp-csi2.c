...
static const struct csi2_format csi2_formats[] = {
	...
	{ .code = MEDIA_BUS_FMT_VYUY8_1X16, .datatype = 0x1e, .bpp = 16 },
	{ .code = MEDIA_BUS_FMT_YUYV8_1X16, .datatype = 0x1e, .bpp = 16 },
	{ .code = MEDIA_BUS_FMT_YVYU8_1X16, .datatype = 0x1e, .bpp = 16 },
...
static int csi2_parse_dt(struct csi2_dev *priv)
{
	...
	ret = of_property_read_u32(priv->dev->of_node, "eys3d,id", &id);
	if (ret) {
		dev_err(priv->dev, "%pOF: No eys3d,id property found\n",
			priv->dev->of_node);
		return ret;
	}

	if (id >= CSI_MAX_NUM) {
		dev_err(priv->dev, "%pOF: Invalid eys3d,id '%u'\n",
			priv->dev->of_node, id);
		ret = -EINVAL;
		return ret;
	}
...
static const struct csi2_info sp_csi2_info_ecv5546 = {
...
static const struct of_device_id sp_csi2_of_table[] = {
	{
		.compatible = "eys3d,ecv5546-mipicsi-rx",
		.data = &sp_csi2_info_ecv5546,
	},
	{ /* sentinel */ },
};
...
static int sp_csi2_suspend(struct device *dev)
{
...
static int sp_csi2_resume(struct device *dev)
{
	struct csi2_dev *priv = dev_get_drvdata(dev);
	int ret;
...
MODULE_DESCRIPTION("eYs3D MIPI CSI-2 receiver driver");
MODULE_LICENSE("GPL");
