...
/*
 *  HID driver for some eys3d "special" devices
 *
		...
		unsigned int *rsize)
{
	if (*rsize >= 112 && rdesc[104] == 0x26 && rdesc[105] == 0x80 &&
			rdesc[106] == 0x03) {
		hid_info(hdev, "fixing up eYs3D Wireless Desktop report descriptor\n");
...
static const struct hid_device_id sp_devices[] = {
	{ HID_USB_DEVICE(USB_VENDOR_ID_EYS3D, USB_DEVICE_ID_EYS3D_WDESKTOP) },
	{ }
};
