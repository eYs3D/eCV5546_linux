...
/**
 * struct sp_rproc_data - eys3d rproc private data
...
static void ecv5546_request_firmware_callback(const struct firmware *fw, void *context)
{
...
static int ecv5546_load_warmboot_firmware(struct rproc *rproc)
{
	int ret;

	ret = request_firmware_nowait(THIS_MODULE, FW_ACTION_UEVENT,
				      FIRMWARE_WARMBOOT_NAME, &rproc->dev, GFP_KERNEL,
				      rproc, ecv5546_request_firmware_callback);
	if (ret < 0)
...
static int sp_rproc_start(struct rproc *rproc)
{
	...
	ecv5546_load_warmboot_firmware(rproc);
...
#ifdef CONFIG_ARM_eCV5546_CPUFREQ
...
MODULE_AUTHOR("eys3d@eys3d.com");
MODULE_LICENSE("GPL v2");
MODULE_DESCRIPTION("eYs3D remote processor control driver");
