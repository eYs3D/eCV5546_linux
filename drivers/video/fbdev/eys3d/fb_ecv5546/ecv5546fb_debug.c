 ...
 *
 * linux/drivers/video/eys3d/fb_ecv5546/ecv5546fb_debug.c
 *
...
#include "ecv5546fb_main.h"

#define eCV5546_FB_LOAD_BMP
#define eCV5546_FB_LOAD_FONT
#define eCV5546_FB_DBG_CMD_LEN		256

#ifdef eCV5546_FB_LOAD_FONT
#include "ecv5546fb_font_8x8.h"
#include "ecv5546fb_font_8x16.h"
#include "ecv5546fb_font_16x32.h"
#endif
...
static void ecv5546_device_info(struct fb_info *fbinfo)
{
	struct ecv5546fb_device *fbdev =
		(struct ecv5546fb_device *)fbinfo->par;
...
void ecv5546_rgb2yuv(int argb, int *y, int *u, int *v)
{
	int r, g, b;
...
static void ecv5546_gen_colorbar_data(struct fb_info *info, char *ptr,
	int id, int x, int y)
{
	struct ecv5546fb_device *fbdev =
		(struct ecv5546fb_device *)info->par;
	...
	if (info->var.nonstd) {
		for (k = 0; k < sizeof(color) / sizeof(u32); ++k) {
			ecv5546_rgb2yuv(sp_color_bar[k], &yy, &uu, &vv);
			color[k] = (yy << 16) | (uu << 8) | (vv << 0);
			//pr_info("color[%d] = 0x%08x\n", k, color[k]);
		}
	} else {
		for (k = 0; k < sizeof(color) / sizeof(u32); ++k) {
			color[k] = ecv5546fb_chan_by_field(
				sp_color_bar[k] >> 24,
				&info->var.transp);
			color[k] |= ecv5546fb_chan_by_field(
				sp_color_bar[k] >> 16,
	...
	if (fbdev->color_mode == eCV5546_OSD_COLOR_MODE_8BPP) {
	...
	} else if (fbdev->color_mode == eCV5546_OSD_COLOR_MODE_YUY2) {
		if ((y == 0) || (y == (h - 1)) || (x == 0) || (x == (w - 1))) {
			idx = ((id & 0x1) ^ (x < (w >> 1)));
...
static void ecv5546_gen_colorbar(struct fb_info *info, char *ptr, int id)
{
	int i, j;
	u32 w, h;
	...
	for (i = 0; i < h; ++i) {
		for (j = 0; j < w; ++j) {
			ecv5546_gen_colorbar_data(info, ptr, id, j, i);
			ptr += info->var.bits_per_pixel >> 3;
		}
	}
}

static void ecv5546_fill_color(char *ptr, int size, int argb, struct fb_info *info)
{
	struct ecv5546fb_device *fbdev =
		(struct ecv5546fb_device *)info->par;
	...
	if (info->var.nonstd) {
		...
		ecv5546_rgb2yuv(argb, &y, &u, &v);
		color = (y << 16) | (u << 8) | (v << 0);
	} else {
		color = ecv5546fb_chan_by_field((argb >> 24) & 0xff,
				&info->var.transp);
		color |= ecv5546fb_chan_by_field((argb >> 16) & 0xff,
				&info->var.red);
		color |= ecv5546fb_chan_by_field((argb >> 8) & 0xff,
				&info->var.green);
		color |= ecv5546fb_chan_by_field((argb >> 0) & 0xff,
				&info->var.blue);
	}
...
#ifdef eCV5546_FB_LOAD_BMP
static void ecv5546_draw_bmpfile(char *filename,
		...
		int start_y)
{
	...
	struct ecv5546_bmp_header bmp = {0};
	struct file *fp = NULL;
	char *tmpbuf = NULL;
	//mm_segment_t oldfs;
	int ret;
	...
	if (!IS_ERR(fp)) {
		...
		readlen = sizeof(struct ecv5546_bmp_header) -
		...
		line_pitch = eCV5546_DISP_ALIGN((bmp.width * bpp), 4); 
...
#ifdef eCV5546_FB_LOAD_FONT
static void ecv5546_draw_font(unsigned char *fb_ptr, struct fb_info *fbinfo,
		int start_x, int start_y, int font_idx)
{
	...
	font_color = ecv5546fb_chan_by_field((sp_font_color >> 24) & 0xff,
		&fbinfo->var.transp);
	font_color |= ecv5546fb_chan_by_field((sp_font_color >> 16) & 0xff,
		&fbinfo->var.red);
	font_color |= ecv5546fb_chan_by_field((sp_font_color >> 8) & 0xff,
		&fbinfo->var.green);
	font_color |= ecv5546fb_chan_by_field((sp_font_color >> 0) & 0xff,
		&fbinfo->var.blue);
	/* re-arrange font back ground color by osd color_mode */
	bg_color = ecv5546fb_chan_by_field((sp_font_color_bg >> 24) & 0xff,
		&fbinfo->var.transp);
	bg_color |= ecv5546fb_chan_by_field((sp_font_color_bg >> 16) & 0xff,
		&fbinfo->var.red);
	bg_color |= ecv5546fb_chan_by_field((sp_font_color_bg >> 8) & 0xff,
		&fbinfo->var.green);
	bg_color |= ecv5546fb_chan_by_field((sp_font_color_bg >> 0) & 0xff,
		&fbinfo->var.blue);
	...
	for (y = 0; y < sp_font_height; y++) {
		...
		if ((sp_font_width == 8) && (sp_font_height == 8))
			/* in case of font 8x8 */
			ch = eCV5546_FB_FONT_ASCII_8X8[font_offset + y + 0];
		else if ((sp_font_width == 8) && (sp_font_height == 16))
			/* in case of font 8x16 */
			ch = eCV5546_FB_FONT_ASCII_8X16[font_offset + y + 0];
		else if ((sp_font_width == 16) && (sp_font_height == 32))
			/* in case of font 16x32 */
			ch = eCV5546_FB_FONT_ASCII_16X32[font_offset + y * 2 + 0] << 8 |
				eCV5546_FB_FONT_ASCII_16X32[font_offset + y * 2 + 1];
...
/*
 * Dump fb info
 *   echo "fbinfo" > /sys/module/ecv5546fb/parameter/debug
 *
 * Fill buf with colorbar
 *   echo "set bar" > /sys/module/ecv5546fb/parameter/debug
 * Fill buf with one color
 *   echo "set color 0xffff0000" > /sys/module/ecv5546fb/parameter/debug
 *   echo "set color 0xff00ff00" > /sys/module/ecv5546fb/parameter/debug
 *   echo "set color 0xff0000ff" > /sys/module/ecv5546fb/parameter/debug
 * Fill buf palette (range 0 ~ 255)
 *   echo "set pal 0 0xffff0000" > /sys/module/ecv5546fb/parameter/debug
 *   echo "set pal 255 0xffff0000" > /sys/module/ecv5546fb/parameter/debug
 * Change buf id (range 0 ~ 1)
 *   echo "set buf 0" > /sys/module/ecv5546fb/parameter/debug
 *   echo "set buf 1" > /sys/module/ecv5546fb/parameter/debug
 *
 * Show buf palette
 *   echo "get pal" > /sys/module/ecv5546fb/parameter/debug
 * Get buf id (range 0 ~ 1)
 *   echo "get buf" > /sys/module/ecv5546fb/parameter/debug
 *
 * Load bmp file
 *   echo "bmp 0 0 0 /usr/share/argb8888.bmp" > /sys/module/ecv5546fb/parameter/debug
 *     file path /usr/share/argb8888.bmp depends on your bmp file location.
 *     only support 24bit/32bit bmp file.
 *
 * Load font
 *   echo "font color 0xffff0000 0xff000000" > /sys/module/ecv5546fb/parameter/debug
 *      font_color, font_color_bg
 *		setting value : 0xaa-rr-gg-bb (aa=alpha, rr=RED, gg=Green, bb=BLUE)
 *   echo "font size 8 16" > /sys/module/ecv5546fb/parameter/debug
 *      font_width, font_height, support 8x8 8x16 16x32 ascii code
 *
 *   echo "font draw 0 10 20 test" > /sys/module/ecv5546fb/parameter/debug
 *      buf_id=0, x=10, y=20 and display string
 */
static void ecv5546_fb_dbg_cmd(char *str, struct fb_info *fbinfo)
{
	struct ecv5546fb_device *fbdev;
	...
	fbdev = (struct ecv5546fb_device *)fbinfo->par;
	...
	if (!strncasecmp(str, "fbinfo", 4)) {
		...
		if (!strncasecmp(str, "pal", 3)) {
			...
			//for (i = 0; i < (eCV5546_FB_PALETTE_LEN / sizeof(u32)); ++i) {
			//	if (!(i % 16))
			//		pr_info("palette %03d -- %03d\n", i, i + 15);
			//	pr_info(" 0x%08x\n", *(palette_ptr++));
			//}
			for (i = 0; i < (eCV5546_FB_PALETTE_LEN / sizeof(u32) / 4); i++) {
				if (!(i % 4))
					pr_info("palette %03d -- %03d\n", i * 4, i * 4 + 15);
		...
		if (!strncasecmp(str, "bar", 3)) {
			...
			ecv5546_fill_color(fbinfo->screen_base,
			...
			if (fbdev->color_mode == eCV5546_OSD_COLOR_MODE_8BPP) {
				...
				ret = ecv5546fb_swapbuf(fbinfo->var.yoffset / fbinfo->var.yres,
					buf_num);
				pr_info("update palette by swapbuf ret:%d\n", ret);
			}
			...
			if (fbdev->color_mode == eCV5546_OSD_COLOR_MODE_YUY2) {
				int r, g, b;
				int y, u, v;
			...
			if (index >= (eCV5546_FB_PALETTE_LEN / sizeof(u32))) {
				pr_err("index %d > %ld\n", index,
					eCV5546_FB_PALETTE_LEN / sizeof(u32));
				return;
			}

			palette[index] = ecv5546fb_chan_by_field((argb >> 24) & 0xff,
				&fbinfo->var.transp);
			palette[index] |= ecv5546fb_chan_by_field((argb >> 16) & 0xff,
				&fbinfo->var.red);
			palette[index] |= ecv5546fb_chan_by_field((argb >> 8) & 0xff,
				&fbinfo->var.green);
			palette[index] |= ecv5546fb_chan_by_field((argb >> 0) & 0xff,
				&fbinfo->var.blue);
			...
			ret = ecv5546fb_swapbuf(buf_id, fbdev->buf_num);
		...
		if ((fbdev->color_mode == eCV5546_OSD_COLOR_MODE_8BPP)
			|| (fbdev->color_mode == eCV5546_OSD_COLOR_MODE_YUY2)) {
			pr_err("color format unsupported.\n");
			return;
		}

		ecv5546_draw_bmpfile(str,
		...
		if (!strncasecmp(str, "color", 5)) {
			...
			for (i = 0; i < strlen(str); i++) {
				str_index = *(str + i);
				ecv5546_draw_font((unsigned char *)(fbinfo->screen_base
...
static int ecv5546_set_dbg_cmd(const char *val, const struct kernel_param *kp)
{
	ecv5546_fb_dbg_cmd(strstrip((char *)val), gsp_fbinfo);

	return 0;
}

static struct kernel_param_ops fb_debug_param_ops = {
	.set = ecv5546_set_dbg_cmd,
};
