 ...
 *
 * linux/drivers/video/eys3d/fb_ecv5546/ecv5546fb_main.c
 *
...
int ecv5546fb_swapbuf(u32 buf_id, int buf_max)
{
	...
	ret = ecv5546_osd_region_buf_fetch_done(buf_id);

	return ret;
}

static int ecv5546fb_pan_display(struct fb_var_screeninfo *var,
...
static int ecv5546fb_ioctl(struct fb_info *fbinfo,
		unsigned int cmd,
		unsigned long arg)
{
	switch (cmd) {
	case FBIO_WAITFORVSYNC:
		ecv5546_osd_region_wait_sync();
		break;
	}
...
static int ecv5546fb_setcmap(struct fb_cmap *cmap, struct fb_info *info)
{
	struct ecv5546fb_device *fbdev =
	...
	for (i = cmap->start; (i < (cmap->start + cmap->len)) ||
		(i < (eCV5546_FB_PALETTE_LEN / sizeof(u32))); ++i) {
		if (transp)
			trans = *(transp++);

		if (fbdev->color_mode == eCV5546_OSD_COLOR_MODE_ARGB8888) {
			palette[i] = 0xff000000;
			palette[i] |= ecv5546fb_chan_by_field((unsigned char)*(red++),
					&info->var.red);
			palette[i] |= ecv5546fb_chan_by_field((unsigned char)*(green++),
					&info->var.green);
			palette[i] |= ecv5546fb_chan_by_field((unsigned char)*(blue++),
					&info->var.blue);
		} else if (fbdev->color_mode == eCV5546_OSD_COLOR_MODE_RGBA8888) {
			palette[i] = ecv5546fb_chan_by_field((unsigned char)*(red++),
					&info->var.red);
			palette[i] |= ecv5546fb_chan_by_field((unsigned char)*(green++),
					&info->var.green);
		...
		} else if ((fbdev->color_mode == eCV5546_OSD_COLOR_MODE_8BPP) ||
			(fbdev->color_mode == eCV5546_OSD_COLOR_MODE_RGB565)) {
			palette[i] = ecv5546fb_chan_by_field((unsigned char)trans,
					&info->var.transp);
			palette[i] |= ecv5546fb_chan_by_field((unsigned char)*(red++),
					&info->var.red);
			palette[i] |= ecv5546fb_chan_by_field((unsigned char)*(green++),
			...
			palette[i] |= ecv5546fb_chan_by_field((unsigned char)*(blue++),
					&info->var.blue);
		} else if (fbdev->color_mode == eCV5546_OSD_COLOR_MODE_ARGB4444) {
			...
			palette[i] |= ecv5546fb_chan_by_field((unsigned char)*(blue++),
					&info->var.blue);
		} else if (fbdev->color_mode == eCV5546_OSD_COLOR_MODE_RGBA4444) {
			palette[i] = ecv5546fb_chan_by_field((unsigned char)*(red++),
					&info->var.red);
			palette[i] |= ecv5546fb_chan_by_field((unsigned char)*(green++),
					&info->var.green);
		...
		} else if (fbdev->color_mode == eCV5546_OSD_COLOR_MODE_ARGB1555) {
...
static struct fb_ops framebuffer_ops = {
	.owner			= THIS_MODULE,
	.fb_pan_display		= ecv5546fb_pan_display,
	.fb_setcmap		= ecv5546fb_setcmap,
	.fb_ioctl		= ecv5546fb_ioctl,
	.fb_fillrect		= cfb_fillrect,
	.fb_copyarea		= cfb_copyarea,
	.fb_imageblit		= cfb_imageblit,
};

static int ecv5546fb_create_device(struct platform_device *pdev,
		struct ecv5546fb_info *sp_fbinfo)
{
	...
	fbdev->buf_size_page = eCV5546_DISP_ALIGN((fbdev->width
			* fbdev->height
			* (sp_fbinfo->cmod.bits_per_pixel >> 3)),
			sp_fbinfo->buf_align);
	...
	/* alloc ecv5546fb palette */
	fbdev->pal = kzalloc(eCV5546_FB_PALETTE_LEN, GFP_KERNEL);
	if (!fbdev->pal)
		goto err_fb_init;

	/* assign fb info for ecv5546fb */
	...
	/* update ecv5546fb_info for osd region */
...
static int ecv5546fb_probe(struct platform_device *pdev)
{
	struct ecv5546fb_device *fbdev;
	struct ecv5546fb_info *sp_fbinfo;
	int ret;
	...
	ret = ecv5546_osd_get_fbinfo(sp_fbinfo);
	if (ret) {
		kfree(sp_fbinfo);
	...
	pr_info("fbdev: fb create %dx%d, cmod %d(%s)\n",
		...
		sp_fbinfo->cmod.name);
	ret = ecv5546fb_create_device(pdev, sp_fbinfo);
	if (ret)
		goto err_fb_probe;

	ret = ecv5546_osd_region_create(sp_fbinfo);
	if (ret)
		goto err_fb_probe;

	fbdev = (struct ecv5546fb_device *)gsp_fbinfo->par;

	fbdev->osd_handle = sp_fbinfo->handle;

	platform_set_drvdata(pdev, fbdev);

	ecv5546_osd_region_irq_enable();
...
err_fb_probe:
	...
	ecv5546_osd_region_irq_disable();
	ecv5546_osd_region_release();

	return ret;
}

static int ecv5546fb_remove(struct platform_device *pdev)
{
	struct ecv5546fb_device *fbdev = platform_get_drvdata(pdev);
...
static const struct of_device_id ecv5546fb_dt_ids[] = {
	{ .compatible = "eys3d,ecv5546-fb", },
	{}
};
MODULE_DEVICE_TABLE(of, ecv5546fb_dt_ids);

static struct platform_driver ecv5546fb_driver = {
	.probe		= ecv5546fb_probe,
	.remove		= ecv5546fb_remove,
	.driver		= {
		.name	= "ecv5546-fb",
		.of_match_table = ecv5546fb_dt_ids,
	},
};
module_platform_driver(ecv5546fb_driver);

MODULE_DESCRIPTION("eCV5546 Framebuffer Driver");
