...
static int eys3d_parse_cfg_partitions(struct mtd_info *master,
					const struct mtd_partition **pparts,
					struct mtd_part_parser_data *data)
{
	int ret = 0;
	struct mtd_partition *eys3d_parts;

	eys3d_parts = kzalloc(sizeof(*eys3d_parts) * 2, GFP_KERNEL);
	if (!eys3d_parts)
		return -ENOMEM;
	eys3d_parts[0].name = "Linux";
	eys3d_parts[0].offset = 0;
	eys3d_parts[0].size = master->size>>1;
	eys3d_parts[0].mask_flags = MTD_NO_ERASE;

	eys3d_parts[1].name = "User";
	eys3d_parts[1].offset = eys3d_parts[0].offset+eys3d_parts[0].size;
	eys3d_parts[1].size = master->size>>1;
	eys3d_parts[1].mask_flags = MTD_WRITEABLE;
...
static struct mtd_part_parser eys3d_nand_parser = {
	.parse_fn = eys3d_parse_cfg_partitions,
	.name = "eys3d_part",
};
...
static int sp_spinand_probe(struct platform_device *pdev)
{
	...
	register_mtd_parser(&eys3d_nand_parser);
	...
	SPINAND_LOGI("====eYs3D SPI-NAND Driver====\n");
...
static int sp_spinand_remove(struct platform_device *pdev)
{
	...
	deregister_mtd_parser(&eys3d_nand_parser);
...
static const struct of_device_id eys3d_nand_of_match[] = {
	{ .compatible = "eys3d,ecv5546-spi-nand" },
	{},
};
MODULE_DEVICE_TABLE(of, eys3d_nand_of_match);
...
static struct platform_driver sp_spinand_driver = {
	...
	.driver = {
		.name = "eys3d,spinand",
		.owner = THIS_MODULE,
		.of_match_table = eys3d_nand_of_match,
...
MODULE_DESCRIPTION("eYs3D SPI-NAND flash controller driver");
