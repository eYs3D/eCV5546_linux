...
#define PAGE_ACCESS_MODE        BIT(29)
#define PAGE_ACCESSMODE_DISABLE (~(1 << 29))
#define AUTO_SPI_WEL_EN         BIT(19)
...
#define USER_DEFINED_READ_EN    BIT(20)
#define PREFETCH_ENABLE         BIT(22)
#define PREFETCH_DISABLE        (~(1 << 22))
#define PIO_TRIGGER             BIT(21)
#define AUTO_RDSR_EN            BIT(18)
#define DMA_TRIGGER             BIT(17)

//spi_cfg0
#define ENHANCE_DATA(x)         ((x) << 24)
#define CLEAR_ENHANCE_DATA      (~(0xff << 24))
#define DATA64_EN               BIT(20)
#define DATA64_DIS              ~(1 << 20)
#define SPI_TRS_MODE            BIT(19)
...
{
	static int pre_pllh = 0xffffffff;
	...
	value = (0x2 << 22) | (0x16 << 16) | pspi->rw_timing_sel;
		//2 = 200(MHz) * 10 / 1000 (minimum val = 3), 0x16 = 105 * 200(MHz) / 1000.
...
#if (SP_SPINOR_DMA)
	dma_free_coherent(&pdev->dev, PAGE_ALIGN(pspi->buff.size), pspi->buff.virt,
			  pspi->buff.phys);
#endif
	mutex_destroy(&pspi->lock);
...
static const struct of_device_id sp_spi_nor_ids[] = {
	{.compatible = "eys3d,ecv5546-spi-nor"},
	{}
};
...
MODULE_AUTHOR("Wells Lu <eys3d@eys3d.com>");
MODULE_DESCRIPTION("eYs3D spi nor driver");
