...
#if defined(SUPPORT_GPIO_AO_INT)
	if (of_property_read_u32(np, "eys3d,ao-pin-prescale",
				 &pc->gpio_ao_int_prescale) == 0) {
		writel(pc->gpio_ao_int_prescale & 0xfffff,
		       pc->gpio_ao_int_regs_base); // PRESCALE
	}

	if (of_property_read_u32(np, "eys3d,ao-pin-debounce",
				      ...
				      struct device_node *np_config)
{
	...
	ret = of_property_read_string(np_config, "eys3d,ms-dvio-group-0",
				      &ms_val);
	if (!ret) {
	...
	ret = of_property_read_string(np_config, "eys3d,ms-dvio-group-1",
				      &ms_val);
	if (!ret) {
		if (!strcmp(ms_val, "3V0"))
			value = 0;
	...
	ret = of_property_read_string(np_config, "eys3d,ms-dvio-ao-group-0",
				      &ms_val);
	if (!ret) {
		if (!strcmp(ms_val, "3V0"))
			value = 0;
	...
	ret = of_property_read_string(np_config, "eys3d,ms-dvio-ao-group-1",
				      &ms_val);
	if (!ret) {
		if (!strcmp(ms_val, "3V0"))
			value = 0;
	...
	ret = of_property_read_string(np_config, "eys3d,ms-dvio-ao-group-2",
				      &ms_val);
	if (!ret) {
		if (!strcmp(ms_val, "3V0"))
			value = 0;
