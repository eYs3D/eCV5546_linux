 ...
 * In eYs3D bus communication between memory bus and peripheral bus with
...
static u32 eys3d_tx_buf_not_full(struct uart_port *port)
{
...
static unsigned int eys3d_tx_empty(struct uart_port *port)
{
	unsigned int lsr = readl(port->membase + SUP_UART_LSR);
...
static void eys3d_set_mctrl(struct uart_port *port, unsigned int mctrl)
{
...
static unsigned int eys3d_get_mctrl(struct uart_port *port)
{
...
static void eys3d_stop_tx(struct uart_port *port)
{
	unsigned int isc;
...
static void eys3d_start_tx(struct uart_port *port)
{
	unsigned int isc;
...
static void eys3d_stop_rx(struct uart_port *port)
{
	unsigned int isc;
...
static void eys3d_break_ctl(struct uart_port *port, int ctl)
{
	unsigned long flags;
	unsigned int lcr;
...
static void transmit_chars(struct uart_port *port)
{
	...
	do {
		...
		if (uart_circ_empty(xmit))
			break;
	} while (eys3d_tx_buf_not_full(port));
...
static int eys3d_startup(struct uart_port *port)
{
	unsigned long flags;
	unsigned int isc = 0;
	int ret;

	ret = request_irq(port->irq, eys3d_uart_irq, 0, "eys3d_uart", port);
	if (ret)
		return ret;
...
static void eys3d_shutdown(struct uart_port *port)
{
	unsigned long flags;
...
static void eys3d_set_termios(struct uart_port *port,
...
static void eys3d_set_ldisc(struct uart_port *port, struct ktermios *termios)
{
...
static const char *eys3d_type(struct uart_port *port)
{
	return port->type == PORT_EYS3D ? "eys3d_uart" : NULL;
}

static void eys3d_config_port(struct uart_port *port, int type)
{
	if (type & UART_CONFIG_TYPE)
		port->type = PORT_EYS3D;
}

static int eys3d_verify_port(struct uart_port *port, struct serial_struct *ser)
{
	if (ser->type != PORT_UNKNOWN && ser->type != PORT_EYS3D)
		return -EINVAL;

	return 0;
}

#if defined(CONFIG_SERIAL_EYS3D_CONSOLE) || defined(CONFIG_CONSOLE_POLL)
...
static void eys3d_poll_put_char(struct uart_port *port, unsigned char data)
{
	wait_for_xmitr(port);
	sp_uart_put_char(port, data);
}

static int eys3d_poll_get_char(struct uart_port *port)
{
	unsigned int lsr = readl(port->membase + SUP_UART_LSR);
...
static const struct uart_ops eys3d_uart_ops = {
	.tx_empty	= eys3d_tx_empty,
	.set_mctrl	= eys3d_set_mctrl,
	.get_mctrl	= eys3d_get_mctrl,
	.stop_tx	= eys3d_stop_tx,
	.start_tx	= eys3d_start_tx,
	.stop_rx	= eys3d_stop_rx,
	.break_ctl	= eys3d_break_ctl,
	.startup	= eys3d_startup,
	.shutdown	= eys3d_shutdown,
	.set_termios	= eys3d_set_termios,
	.set_ldisc	= eys3d_set_ldisc,
	.type		= eys3d_type,
	.config_port	= eys3d_config_port,
	.verify_port	= eys3d_verify_port,
#ifdef CONFIG_CONSOLE_POLL
	.poll_put_char	= eys3d_poll_put_char,
	.poll_get_char	= eys3d_poll_get_char,
#endif
};

#ifdef CONFIG_SERIAL_EYS3D_CONSOLE
static struct eys3d_uart_port *eys3d_console_ports[SUP_UART_NR];

static void eys3d_uart_console_putchar(struct uart_port *port,
					 unsigned char ch)
{
	wait_for_xmitr(port);
				  ...
				  unsigned int count)
{
	...
	if (eys3d_console_ports[co->index]->port.sysrq)
		locked = 0;
	else if (oops_in_progress)
		locked = spin_trylock(&eys3d_console_ports[co->index]->port.lock);
	else
		spin_lock(&eys3d_console_ports[co->index]->port.lock);

	uart_console_write(&eys3d_console_ports[co->index]->port, s, count,
			   eys3d_uart_console_putchar);

	if (locked)
		spin_unlock(&eys3d_console_ports[co->index]->port.lock);
...
static int __init eys3d_console_setup(struct console *co, char *options)
{
	...
	sup = eys3d_console_ports[co->index];
	if (!sup)
		return -ENODEV;
...
static struct console eys3d_uart_console = {
	.name		= "ttySUP",
...
#define	SERIAL_EYS3D_CONSOLE	(&eys3d_uart_console)
#else
#define	SERIAL_EYS3D_CONSOLE	NULL
#endif
...
static struct uart_driver eys3d_uart_driver = {
	...
	.minor		= 64,
	.nr		= SUP_UART_NR,
	.cons		= SERIAL_EYS3D_CONSOLE,
};

static void eys3d_uart_disable_unprepare(void *data)
{
	clk_disable_unprepare(data);
}

static void eys3d_uart_reset_control_assert(void *data)
{
	reset_control_assert(data);
}

static int eys3d_uart_probe(struct platform_device *pdev)
{
	struct eys3d_uart_port *sup;
	struct uart_port *port;
	struct resource *res;
	...
	ret = devm_add_action_or_reset(&pdev->dev, eys3d_uart_disable_unprepare, sup->clk);
	if (ret)
		return ret;
	...
	ret = devm_add_action_or_reset(&pdev->dev, eys3d_uart_reset_control_assert, sup->rstc);
	if (ret)
		return ret;

#ifdef CONFIG_SERIAL_EYS3D_CONSOLE
	eys3d_console_ports[sup->port.line] = sup;
#endif

	platform_set_drvdata(pdev, &sup->port);

	ret = uart_add_one_port(&eys3d_uart_driver, &sup->port);
#ifdef CONFIG_SERIAL_EYS3D_CONSOLE
	if (ret)
		eys3d_console_ports[sup->port.line] = NULL;
#endif

	return ret;
}

static int eys3d_uart_remove(struct platform_device *pdev)
{
	struct eys3d_uart_port *sup = platform_get_drvdata(pdev);

	uart_remove_one_port(&eys3d_uart_driver, &sup->port);

	return 0;
}

static int __maybe_unused eys3d_uart_suspend(struct device *dev)
{
	struct eys3d_uart_port *sup = dev_get_drvdata(dev);

	if (!uart_console(&sup->port))
		uart_suspend_port(&eys3d_uart_driver, &sup->port);

	return 0;
}

static int __maybe_unused eys3d_uart_resume(struct device *dev)
{
	struct eys3d_uart_port *sup = dev_get_drvdata(dev);

	if (!uart_console(&sup->port))
		uart_resume_port(&eys3d_uart_driver, &sup->port);

	return 0;
}

static const struct dev_pm_ops eys3d_uart_pm_ops = {
	SET_SYSTEM_SLEEP_PM_OPS(eys3d_uart_suspend, eys3d_uart_resume)
};

static const struct of_device_id sp_uart_of_match[] = {
	{ .compatible = "eys3d,sp7021-uart" },
	{}
};
MODULE_DEVICE_TABLE(of, sp_uart_of_match);

static struct platform_driver eys3d_uart_platform_driver = {
	.probe		= eys3d_uart_probe,
	.remove		= eys3d_uart_remove,
	.driver = {
		.name	= "eys3d_uart",
...
static void __exit eys3d_uart_exit(void)
{
	platform_driver_unregister(&eys3d_uart_platform_driver);
	uart_unregister_driver(&eys3d_uart_driver);
}
module_exit(eys3d_uart_exit);
...
static void eys3d_uart_early_write(struct console *con, const char *s, unsigned int n)
{
...
static int __init
eys3d_uart_early_setup(struct earlycon_device *dev, const char *opt)
{
	if (!(dev->port.membase || dev->port.iobase))
		return -ENODEV;

	dev->con->write = eys3d_uart_early_write;

	return 0;
}
OF_EARLYCON_DECLARE(eys3d_uart, "eys3d,sp7021-uart", eys3d_uart_early_setup);
#endif
