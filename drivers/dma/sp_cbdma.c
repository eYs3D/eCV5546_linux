				  ...
				  struct device_node *node, int chan_id)
{
	...
	if (of_device_is_compatible(node, "eys3d,cb-vdma-mm2s-channel") ||
	    of_device_is_compatible(node, "eys3d,cb-dma-mm2s-channel") ||
	    of_device_is_compatible(node, "eys3d,cb-cdma-channel")) {
	    // Change direction from DMA_MEM_TO_DEV to DMA_MEM_TO_MEM for cdma
		if (of_device_is_compatible(node, "eys3d,cb-cdma-channel"))
		...
		chan->ctrl_offset = SP_CBDMA_MM2S_CTRL_OFFSET;
					   ...
					   "eys3d,cb-vdma-s2mm-channel") ||
		   of_device_is_compatible(node,
					   "eys3d,cb-dma-s2mm-channel")) {
...
static const struct of_device_id sp_cbdma_of_ids[] = {
	{ .compatible = "eys3d,cb-dma", .data = &dma_config },
	{ .compatible = "eys3d,cb-cdma", .data = &cdma_config },
	{ /* sentinel */ }
};
...
static int sp_cbdma_probe(struct platform_device *pdev)
{
	...
	err = of_property_read_u32(node, "eys3d,addrwidth", &addr_width);
	if (err < 0)
	...
	dev_info(&pdev->dev, "eYs3D CB DMA engine driver, %d channels\n", xdev->nr_channels);
...
MODULE_DESCRIPTION("eYs3D CB-DMA controller driver");
