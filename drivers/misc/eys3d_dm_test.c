...
/*
 * Dummy Master device driver for EYS3D eCV5546 SoC
 *
 *  <dump help info>
 *    echo > /sys/module/eys3d_dm_test/parameters/test
 *  <dump dm reg info>
 *    echo 10 0 0 > /sys/module/eys3d_dm_test/parameters/test
 *  <dump set parameter info>
 *    echo 11 0 0 > /sys/module/eys3d_dm_test/parameters/test
 *
 *  <set address>
 *    echo 20 aa bb > /sys/module/eys3d_dm_test/parameters/test
 *  <set length and op_mode>
 *    echo 21 cc dd > /sys/module/eys3d_dm_test/parameters/test
 *
 *  <run dummymaster0>
 *    echo 1 0 0 > /sys/module/eys3d_dm_test/parameters/test
 *  <run dummymaster1>
 *    echo 0 1 0 > /sys/module/eys3d_dm_test/parameters/test
 *  <run dummymaster2>
 *    echo 0 0 1 > /sys/module/eys3d_dm_test/parameters/test
 *
 *  <test process>
...
#define eCV5546_SYSTEM_CLOCK 600000 /* 600 MHz*/
#define eCV5546_DRAM_CLOCK 1600000 /* 1600 MHz*/
#define eCV5546_MAX_BANDWIDTH_DM0 (eCV5546_DRAM_CLOCK * 8)
#define eCV5546_MAX_BANDWIDTH_DM1 (eCV5546_DRAM_CLOCK * 8 / 2)
...
u32 target_bandwidth[11] = {
	(eCV5546_DRAM_CLOCK * 8/1000) * 1 / 10, //Target BW 10%
	(eCV5546_DRAM_CLOCK * 8/1000) * 2 / 10, //Target BW 20%
	(eCV5546_DRAM_CLOCK * 8/1000) * 3 / 10, //Target BW 30%
	(eCV5546_DRAM_CLOCK * 8/1000) * 4 / 10, //Target BW 40%
	(eCV5546_DRAM_CLOCK * 8/1000) * 5 / 10, //Target BW 50%
	(eCV5546_DRAM_CLOCK * 8/1000) * 6 / 10, //Target BW 60%
	(eCV5546_DRAM_CLOCK * 8/1000) * 7 / 10, //Target BW 70%
	(eCV5546_DRAM_CLOCK * 8/1000) * 8 / 10, //Target BW 80%
	(eCV5546_DRAM_CLOCK * 8/1000) * 9 / 10, //Target BW 90%
	(eCV5546_DRAM_CLOCK * 8/1000), //Target BW 100%
	(eCV5546_DRAM_CLOCK * 8/1000), //Target BW max  (1.1 cycle)
};
...
struct eys3d_dummy_master *sp_dm_test;

static void test_help(void)
{
	pr_info(
		"sp_dm test:\n"
		"  -- This tool works for eYs3D SoC DummyMaster module --\n"
		"  show this help message\n"
		"    echo > /sys/module/eys3d_dm_test/parameters/test\n"
		"  dump clken status\n"
		"    echo 0 0 0 > /sys/module/eys3d_dm_test/parameters/test\n"
...
static int test_set(const char *val, const struct kernel_param *kp)
{
	...
	case 3:
		...
		if ((value1 == 1) && (value2 == 0) && (value3 == 0)) {
			 ...
			 */
			tmp_value1 = eCV5546_SYSTEM_CLOCK;
			tmp_value2 = eCV5546_MAX_BANDWIDTH_DM0/(set_write_length*16*10);
			for(i = 0; i < 10; i++) {
			...
			pr_info("sysclk = %dMHz,  dramclk = %dMHz, max_BW = %dMBps\n",
				eCV5546_SYSTEM_CLOCK/1000, eCV5546_DRAM_CLOCK/1000, eCV5546_MAX_BANDWIDTH_DM0/1000);
			...
			for(i = 0; i < 11; i++) {
				//measure_bandwidth[i] = (total_cmd_count[i] * 256 * (eCV5546_SYSTEM_CLOCK / 100000) / (total_cycle_count[i] / 100));
				measure_bandwidth[i] = (u32)DIV_ROUND_CLOSEST_ULL((u64)total_cmd_count[i]*256*eCV5546_SYSTEM_CLOCK, (u64)total_cycle_count[i]*1000);
			 ...
			 */
			tmp_value1 = eCV5546_SYSTEM_CLOCK;
			tmp_value2 = eCV5546_MAX_BANDWIDTH_DM1/(set_write_length*16*10);
			for(i = 0; i < 10; i++) {
				if(tmp_value1%(tmp_value2 *(i+1)) != 0) {
					tmp_value = tmp_value1/(tmp_value2*(i+1));
					tmp_value = tmp_value1 - (tmp_value * tmp_value2 * (i+1));
			...
			pr_info("sysclk = %dMHz,  dramclk = %dMHz, max_BW = %dMBps\n",
				eCV5546_SYSTEM_CLOCK/1000, eCV5546_DRAM_CLOCK/1000, eCV5546_MAX_BANDWIDTH_DM1/1000);
			pr_info("-- Dummy Master Parameter Setting --\n");
			...
			pr_info("-- Target / Measure / Theory / percentage(%%) --\n");
			for(i = 0; i < 11; i++) {
				//measure_bandwidth[i] = (total_cmd_count[i] * 256 * (eCV5546_SYSTEM_CLOCK / 100) / (total_cycle_count[i] / 100))/1000;
				measure_bandwidth[i] = (u32)DIV_ROUND_CLOSEST_ULL((u64)total_cmd_count[i]*256*eCV5546_SYSTEM_CLOCK, (u64)total_cycle_count[i]*1000);
				measure_bandwidth[i] = (u32)DIV_ROUND_CLOSEST_ULL((u64)measure_bandwidth[i]*1000*1000, (u64)1024*1024);
				//pr_info("%d%% req_period 0x%08x\n", (i+1)*10, request_period_bw[i]);
				if (i == 10)
			...
			pr_info("sysclk = %dMHz,  dramclk = %dMHz, max_BW = %dMBps(DM0) %dMBps(DM1)\n",
				eCV5546_SYSTEM_CLOCK/1000, eCV5546_DRAM_CLOCK/1000, eCV5546_MAX_BANDWIDTH_DM0/1000, eCV5546_MAX_BANDWIDTH_DM1/1000);	
...
static const struct of_device_id sp_dm_test_of_match[] = {
	{ .compatible = "eys3d,ecv5546-dm-test" },
	{}
};
...
static int sp_dm_test_probe(struct platform_device *pdev)
{
	...
	struct eys3d_dummy_master *priv;
	int ret = 0;
...
{
	//struct eys3d_dummy_master *priv = dev_get_drvdata(dev);
...
MODULE_DESCRIPTION("eYs3D SoC DM Test Driver");
