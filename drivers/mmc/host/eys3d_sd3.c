...
/**
 * (C) Copyright 2019 eYs3D Technology. <http://eys3d.com/>
 *
 * eYs3D SD host controller v3.0
 *
 */
...
static void spsdc_prepare_data(struct spsdc_host *host, struct mmc_data *data)
{
	...
	if (likely(host->dmapio_mode == SPSDC_DMA_MODE)) {
		...
		if (unlikely(!count)) {
			spsdc_pr(host->mode, ERROR, "error at dma_mapp_sg: count = %d\n", count);
			data->error = -EINVAL;
			return;
		}
	...
	if (unlikely(cmd->flags & MMC_RSP_136)) {
		...
		if (unlikely(host->dmapio_mode == SPSDC_PIO_MODE && data)) {
			...
			if (!(host->use_int || host->dma_use_int)) {
				...
				if (!wait_for_completion_interruptible_timeout(&host->dma_complete,
									       host->timeout)) {
...
static int spmmc_select_drive_strength(struct mmc_card *card, unsigned int max_dtr,
...
static const struct of_device_id spsdc_of_table[] = {
	{
		.compatible = "eys3d,ecv5546-sd",
		.data = &sp_sd_654_compat,
	},
	{
		.compatible = "eys3d,ecv5546-loscr-sd",
		.data = &sp_sd_654_loscr_compat,
	},
	{
		.compatible = "eys3d,ecv5546-1v8-sdio",
...
static int spsdc_drv_probe(struct platform_device *pdev)
{
	...
	host = mmc_priv(mmc);
	if (!of_property_read_u32(pdev->dev.of_node, "eys3d.driver-type", &host->target_drv))
		host->target_drv = 0;	//0:TypeB 1:TypeA 2:TypeC 3:TypeD

	if (device_property_read_bool(&pdev->dev, "eys3d,high-segs"))
	...
	if (device_property_read_bool(&pdev->dev, "eys3d,high-mem"))
		host->ram = 8;
	else
		host->ram = 3;
	...
	if (devm_request_irq(&pdev->dev, host->irq, spsdc_irq, IRQF_SHARED,
	...
	if (host->segs_no > SPSDC_HW_SEGS) {
		...
		host->buffer = devm_kmalloc(&pdev->dev, SPSDC_MAX_BLK_CNT * 512,
					    GFP_KERNEL | GFP_DMA);
		if (!host->buffer) {
			/*
	...
	mmc->max_blk_size = 512; /* Limited by the max value of dma_size */
...
MODULE_AUTHOR("lh.kuo <eys3d@eys3d.com>");
MODULE_DESCRIPTION("eYs3D SD/SDIO host controller v3.0 driver");
