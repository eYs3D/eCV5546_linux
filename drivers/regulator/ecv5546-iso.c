...
#include <dt-bindings/regulator/eys3d,ecv5546-iso.h>
...
static int reg_iso_probe(struct platform_device *pdev)
{
	...
	if (pdev->dev.of_node) {
		...
		ret = device_property_read_u32(&pdev->dev, "eys3d,iso-selector",
					       &drvdata->iso);
		if (ret) {
			dev_err(&pdev->dev, "No eys3d,iso-selector from DT\n");
			return ret;
		}
		dev_err(&pdev->dev, "eys3d,iso-selector:%d\n", drvdata->iso);
	}

	if (!drvdata->desc.name)
		drvdata->desc.name = "ecv5546-iso";
...
static const struct of_device_id of_match[] = {
	{
		.compatible = "eys3d,ecv5546-regulator-iso",
	},
	{
	},
};
...
MODULE_AUTHOR("YuBo Leng <eys3d@eys3d.com>");
MODULE_DESCRIPTION("eYs3D Power Isolation Regulator");
MODULE_LICENSE("GPL");
