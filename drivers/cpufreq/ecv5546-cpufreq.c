...
static struct ecv5546_cpu_dvfs_info *ecv5546_cpu_dvfs_info_lookup(int cpu)
{
	struct ecv5546_cpu_dvfs_info *info;
...
void ecv5546_dvfs_unlock(void)
{
	struct ecv5546_cpu_dvfs_info *info = ecv5546_cpu_dvfs_info_lookup(0);

	hwspin_unlock_raw(info->hwlock);
}
EXPORT_SYMBOL_GPL(ecv5546_dvfs_unlock);

static int ecv5546_cpufreq_set_target(struct cpufreq_policy *policy,
				     unsigned int index)
{
	struct cpufreq_frequency_table *freq_table = policy->freq_table;
	struct clk *cpu_clk = policy->clk;
	struct ecv5546_cpu_dvfs_info *info = policy->driver_data;
...
static int ecv5546_cpu_dvfs_info_init(struct ecv5546_cpu_dvfs_info *info, int cpu)
{
	struct device *cpu_dev;
...
static void ecv5546_cpu_dvfs_info_release(struct ecv5546_cpu_dvfs_info *info)
{
...
static int ecv5546_cpufreq_init(struct cpufreq_policy *policy)
{
	struct ecv5546_cpu_dvfs_info *info;
	struct cpufreq_frequency_table *freq_table;
	int ret;

	info = ecv5546_cpu_dvfs_info_lookup(policy->cpu);
	if (!info) {
...
static int ecv5546_cpufreq_exit(struct cpufreq_policy *policy)
{
	struct ecv5546_cpu_dvfs_info *info = policy->driver_data;
...
static struct cpufreq_driver ecv5546_cpufreq_driver = {
	.flags = CPUFREQ_NEED_INITIAL_FREQ_CHECK |
		 CPUFREQ_HAVE_GOVERNOR_PER_POLICY |
		 CPUFREQ_IS_COOLING_DEV,
	.verify = cpufreq_generic_frequency_table_verify,
	.target_index = ecv5546_cpufreq_set_target,
	.get = cpufreq_generic_get,
	.init = ecv5546_cpufreq_init,
	.exit = ecv5546_cpufreq_exit,
	.name = "ecv5546-cpufreq",
	.attr = cpufreq_generic_attr,
};

static int ecv5546_cpufreq_probe(struct platform_device *pdev)
{
	struct ecv5546_cpu_dvfs_info *info, *tmp;
	int cpu, ret;

	for_each_possible_cpu(cpu) {
		info = ecv5546_cpu_dvfs_info_lookup(cpu);
		if (info)
			continue;
		...
		ret = ecv5546_cpu_dvfs_info_init(info, cpu);
	...
	ret = cpufreq_register_driver(&ecv5546_cpufreq_driver);
	if (ret) {
		dev_err(&pdev->dev, "failed to register ecv5546 cpufreq driver\n");
...
release_dvfs_info_list:
	list_for_each_entry_safe(info, tmp, &dvfs_info_list, list_head) {
		ecv5546_cpu_dvfs_info_release(info);
...
static struct platform_driver ecv5546_cpufreq_platdrv = {
	.driver = {
		.name	= "ecv5546-cpufreq",
	},
	.probe		= ecv5546_cpufreq_probe,
};

/* List of machines supported by this driver */
static const struct of_device_id ecv5546_cpufreq_machines[] __initconst = {
	{ .compatible = "eys3d,ecv5546", },
	{ }
};
MODULE_DEVICE_TABLE(of, ecv5546_cpufreq_machines);

static int __init ecv5546_cpufreq_driver_init(void)
{
	...
	match = of_match_node(ecv5546_cpufreq_machines, np);
	of_node_put(np);
	if (!match) {
		pr_debug("Machine is not compatible with ecv5546-cpufreq\n");
		return -ENODEV;
	}

	err = platform_driver_register(&ecv5546_cpufreq_platdrv);
	if (err)
		return err;
	 ...
	 * probing.
	 */
	pdev = platform_device_register_simple("ecv5546-cpufreq", -1, NULL, 0);
	if (IS_ERR(pdev)) {
		pr_err("failed to register ecv5546-cpufreq platform device\n");
		return PTR_ERR(pdev);
	}

	return 0;
}
device_initcall(ecv5546_cpufreq_driver_init);

MODULE_DESCRIPTION("eYs3D eCV5546 CPUFreq driver");
