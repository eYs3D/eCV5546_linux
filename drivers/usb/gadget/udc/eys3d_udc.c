...
#ifdef CONFIG_PM
static int udc_eys3d_drv_suspend(struct device *dev);
static int udc_eys3d_drv_resume(struct device *dev);
#endif
...
static const struct of_device_id eys3d_udc_ids[] = {
	{ .compatible = "eys3d,ecv5546-usb-udc" },
	{ }
};
MODULE_DEVICE_TABLE(of, eys3d_udc_ids);

#ifdef CONFIG_PM
struct dev_pm_ops const udc_eys3d_pm_ops = {
	.suspend 		= udc_eys3d_drv_suspend,
	.resume 		= udc_eys3d_drv_resume,
};
#endif
...
#elif !defined(CONFIG_USB_EHCI_EYS3D) && !defined(CONFIG_USB_OHCI_EYS3D)
	usb_switch(1);
#else
...
MODULE_DESCRIPTION("eYs3D USB Device Controller Driver");
MODULE_VERSION("V1.0");
MODULE_LICENSE("GPL");
