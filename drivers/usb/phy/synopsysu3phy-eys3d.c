...
static void typec_gpio(struct work_struct *work)
{
	...
	if (u3phy->dir != gpiod_get_value(u3phy->gpiodir)) {
		...
		if (gpiod_get_value(u3phy->gpiodir)) {
			...
			result = wait_event_timeout(u3phy->wq, !u3phy->busy, msecs_to_jiffies(100));
			//if (!result)
			//	dev_dbg(u3phy->dev, "reset failed 5\n");
				//return -ETIME;
			u3phy->dir = 0;
		}
...
static int sp_u3phy_init(struct phy *phy)
{
	...
	schedule_delayed_work(&u3phy->typecdir, msecs_to_jiffies(2000));
	return 0;
}
...
static int sp_u3phy_power_on(struct phy *phy)
{
	...
	for (i = 0; i < 3; i++) {
		...
		if (!result)
			dev_err(u3phy->dev, "XXXXXx reset failed 1, count %d\n", i);
		else
			break;
	}
	result = readl(&dwc3portsc_reg->cfg[0]);
	...
	if (gpiod_get_value(u3phy->gpiodir)) {
		...
		result = wait_event_timeout(u3phy->wq, !u3phy->busy, msecs_to_jiffies(100));
		//if (!result)
		//	dev_dbg(u3phy->dev, "reset failed 2\n");
			//return -ETIME;
		u3phy->dir = 1;
	} else {
		writel(result | 0x11, &dwc3phy_reg->cfg[5]);
...
static int eys3d_usb_synopsys_u3phy_probe(struct platform_device *pdev)
{
	u32 ret;
...
static int eys3d_usb_synopsys_u3phy_remove(struct platform_device *pdev)
{
	struct usb3_phy *u3phy = platform_get_drvdata(pdev);
...
static const struct of_device_id synopsys_u3phy_eys3d_dt_ids[] = {
	{ .compatible = "eys3d,usb3-phy" },
	{ }
};
MODULE_DEVICE_TABLE(of, synopsys_u3phy_eys3d_dt_ids);

static struct platform_driver eys3d_synopsys_u3phy_driver = {
	.probe		= eys3d_usb_synopsys_u3phy_probe,
	.remove		= eys3d_usb_synopsys_u3phy_remove,
	.driver		= {
		.name	= "eys3d-usb-synopsys-u3phy",
		.of_match_table = synopsys_u3phy_eys3d_dt_ids,
	},
};
module_platform_driver(eys3d_synopsys_u3phy_driver);

MODULE_ALIAS("eys3d_usb_synpsys_u3phy");
