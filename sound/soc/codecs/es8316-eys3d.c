...
/*
 * es831-eys3d.c -- eYs3D es8316 ALSA SoC audio driver
 * Copyright Everest Semiconductor Co.,Ltd
 *
 * Authors: ChingChou Huang <eys3d@eys3d.com>
 */
...
static void es8316_enable_micbias_for_mic_gnd_short_detect(struct snd_soc_component *component)
{
...
static void es8316_disable_micbias_for_mic_gnd_short_detect(struct snd_soc_component *component)
{
	struct snd_soc_dapm_context *dapm = snd_soc_component_get_dapm(component);
...
static int es8316_probe(struct snd_soc_component *component)
{
	...
	snd_soc_component_write(component, ES8316_RESET, ES8316_RESET_CSM_ON | 0x40);
	msleep(30);
	...
	mask = 0x3f;
	snd_soc_component_update_bits(component, ES8316_SYS_PDN, mask, 0x0);
	/* Enable BCLK and MCLK inputs in slave mode */
	clksw = ES8316_CLKMGR_CLKSW_MCLK_ON | ES8316_CLKMGR_CLKSW_BCLK_ON | 0x10;
	snd_soc_component_update_bits(component, ES8316_CLKMGR_CLKSW, clksw, clksw);
	...
	mask = 0xc0;
	snd_soc_component_update_bits(component, ES8316_DAC_SET1, mask, 0);
	mask = 0x77;
	snd_soc_component_update_bits(component, ES8316_HPMIX_SEL, mask, 0x11);
	mask = 0xf0;
	snd_soc_component_update_bits(component, ES8316_ADC_PDN_LINSEL, mask, 0x10);

	snd_soc_component_write(component, ES8316_SYS_LP1, 0);//0x3f
	snd_soc_component_write(component, ES8316_SYS_LP2, 0);//0x1f
	snd_soc_component_write(component, ES8316_CPHP_ICAL_VOL, 0);
	mask = 0x03;
	snd_soc_component_update_bits(component, ES8316_ADC_DMIC, mask, 0);
	mask = 0xcc;
	snd_soc_component_update_bits(component, ES8316_HPMIX_SWITCH, mask, 0x88);
	mask = 0xff;
	snd_soc_component_update_bits(component, ES8316_HPMIX_PDN, mask, 0x0);
	mask = 0xff;
	snd_soc_component_update_bits(component, ES8316_HPMIX_VOL, mask, 0xbb);
	mask = 0x3f;
	snd_soc_component_update_bits(component, ES8316_CPHP_PDN2, mask, 0x10);
	mask = 0xff;
	snd_soc_component_update_bits(component, ES8316_CPHP_LDOCTL, mask, 0x30);
	mask = 0x07;
	snd_soc_component_update_bits(component, ES8316_CPHP_PDN1, mask, 0x02);
	mask = 0x11;
	snd_soc_component_update_bits(component, ES8316_DAC_PDN, mask, 0);
	mask = 0xff;
	snd_soc_component_update_bits(component, ES8316_CPHP_OUTEN, mask, 0x66);
	snd_soc_component_write(component, ES8316_DAC_VOLL, 0); //playback volume
	snd_soc_component_write(component, ES8316_DAC_VOLR, 0); //playback volume
	snd_soc_component_write(component, ES8316_ADC_VOLUME, 0); //ADC capture volume
	mask = 0xf0;
	snd_soc_component_update_bits(component, ES8316_ADC_PGAGAIN, mask, 0xa0);
...
#if 1 // not binding with sound card
...
MODULE_DESCRIPTION("eYs3D ES8316 ALSA SoC Codec Driver");
MODULE_AUTHOR("ChingChou Huang <eys3d@eys3d.com>");
