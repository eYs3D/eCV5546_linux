...
//		Global Parameters
//--------------------------------------------------------------------------
struct eys3d_audio_base *spauddata = NULL;
...
static int spsoc_pcm_open(struct snd_soc_component *component, struct snd_pcm_substream *substream)
{
	...
	if (!IS_ENABLED(CONFIG_SND_SOC_ES8316_EYS3D) &&
...
static const struct snd_soc_component_driver eys3d_soc_platform = {
	//.ops		= &spsoc_pcm_ops,
...
{
	struct device_node *np  = of_find_compatible_node(NULL, NULL, "eys3d,audio");
...
static int snd_spsoc_pcm_probe(struct platform_device *pdev)
{
	...
	ret = devm_snd_soc_register_component(&pdev->dev, &eys3d_soc_platform, NULL, 0);
...
static struct platform_driver snd_spsoc_driver = {
	.driver = {
		...
		.owner	= THIS_MODULE,
		//.of_match_table	= of_match_ptr(eys3d_audio_pcm_dt_ids),
	},
...
MODULE_DESCRIPTION("eYs3D SoC ALSA PCM module");
MODULE_LICENSE("GPL");
