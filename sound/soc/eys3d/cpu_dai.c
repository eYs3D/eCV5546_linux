...
static const struct snd_soc_component_driver eys3d_cpu_component = {
	.name =	"spsoc-cpu-dai",
};

int eys3d_i2s_register(struct	device *dev)
{
	int ret	= 0;
	...
	pr_debug("eCV5546 aud set done\n");

	ret = devm_snd_soc_register_component(dev, &eys3d_cpu_component, aud_cpu_dai,
					      ARRAY_SIZE(aud_cpu_dai));
	return ret;
}
EXPORT_SYMBOL_GPL(eys3d_i2s_register);
